...
#endif  // NDEBUG
#ifdef WITH_CHANGJIANG_STORAGE_ENGINE
extern bool changjiang_sysvar_engine_force;
#endif
...
#ifdef WITH_CHANGJIANG_STORAGE_ENGINE
  if (changjiang_sysvar_engine_force && changjiang_hton != nullptr && changjiang_hton->state == SHOW_OPTION_YES &&
      /*!thd->is_system_thread() && */
      (my_strcasecmp(system_charset_info, "mysql", create_table->db) != 0 &&
       my_strcasecmp(system_charset_info, "information_schema", create_table->db) != 0 &&
       my_strcasecmp(system_charset_info, "performance_schema", create_table->db) != 0 &&
       my_strcasecmp(system_charset_info, "sys", create_table->db) != 0))
    create_info.db_type = changjiang_hton;
#endif
