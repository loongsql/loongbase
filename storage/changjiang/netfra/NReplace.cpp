  ...
  for (int n = 0; n < index->numberFields; ++n) {
    ...
    if (!node)
      throw SQLEXCEPTION(SYNTAX_ERROR,
                         "primary key field \"%s\" not in replace field list",
...
void NReplace::evalStatement(Statement *statement) {
  if (select) {
    ...
    while (resultSet->next()) doReplace(statement, resultSet);
  } else
...
void NReplace::prettyPrint(int level, PrettyPrint *pp) {
  ...
  if (select) select->prettyPrint(level, pp);
}
