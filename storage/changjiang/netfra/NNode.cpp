...
NNode::NNode(CompiledStatement *statement, NType nType, int numberChildren) {
  ...
  if (numberChildren) allocChildren(numberChildren);
  ...
  switch (type) {
    ...
    case Upper:
    case Lower:
      if (statement) valueSlot = statement->getValueSlot();
      break;
  ...
  switch (type) {
    case CaseSearch: {
      ...
      for (n = 1; n + 1 < count; n += 2) {
        ...
        if (val->compare(test) == 0) return children[n + 1]->eval(statement);
      }

      if (n < count) return children[n]->eval(statement);
    ...
    case Case: {
      ...
      if (count & 1) return children[count - 1]->eval(statement);
...
void NNode::setChild(int index, NNode *node) { children[index] = node; }

void NNode::decomposeConjuncts(LinkedList &conjuncts) {
  if (type == And) {
    for (int n = 0; n < count; ++n) children[n]->decomposeConjuncts(conjuncts);
  } else
...
bool NNode::computable(CompiledStatement *statement) {
  for (int n = 0; n < count; ++n)
    if (!children[n]->computable(statement)) return false;

  return true;
}

void NNode::matchIndex(CompiledStatement *statement, Context *context,
...
int NNode::matchField(Context *context, Index *index) { return -1; }

Field *NNode::getField() { return NULL; }

bool NNode::isMember(Table *table) {
  for (int n = 0; n < count; ++n)
    if (children[n]->isMember(table)) return true;
...
bool NNode::references(Table *table) {
  for (int n = 0; n < count; ++n)
    if (children[n]->references(table)) return true;

  return false;
}

const char *NNode::getName() { return ""; }
...
void NNode::prettyPrint(int level, PrettyPrint *pp) {
  ...
  for (int n = 0; n < count; ++n) printNode(level, children[n], pp);
}
...
NNode **NNode::copyVector(int numberNodes, NNode **nodes) {
  ...
  for (int n = 0; n < numberNodes; ++n) newNodes[n] = nodes[n];
...
void NNode::gen(Stream *stream) {
  ...
  if (paren) stream->putCharacter('(');

  for (int n = 0; n < count; ++n) {
    if (n != 0) stream->putSegment(op);
    children[n]->gen(stream);
  }
...
bool NNode::isStatistical() {
  ...
  for (int n = 0; n < count; ++n)
    if (!children[n]->isStatistical()) return false;
...
void NNode::increment(Statement *statement) {
  switch (type) {
    ...
    case Mod:
    case Negate: {
      for (int n = 0; n < count; ++n) children[n]->increment(statement);
    } break;
...
NNode *NNode::makeInversion(CompiledStatement *statement, Context *context,
                            Index *index) {
  if (type == Starting) {
    ...
    if (!children[1]->computable(statement)) return NULL;

    NBitmap *bitmap =
        new NBitmap(statement, index, 1, children + 1, children + 1);
    bitmap->partial = true;
...
bool NNode::equiv(NNode *node) {
  ...
  for (int n = 0; n < count; ++n)
    if (!children[n]->equiv(node->children[n])) return false;

  return true;
}

bool NNode::isMember(NNode *node) {
  if (node->type == ValueAlias) node = ((NAlias *)node)->expr;

  for (int n = 0; n < count; ++n)
    if (children[n]->equiv(node)) return true;
...
int NNode::evalBoolean(Statement *statement) {
  switch (type) {
    ...
    case Lss: {
      ...
      if (v1->isNull()) return NULL_BOOLEAN;

      v2 = children[1]->eval(statement);

      if (v2->isNull()) return NULL_BOOLEAN;

      int comparison =
          (collation) ? collation->compare(v1, v2) : v1->compare(v2);
    ...
    case Not: {
      ...
      if (ret == NULL_BOOLEAN) return ret;

      if (ret == FALSE_BOOLEAN) return TRUE_BOOLEAN;
    ...
    case IsActiveRole: {
      ...
      if (!string1[0]) return TRUE_BOOLEAN;
...
void NNode::evalStatement(Statement *statement) { ASSERT(false); }
...
bool NNode::isRedundantIndex(LinkedList *indexes) { return false; }

bool NNode::isInvertible(CompiledStatement *statement, Context *context) {
  switch (type) {
    case Or: {
      for (int n = 0; n < count; ++n)
        if (!statement->isInvertible(context, children[n])) return false;
    ...
    case And: {
      for (int n = 0; n < count; ++n)
        if (statement->isInvertible(context, children[n])) return true;
...
void NNode::open(Statement *statement) {
  for (int n = 0; n < count; ++n) children[n]->open(statement);
}

void NNode::close(Statement *statement) {
  for (int n = 0; n < count; ++n) children[n]->close(statement);
}

bool NNode::isUniqueIndex() { return false; }
