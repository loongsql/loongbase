...
NSelect::NSelect(CompiledStatement *statement, Syntax *syntax, NNode *inExpr)
    : NNode(statement, Select) {
  int debug = 0;
  ...
  if (syntax->type == nod_select) {
    ...
    if (pUnion->count != 1) {
      ...
      for (int n = 0; n < count; ++n) {
        NSelect *subStream = unionBranches[n] =
            new NSelect(statement, pUnion->getChild(n), NULL);
  ...
  if (items) {
    ...
    if (view)
      for (int id = 0; id < view->numberColumns; ++id)
        values->setChild(n++,
                         view->columns->children[id]->copy(statement, context));
    else
  ...
  if (groupBy) {
    ...
    if (countSlot < 0) countSlot = statement->getGeneralSlot();
  }

  for (n = 0; n < values->count; ++n)
    if (values->children[n]->isStatistical()) statistical = true;
  ...
  for (n = 0; n < values->count; ++n) {
    ...
    if (stat != statistical)
      if (!groups || !groups->isMember(child))
        throw SQLEXCEPTION(COMPILE_ERROR,
                           "mix of statistic/scalar expression in select list");
  }
  ...
  if (filter) {
    if (statement->filteredTables.isMember(table))
      throw SQLEXCEPTION(
          COMPILE_ERROR, "filterset %s.%s has circular reference to %s.%s",
...
void NSelect::evalStatement(Statement *statement) {
  ...
  if (countSlot >= 0) statement->slots[countSlot] = 0;
}
...
bool NSelect::next(Statement *statement, ResultSet *resultSet) {
  ...
  if (statistical && !groups) {
    if (statement->eof) return false;

    NNode **children = values->children;

    for (int n = 0; n < numberColumns; ++n) children[n]->reset(statement);

    while (stream->fetch(statement))
      for (int n = 0; n < numberColumns; ++n) children[n]->increment(statement);
  ...
  /***
  if (unionBranches)
          unionBranches [stream->getStreamIndex (statement)]->setValues
  (statement, resultSet); else setValues (statement, resultSet);
  ***/
...
Field *NSelect::getField(int index) {
  if (unionBranches) return unionBranches[0]->getField(index);

  return values->children[index]->getField();
}

bool NSelect::isMember(Table *table) { return values->isMember(table); }

bool NSelect::references(Table *table) {
  for (int n = 0; n < numberContexts; ++n)
    if (contexts[n]->table == table) return true;
...
Field *NSelect::getField(const char *fieldName) {
  for (int n = 0; n < numberColumns; ++n) {
    ...
    if (field && !strcasecmp(fieldName, field->name)) return field;
  }
...
const char *NSelect::getColumnName(int index) {
  if (unionBranches) return unionBranches[0]->getColumnName(index);

  return values->children[index]->getName();
}

int NSelect::getColumnIndex(const char *columnName) {
  if (unionBranches) return unionBranches[0]->getColumnIndex(columnName);

  for (int n = 0; n < numberColumns; ++n)
    if (!strcasecmp(columnNames[n], columnName)) return n;
...
bool NSelect::computable(CompiledStatement *statement) {
  if (!orgBoolean) return true;

  int n;

  for (n = 0; n < numberContexts; ++n) statement->pushContext(contexts[n]);

  bool ret = orgBoolean->computable(statement);

  for (n = 0; n < numberContexts; ++n) statement->popContext();
...
int NSelect::compileJoin(Syntax *node, bool innerJoin, NSelectEnv *env) {
  ...
  switch (node->type) {
    ...
    case nod_table: {
      ...
      context->getTableContexts(*env->tables,
                                (innerJoin) ? CtxInnerJoin : CtxOuterJoin);
    ...
    default:
      ...
      throw SQLEXCEPTION(FEATURE_NOT_YET_IMPLEMENTED,
                         "join type is not yet implemented");
  }
...
void NSelect::prettyPrint(int level, PrettyPrint *pp) {
  ...
  if (stream) stream->prettyPrint(level, pp);
}

NNode *NSelect::getValue(const char *name) {
  if (columnNames)
    for (int n = 0; n < numberColumns; ++n)
      if (!strcmp(columnNames[n], name)) return values->children[n];
...
int NSelect::getNumberValues() { return numberColumns; }

void NSelect::pushContexts() {
  for (int n = 0; n < numberContexts; ++n) statement->pushContext(contexts[n]);
}
