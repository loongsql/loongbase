...
NBitmap::NBitmap(CompiledStatement *statement, Index *idx, int numberNodes,
                 NNode **min, NNode **max)
    : NNode(statement, BitmapNode) {
  ...
  for (int n = 0; n < nodeCount; ++n) {
    if (minNodes[n] != maxNodes[n]) equality = false;
    if (maxNodes[n])
      values = n + 1;
    else {
      partial = true;
...
void NBitmap::prettyPrint(int level, PrettyPrint *pp) {
  ...
  for (int n = 0; n < nodeCount; ++n) {
    ...
    if (minNodes[n] != maxNodes[n]) diff = true;
  }

  if (diff)
    for (int n = 0; n < nodeCount; ++n) printNode(level, maxNodes[n], pp);
}
...
bool NBitmap::isRedundantIndex(LinkedList *indexes) {
  ...
  if (node != this && node->type == BitmapNode && !node->redundant)
    if (isRedundantIndex(node)) return true;
  END_FOR;
...
bool NBitmap::isRedundantIndex(NBitmap *node) {
  ...
  if (!equality || !node->equality) return false;

  // If we more specific than the other guy, we're done

  if (values > node->values) return false;
  ...
  for (n = 0; n < cnt; ++n)
    if (!minNodes[n]->equiv(node->minNodes[n])) return false;

  // If we match the same number of values, pick the denser (shorter)
  // index.

  if (values == node->values) {
    if (index->numberFields < node->index->numberFields) return false;
  }
...
bool NBitmap::isUniqueIndex() {
  ...
  if (!equality || partial) return false;

  if (!INDEX_IS_UNIQUE(index->type)) return false;
