...
int Filter::getWord(int bufferLength, char *buffer) {
  ...
  for (;; ++p) {
    if (p >= endSegment && !nextSegment()) return 0;
  ...
  while (q < end) {
    if (p >= endSegment && !nextSegment()) break;
    char c = *p++;
    if (characters[(int)c] & (WHITE | DELIMITER)) break;
    *q++ = c;
  }
...
void Filter::start() {
  ...
  if ((segment = stream.segments)) {
    ...
    for (;; ++p) {
      if (p >= endSegment && !nextSegment()) return;
      char type = characters[(int)*p];
      if (!(type & (WHITE))) break;
    }
...
const char *Filter::nextSegment() {
  if (!(segment = segment->next)) return NULL;
