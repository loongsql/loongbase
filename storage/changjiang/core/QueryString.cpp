...
QueryString::QueryString() { useCount = 1; }
...
int QueryString::getParameter(const char *queryString, const char *name,
...
void QueryString::setString(const char *string) {
  if (!(queryString = string)) queryString = "";
  ...
  for (const char *p = queryString; *p;) {
    ...
    *q = 0;
    ...
    for (ptr = variables + slot; (parameter = *ptr);
         ptr = &parameter->collision) {
    ...
    *ptr =
        new Parameter(NULL, temp, (int)strlen(temp), value, (int)strlen(value));
  }
...
Parameter *QueryString::findParameter(const WCString *name) {
  ...
  for (Parameter *variable = variables[slot]; variable;
       variable = variable->collision)
    if (variable->isNamed(name)) return variable;

  return NULL;
}

void QueryString::addRef() { INTERLOCKED_INCREMENT(useCount); }
