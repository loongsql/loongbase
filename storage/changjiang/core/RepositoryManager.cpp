...
Repository *RepositoryManager::findRepository(const char *schema,
                                              const char *name) {
  Repository *repository;
  int slot = HASH(schema, REPOSITORY_HASH_SIZE);

  for (repository = repositories[slot]; repository;
       repository = repository->collision)
    if (repository->name == name) return repository;

  PreparedStatement *statement = database->prepareStatement(
      "select sequenceName,filename,rollovers,currentVolume from "
      "system.repositories where schema=? and repositoryName=?");
  ...
  if (resultSet->next()) try {
      ...
      Sequence *sequence =
          database->sequenceManager->getSequence(schema, sequenceName);
      repository = new Repository(name, schema, database, sequence, fileName,
                                  rollovers, volume);
...
Repository *RepositoryManager::getRepository(const char *schema,
                                             const char *name) {
...
Repository *RepositoryManager::createRepository(
    const char *name, const char *schema, Sequence *sequence,
    const char *fileName, int volume, const char *rolloverString) {
  ...
  if (!fileName) fileName = "%d/repositories/%s-%r-%v.nfr";

  repository = new Repository(name, schema, database, sequence, fileName,
                              rolloverString, (volume) ? volume : 1);
  int slot = HASH(schema, REPOSITORY_HASH_SIZE);
  repository->collision = repositories[slot];
  repositories[slot] = repository;
  repository->save();
...
void RepositoryManager::execute(Scheduler *scheduler) {
  for (int n = 0; n < REPOSITORY_HASH_SIZE; ++n)
    for (Repository *repository = repositories[n]; repository;
         repository = repository->collision)
      repository->scavenge();
