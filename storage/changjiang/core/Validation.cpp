...
void Validation::sectionInUse(int sectionId) { sections.set(sectionId); }
...
bool Validation::error(const char *text, ...) {
  ...
  if (options & validateOrBreak) Error::error("  %s\n", temp);
...
bool Validation::isPageType(Bdb *bdb, PageType type, const char *text, ...) {
  ...
  if (phase == 0)
    error("Page %d (%s) wrong type; expected %d, got %d", bdb->pageNumber, temp,
          type, bdb->buffer->pageType);
...
bool Validation::minutia() { return (options & validateMinutia) != 0; }
