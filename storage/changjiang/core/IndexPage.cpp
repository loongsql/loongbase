...
#define SUPERNODE_COMPRESSION_LOSS_LIMIT(pageSize) \
  (pageSize) / (SUPERNODES + 1) / 2

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

AddNodeResult IndexPage::addNode(Dbb *dbb, IndexKey *indexKey,
                                 int32 recordNumber) {
...
AddNodeResult IndexPage::addNode(Dbb *dbb, IndexKey *indexKey,
                                 int32 recordNumber, IndexNode *node,
                                 IndexKey *priorKey, IndexKey *nextKey) {
  UCHAR *key = indexKey->key;
  ...
  if (dbb->debug & (DEBUG_PAGES | DEBUG_NODES_ADDED)) printPage(this, 0, false);
  // validate(NULL);

  // Compute delta size of new node
  int offset1 = computePrefix(priorKey, indexKey);

  // Check whether inserted node shall be supernode. If yes, does not compress
  // the prefix.
  bool makeNextSuper;
  bool makeSuper = checkAddSuperNode(dbb->pageSize, node, indexKey,
                                     recordNumber, offset1, &makeNextSuper);
  if (makeSuper) offset1 = 0;
  ...
  if ((UCHAR *)node->node == (UCHAR *)this + length) {
    ...
    if (node->length >= 128 && (node->length - deltaOffset) < 128) --delta;

    if (node->offset < 128 && (node->offset + deltaOffset) >= 128) ++delta;
  ...
  if (length + delta > dbb->pageSize) {
    if ((char *)node->nextNode < (char *)this + length) return SplitMiddle;

    if (node->getNumber() == END_LEVEL) return SplitEnd;

    if (offset2 == -1 || nextKey->compare(indexKey) >= 0) return NextPage;
  ...
  ***/
  ...
  if (offset2 >= 0) {
    ...
    if (tailLength > 0) moveMemory(tail + delta, tail);
  }
  ...
  if (makeSuper) addSupernode(node->node);

  newNode.insert(node->node, offset1, length1, key, recordNumber);

  // If necessary, rebuild next node
  if (makeNextSuper) addSupernode(newNode.nextNode);

  if (offset2 >= 0)
    newNode.insert(newNode.nextNode, offset2, nextKey->keyLength - offset2,
                   nextKey->key, nextNumber);
...
Btn *IndexPage::findNodeInLeaf(IndexKey *indexKey, IndexKey *foundKey) {
  ...
  if (indexKey->keyLength == 0) {
    if (foundKey) foundKey->keyLength = 0;
  ...
  for (;; node.getNext(bucketEnd)) {
    if (node.node >= bucketEnd) {
      ...
      if (foundKey) foundKey->keyLength = priorLength;
    ...
    if (number == END_LEVEL) break;

    if (node.offset < offset) break;

    if (node.offset > offset || node.length == 0) continue;
    ...
    for (;;) {
      if (p == keyEnd) goto exit;

      if (q == nodeEnd) {
        if (indexKey->checkTail(p) < 0) goto exit;
        break;
      }
...
int IndexPage::computePrefix(IndexKey *key1, IndexKey *key2) {
  ...
  if (key1->recordNumber) len1 -= key1->getAppendedRecordNumberLength();

  int len2 = key2->keyLength;
  if (key2->recordNumber) len2 -= key2->getAppendedRecordNumberLength();
...
Bdb *IndexPage::splitIndexPageMiddle(Dbb *dbb, Bdb *bdb, IndexKey *splitKey,
                                     TransId transId) {
  if (dbb->debug & (DEBUG_PAGES | DEBUG_SPLIT_PAGE)) printPage(this, 0, false);
  ...
  Btn *midpoint =
      (Btn *)((UCHAR *)nodes + (length - OFFSET(IndexPage *, nodes)) / 2);
  ...
  for (; node.node < pageEnd; node.getNext(bucketEnd)) {
    ...
    if (node.offset || node.length) {
      if (node.nextNode > midpoint) break;
  ...
  if ((UCHAR *)node.nextNode > (UCHAR *)this + length) node = prevNode;
  ...
  // validate(split);
  split->length =
      (int)(((UCHAR *)newNode.nextNode + tailLength) - (UCHAR *)split);
  // split->validate(this);

  if (level == 0) splitKey->appendRecordNumber(recordNumber);

  // Split supernodes
  memset(split->superNodes, 0, sizeof(split->superNodes));
  int i = 0;
  for (i = 0; i < SUPERNODES && superNodes[i]; i++)
    if (nodes + superNodes[i] >= node.node) break;
...
Btn *IndexPage::findNodeInBranch(IndexKey *searchKey,
                                 int32 searchRecordNumber) {
  ASSERT(level > 0);
  ...
  bool hitSupernode;
  Btn *super = findSupernode(level, key, keyEnd - key, searchRecordNumber, 0,
                             &hitSupernode);

  if (hitSupernode) return super;

  Btn *prior = super;

  for (IndexNode node(super); node.node < bucketEnd;
       prior = node.node, node.getNext(bucketEnd)) {
    if (node.offset < matchedOffset) return prior;

    if (node.getNumber() == END_LEVEL) return prior;

    if (node.offset > matchedOffset || node.length == 0) continue;
    ...
    for (;;) {
      ...
      if (q >= nodeEnd || *p > *q) break;  // searchKey > node, keep looking
...
void IndexPage::validate(void *before) {
  ...
  for (int i = 0; i < SUPERNODES; i++) {
    ...
    if (superNodes[i] < 0) FATAL("Index validate: negative supernode");
  ...
  for (IndexNode node(this);; node.getNext(bucketEnd)) {
    if (node.node >= bucketEnd) {
      if (node.node != bucketEnd) {
        if (before) printPage((IndexPage *)before, 0, false);
    ...
    if (superIdx < SUPERNODES && superNodes[superIdx]) {
      ...
      if (delta == 0) {
        superIdx++;
        isSuper = true;
        if (node.offset != 0) FATAL("Non-zero offset in supernode");
      } else if (delta < 0)
        FATAL("Index validate: next supernode offset < current offset");
    }

    keyLength = node.keyLength();
    if (level > 0) keyLength -= node.getAppendedRecordNumberLength();

    if (isSuper && keyLength == 0) FATAL("Supernode with 0 key length");
    ...
    if (node.number == END_LEVEL) break;
  ...
  if (superIdx != supernodeCount) FATAL("lost supernodes");
}
...
Bdb *IndexPage::findLevel(Dbb *dbb, int32 indexId, Bdb *bdb, int level,
...
int IndexPage::deleteNode(Dbb *dbb, IndexKey *indexKey, int32 recordNumber) {
  ...
  for (bool first = true; node.node < bucketEnd;
       node.getNext(bucketEnd), first = false) {
    offset = (int)((char *)node.node - (char *)this);

    if (!first && (node.offset != keyLength || node.length != 0)) return -1;
    ...
    if (number == recordNumber) {
      ...
      if (next.node >= bucketEnd)
        ...
        if (deleteSupernode(node.nextNode)) {
          if (checkAddSuperNode(dbb->pageSize, &node, &nextKey,
                                next.getNumber(), prefix, 0)) {
            prefix = 0;
        ...
        node.insert(node.node, prefix, nextKey.keyLength - prefix, nextKey.key,
                    num);
        ...
        if (tailLength > 0) moveMemory(newTail, tail);

        if (supernodePosition) addSupernode(supernodePosition);
...
void IndexPage::printPage(IndexPage *page, int32 pageNumber, bool inversion) {
  ...
  Log::debug("Btree Page %d, level %d, length %d, next %d\n", pageNumber,
             page->level, page->length, page->nextPage);
  ...
  for (; node.node < end; node.getNext(end)) {
    bool isSuper = false;
    if (superIdx < SUPERNODES &&
        (node.node - page->nodes == page->superNodes[superIdx])) {
      superIdx++;
      isSuper = true;
    }
    Log::debug(
        "   %d%s. offset %d, length %d, %d: ", (char *)node.node - (char *)page,
        (isSuper ? "(super)" : ""), node.offset, node.length, node.getNumber());
    node.expandKey(key);
...
void IndexPage::printPage(IndexPage *page, int32 pageNum, bool printDetail,
                          bool inversion) {
  LogLock logLock;
  ...
  for (int i = 0; node.node < end; node.getNext(end), i++) {
    if (i < 3 || printDetail)
      ...
      if (i > 3) Log::debug("       ...\n");
      if (i > 4) printNode(i - 2, page, pageNumber, lastNode2, inversion);
      if (i > 3) printNode(i - 1, page, pageNumber, lastNode1, inversion);
      printNode(i, page, pageNumber, node, inversion);
    }
...
void IndexPage::printNode(int i, IndexPage *page, int32 pageNumber,
                          IndexNode &node, bool inversion) {
  UCHAR key[MAX_PHYSICAL_KEY_LENGTH];
  ...
  if (page->level == 0)
    ...
    Log::debug(
        "%4d:  page:   %6d,  ofs: %4d,  len: %4d,  pgofs: %4d, record: %6d | "
        "%6d ",
        i,                                 // node count
...
void IndexPage::printNode(IndexPage *page, int32 pageNumber, Btn *node,
                          bool inversion) {
  Btn *end = (Btn *)((UCHAR *)page + page->length);
  IndexNode tmp(page);
  int i;
...

  {
    ...
    Log::debug("Page: %d  level: %d  length: %d  next: %d  parent: %d\n",
               pageNumber, page->level, page->length, page->nextPage);

    printNode(i, page, pageNumber, tmp, inversion);
  }
}

void IndexPage::validate(Dbb *dbb, Validation *validation, Bitmap *pages,
      ...
      if (pageNumber > 0) {
        ...
        if (level == 1 && bdb->buffer->pageType == PAGE_inversion) {
          ...
          inversionPage->validate(dbb, validation, &children);
                       ...
                       bdb, PAGE_btree,
                       "intermediate index page a, index id %d",
                       validation->indexId)) {
          children.set(pageNumber);
  ...
  if (level) validateNodes(dbb, validation, &children, parentPageNumber);
  ...
  for (int32 pageNumber = nextPage; pageNumber;) {
    ...
    if (validation->isPageType(bdb, PAGE_btree,
                               "intermediate index page b, index id %d",
                               validation->indexId)) {
...
void IndexPage::validateNodes(Dbb *dbb, Validation *validation,
                              Bitmap *children, int32 parentPageNumber) {
  Btn *bucketEnd = getEnd();
  ...
  for (IndexNode node(this); node.node < bucketEnd; node.getNext(bucketEnd)) {
    ...
    if (!children->isSet(pageNumber)) {
      validation->error("lost index child page %d, index id %d", pageNumber,
                        validation->indexId);
...
recordNumber); Btn::printKey ("insert key: ", keyLength, key, 0, false);
        printPage (this, 0, false);
        //ASSERT (false);
}
***/
...
void IndexPage::analyze(int pageNumber) {
  ...
  Log::debug(
      "Index page %d, level %d, %d nodes, %d byte compressed, %d bytes "
      "uncompressed\n",
...
Bdb *IndexPage::splitIndexPageEnd(Dbb *dbb, Bdb *bdb, TransId transId,
  ...
  int nodeOverhead =
      3 + (offset >= 128) + (insertKey->keyLength - offset >= 128);
  char *nextNextNode = (char *)node.nextNode - node.length +
                       insertKey->keyLength - offset + nodeOverhead;
  ...
  if (nextNextNode >= (char *)this + dbb->pageSize) {
    ...
    deleteSupernode(node.node);
    node.insert(node.node, offset, insertKey->keyLength - offset,
                insertKey->key, END_BUCKET);
    length = (int)((char *)(node.nextNode) - (char *)this);
  }

  for (int i = 0; i < SUPERNODES; i++)
    if (nodes + superNodes[i] >= splitPos) superNodes[i] = 0;
...
Btn *IndexPage::appendNode(IndexKey *newKey, int32 recordNumber, int pageSize) {
  ...
  for (; node.node < end; node.getNext(end)) node.expandKey(&key);

  int offset = computePrefix(newKey, &key);

  if (length + (int)newKey->keyLength - offset >= pageSize)
    throw SQLError(INDEX_OVERFLOW,
                   "Cannot split page; expanded key will not fit");
...
Btn *IndexPage::findInsertionPoint(IndexKey *indexKey, int32 recordNumber,
                                   IndexKey *expandedKey) {
  Btn *bucketEnd = getEnd();

  return findInsertionPoint(level, indexKey, recordNumber, expandedKey, nodes,
                            bucketEnd);
}
...
void IndexPage::logIndexPage(Bdb *bdb, TransId transId, Index *index) {
  ...
  if (dbb->streamLog->recovering) return;

  ASSERT(bdb->useCount > 0);
  ASSERT(bdb->lockType == Exclusive);
  ASSERT(bdb->isDirty);
  IndexPage *indexPage = (IndexPage *)bdb->buffer;
  dbb->streamLog->logControl->indexPage.append(
      dbb, transId, index ? index->indexVersion : INDEX_CURRENT_VERSION,
      bdb->pageNumber, indexPage->level, indexPage->nextPage,
      indexPage->length - OFFSET(IndexPage *, superNodes),
      (const UCHAR *)indexPage->superNodes);
}

Btn *IndexPage::findInsertionPoint(int level, IndexKey *indexKey,
                                   int32 recordNumber, IndexKey *expandedKey,
                                   Btn *from, Btn *bucketEnd) {
  UCHAR *key = indexKey->key;
  ...
  Btn *super = findSupernode(
      level, key, keyEnd - key,
  ...
  if (node.node != nodes) {
    ...
    if (level) expandedKey->recordNumber = node.getAppendedRecordNumber();
  }
  ...
  for (;; node.getNext(bucketEnd)) {
    ...
    if (number == END_LEVEL || number == END_BUCKET) break;

    if (node.offset < matchedOffset) break;

    if (node.offset > matchedOffset) continue;

    const UCHAR *p = key + node.offset;
    const UCHAR *q = node.key;
    const UCHAR *nodeEnd = q + node.length;
    if (level) nodeEnd -= node.getAppendedRecordNumberLength();

    for (;;) {
      if (p >= keyEnd) {
        if (q < nodeEnd) goto exit;  // key < node, use this node.
        ...
        if (level) {
          ...
          if (recordNumber > node.number) break;  // keep looking.
        }

        goto exit;  // key record number <= node record number, use this node.
      }

      if (q >= nodeEnd || *p > *q) break;  // key > node, keep looking.

      if (*p++ < *q++) goto exit;  // key < node, use this node.
    }
...
Btn *IndexPage::getEnd(void) { return (Btn *)((UCHAR *)this + length); }
...
/* During node insertion, check whether supernode should be added at insertion
 * point */
bool IndexPage::checkAddSuperNode(int pageSize, IndexNode *node,
                                  IndexKey *indexKey, int recordNumber,
                                  int offset, bool *makeNextSuper) {
  Btn *insertionPoint = node->node;

  if (makeNextSuper) *makeNextSuper = false;

  if (!changjiang_use_supernodes) return false;
  ...
  if (offset >= SUPERNODE_COMPRESSION_LOSS_LIMIT(
                    pageSize))  // compression is too good, no super
    return false;
  ...
  if (level) keyLen -= indexKey->getAppendedRecordNumberLength();
  ...
  if (i > 0) distLeft -= superNodes[i - 1];

  // last node, make supernode, if distance from prior >= idealDistance
  if (superNodes[i] == 0) {
    if (distLeft >= idealDistance) return true;
    return false;
  }
  ...
  distRight = superNodes[i] + nodeLen - position;
  ASSERT(distRight >= 0);

  // Distance between existing supernodes
  int d = (i > 0) ? superNodes[i] - superNodes[i - 1] + nodeLen
                  : superNodes[0] + nodeLen;

  if (d >= idealDistance * 3 / 2 && distLeft >= idealDistance / 2 &&
      distRight >= idealDistance / 2)
    return true;
...
Btn *IndexPage::findPriorNodeForSupernode(Btn *where, IndexKey *priorKey) {
  ...
  for (;; priorNode.getNext(where)) {
    ...
    if (priorNode.nextNode == where) return priorNode.node;
  }
...
void IndexPage::moveMemory(void *dst, void *src) {
  ...
  for (int i = 0; i < SUPERNODES && superNodes[i]; i++)
    if (nodes + superNodes[i] >= src) superNodes[i] += delta;
}
...
void IndexPage::addSupernode(Btn *where) {
  ...
  for (i = 0; i < SUPERNODES && superNodes[i]; i++) {
    ...
    if (superNodes[i] > offset) {
      if (i != SUPERNODES - 1)
        memmove(&superNodes[i + 1], &superNodes[i],
                (SUPERNODES - i - 1) * sizeof(superNodes[0]));
      break;
    }
  }
  ASSERT(i < SUPERNODES);
...
bool IndexPage::deleteSupernode(Btn *where) {
  ...
  for (int i = 0; i < SUPERNODES && superNodes[i]; i++) {
    if (superNodes[i] == offset) {
      if (i != SUPERNODES - 1)
        memmove(&superNodes[i], &superNodes[i + 1],
                (SUPERNODES - i - 1) * sizeof(superNodes[0]));
...
/* compare supernode to given key. Helper function, used for binary search in
 * findSupernode*/

static int compareSuper(IndexNode *node, UCHAR *key, size_t keylen,
                        int recordNumber, int level) {
  int result;

  int len = node->keyLength();
  if (level) len -= node->getAppendedRecordNumberLength();
  ASSERT(len >= 0);

  result = memcmp(node->key, key, MIN((int)keylen, len));

  if (result == 0) result = len - (int)keylen;
...
Btn *IndexPage::findSupernode(int level, UCHAR *key, size_t keylen,
                              int32 recordNumber, Btn *after, bool *found) {
  *found = false;
  if (superNodes[0] == 0) return nodes;
  ...
  if (low == SUPERNODES) return nodes + superNodes[SUPERNODES - 1];
  ...
  if (*found) return nodes + superNodes[low];

  if (low == 0) return nodes;
