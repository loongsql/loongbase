...
#define SUB_KEY NULL
#define PATH_KEY \
  "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Netfrastructure"
#else
#define KEY ""
#endif
...
const char *Registry::findDatabase(const char *name, int bufferSize,
                                   char *buffer) {
#ifdef _WIN32
  char keyString[256];
  ...
  char link[PATH_MAX];
  int n =
      readlink(genLinkName(name, link, sizeof(link)), buffer, bufferSize - 1);
...
void Registry::defineDatabase(const char *name, const char *fileName) {
  ...
  if (ret != ERROR_SUCCESS)
    throw SQLError(RUNTIME_ERROR, "RegCreateKey failed creating \"%s\"",
                   keyString);

  ret = RegSetValue(key, SUB_KEY, REG_SZ, expandedName,
                    (int)strlen(expandedName));

  if (ret != ERROR_SUCCESS)
    throw SQLError(RUNTIME_ERROR, "RegSetValue failed setting \"%s\"",
                   expandedName);
...
char *Registry::genLinkName(const char *name, char *buffer, int bufferSize) {
  ...
  for (q = installPath; *q;) *p++ = *q++;

  for (q = "/db."; *q;) *p++ = *q++;

  for (q = name; *q; ++q) *p++ = UPPER(*q);
...
const char *Registry::getKey() { return KEY; }
...
bool Registry::checkSitePassword(const char *password) {
  ...
  ***/
  ...
  DecodeTransform<FileTransform, Base64Transform> decode(
      getSitePasswordFilename());
