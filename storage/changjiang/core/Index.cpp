...
#define SEGMENT_BYTE(segment, count) \
  ((indexVersion >= INDEX_VERSION_1) ? count - segment : segment)
#define PAD_BYTE(field) \
  ((indexVersion >= INDEX_VERSION_1) ? field->indexPadByte : 0)

static const char *tables[] = {"Indexes", "IndexFields", NULL};
...
Index::Index(Table *tbl, const char *indexName, int indexType, int id,
             int numFields) {
  ASSERT(id != -1);
...
Index::~Index() {
  if (deferredIndexes.first) {
    ...
    for (DeferredIndex *deferredIndex = deferredIndexes.first; deferredIndex;
      ...
      //			deferredIndex->release();  not currently
      //refCounted;
    }
  }
  ...
  if (DIHashTable != NULL) delete[] DIHashTable;
...
void Index::save() {
  ...
  PreparedStatement *statement = database->prepareStatement(
      "insert Indexes "
      "(indexName,schema,tableName,indexType,fieldCount,indexId)values(?,?,?,?,"
      "?,?);");
  ...
  const char *sql =
      (database->fieldExtensions)
          ? "insert IndexFields (field,schema,indexName,position,tableName, "
            "partial)values(?,?,?,?,?,?)"
          : "insert IndexFields "
            "(field,schema,indexName,position,tableName)values(?,?,?,?,?)";
  ...
  for (int n = 0; n < numberFields; ++n) {
    ...
    if (database->fieldExtensions) statement->setInt(6, getPartialLength(n));
...
void Index::insert(IndexKey *key, int32 recordNumber,
  ...
  bool ret =
      database->addIndexEntry(indexId, &key, record->recordNumber, transaction);
#endif
}
...
DeferredIndex *Index::getDeferredIndex(Transaction *transaction) {
  ...
  // Use the shortest linked list. Index::deferredIndexes or
  // transaction->deferredIndexes

  if (deferredIndexes.count < transaction->deferredIndexCount) {
    Sync sync(&deferredIndexes.syncObject, "Index::getDeferredIndex(1)");
    sync.lock(Shared);
    for (deferredIndex = deferredIndexes.first; deferredIndex;
         deferredIndex = deferredIndex->next) {
      if ((deferredIndex->transaction == transaction) &&
          (deferredIndex->virtualOffset == 0))
        break;
    }
  } else {
    for (deferredIndex = transaction->deferredIndexes; deferredIndex;
  ...
  if ((deferredIndex) && (type != PrimaryKey) && (type != UniqueIndex) &&
      (transaction->scanIndexCount == 0)) {
    // Scavenge (or chill) this DeferredIndex and get a new one

    if (deferredIndex->count > 0 &&
        deferredIndex->sizeEstimate >
            database->configuration->indexChillThreshold) {
      if (deferredIndex->chill(dbb)) deferredIndex = NULL;
    }
  }

  if (deferredIndex) {
    ASSERT((deferredIndex->index == this) &&
           (deferredIndex->transaction == transaction));
...
void Index::makeKey(Field *field, Value *value, int segment, IndexKey *indexKey,
  ...
  switch (field->type) {
    ...
    case Char: {
      ...
      if (field->collation) {
        field->collation->makeKey(value, indexKey, partialLength, field->length,
                                  highKey);
      } else {
        ...
        if (partialLength && partialLength < l) l = partialLength;
  ...
  if (indexVersion < INDEX_VERSION_2) return;
...
void Index::makeMultiSegmentKey(int count, Value **values, IndexKey *indexKey,
                                bool highKey) {
  uint p = 0;
  int n;
  UCHAR *key = indexKey->key;
...
void Index::makeMultiSegmentKeyV1(int count, Value **values, IndexKey *indexKey,
                                  bool highKey) {
  uint p = 0;
  int n;
  UCHAR *key = indexKey->key;
  ...
  bool ODSVersion23OrOlder =
      (COMBINED_VERSION(database->dbb->odsVersion,
                        database->dbb->odsMinorVersion) <=
  ...
  for (n = 0; (n < count) && values[n]; ++n) {
    ...
    // - There is a segment byte inserted at the start of the segment and every
    // RUN bytes.
    // - All segments before the last one are padded to the nearest RUN length.

    uint segmentLength =
        (length + length / (RUN - 1) + ((length % (RUN - 1)) ? 1 : 0));
    if (n < numberFields - 1) segmentLength = ROUNDUP(segmentLength, RUN);

    checkIndexKeyOverflow(p + segmentLength,
                          maxIndexKeyRunLength(maxKeyLength));

    for (int i = 0; i < length; ++i) {
      if (p % RUN == 0) key[p++] = SEGMENT_BYTE(n, numberFields);
...
void Index::deleteIndex(Transaction *transaction) {
  if (!damaged && indexId != -1) {
    ...
    // this index, set indexId to -1 before writing to the Stream Log.
...
Bitmap *Index::scanIndex(IndexKey *lowKey, IndexKey *highKey, int searchFlags,
  ...
  if ((database->configuration->useDeferredIndexHash) && (lowKey) &&
      (lowKey == highKey) && INDEX_IS_UNIQUE(type) && (DIHashTableCounts)) {
    ...
    if (transaction) {
      for (DeferredIndex *deferredIndex = deferredIndexes.first; deferredIndex;
        ...
        if (transaction->visible(deferredIndex->transaction,
          ...
          if (transaction->database->dbb->debug &
              (DEBUG_KEYS & DEBUG_DEFERRED_INDEX))
            deferredIndex->print();
        }
      }
    } else
  ...
  if (partialLengths) searchFlags |= Partial;

  if (rootPage == 0) getRootPage();

  IndexRootPage::scanIndex(dbb, indexId, rootPage, lowKey, highKey, searchFlags,
                           NO_TRANSACTION, bitmap);

  if (transaction) transaction->scanIndexCount++;

  return bitmap;
}

IndexWalker *Index::positionIndex(IndexKey *lowKey, IndexKey *highKey,
  ...
  ASSERT(indexId != -1);
  WalkIndex *walkIndex =
      new WalkIndex(this, transaction, searchFlags, lowKey, highKey);
  ...
  if (transaction && deferredIndexes.first) {
    ...
    for (DeferredIndex *deferredIndex = deferredIndexes.first; deferredIndex;
      ...
      if (transaction->visible(deferredIndex->transaction,
        ...
        WalkDeferred *walkDeferred =
            new WalkDeferred(deferredIndex, transaction, searchFlags,
                             &walkIndex->lowerBound, &walkIndex->upperBound);
...
void Index::setIndex(int32 id) { indexId = id; }

void Index::loadFields() {
  memset(fields, 0, sizeof(Field *) * numberFields);
  const char *sql =
      (database->fieldExtensions)
          ? "select field,position,partial,records_per_value from IndexFields "
            "where indexName=? and schema=? and tableName=?"
          : "select field,position,0,0 from IndexFields where indexName=? and "
  ...
  if (INDEX_IS_UNIQUE(type)) recordsPerSegment[numberFields - 1] = 1;

  for (int n = 0; n < numberFields; ++n) {
    if (!fields[n])
      throw SQLError(RUNTIME_ERROR,
                     "Can't find field %d of index %s of table %s.%s\n", n,
                     (const char *)name, table->schemaName, table->name);

    if (type == PrimaryKey) fields[n]->setNotNull();
  }
}

void Index::update(Record *oldRecord, Record *record,
                   Transaction *transaction) {
  // Get key value
  ...
  if (duplicateKey(&key, oldRecord)) return;

  // Update index

  // bool ret = database->addIndexEntry (indexId, &key, record->recordNumber,
  // transaction);
  insert(&key, record->recordNumber, transaction);
}
...
void Index::garbageCollect(Record *leaving, Record *staying,
  ...
  for (Record *record = leaving; record && record != staying;
    ...
    if (record->hasRecord() && record->recordNumber >= 0) {
      ...
      // Delete the index entry for this record version if the key is not used
      // by other record versions

      if (!duplicateKey(
              &key,
              record->getPriorVersion())    // key not in 'leaving' record chain
          && !duplicateKey(&key, staying))  // key not in 'staying' record chain
      {
        ...
        if (deferredIndexes.first) {
          ...
          for (DeferredIndex *deferredIndex = deferredIndexes.first;
               deferredIndex; deferredIndex = deferredIndex->next)
...
bool Index::duplicateKey(IndexKey *key, Record *record) {
  for (Record *oldie = record; oldie; oldie = oldie->getPriorVersion())
    if (oldie->hasRecord()) {
      ...
      if (oldKey.isEqual(key)) return true;
    }

  return false;
}

JString Index::getTableName(Database *database, const char *schema,
  ...
  while (resultSet->next()) tableName = resultSet->getSymbol(1);
...
void Index::deleteIndex(Database *database, const char *schema,
                        const char *indexName) {
  PreparedStatement *statement = database->prepareStatement(
...
void Index::rebuildIndex(Transaction *transaction) {
  ...
  int oldId = indexId;
  indexId =
      dbb->createIndex(TRANSACTION_ID(transaction), INDEX_CURRENT_VERSION);

  getRootPage();

  PreparedStatement *statement = database->prepareStatement(
      "update system.indexes set indexId=? where indexName=? and schema=? and "
      "tableName=?");
  int n = 1;
  ...
  if (n != 1)
    throw SQLEXCEPTION(DDL_ERROR, "couldn't update system.indexs for %s",
                       (const char *)name);
...
void Index::rename(const char *newName) {
  for (const char **tableName = tables; *tableName; ++tableName) {
    ...
    snprintf(sql, sizeof(sql),
             "update system.%s set indexName=? where schema=? and indexName=?",
             *tableName);
    PreparedStatement *statement = database->prepareStatement(sql);
...
const char *Index::getIndexName() { return name; }

const char *Index::getSchemaName() { return table->schemaName; }

int Index::getPartialLength(int segment) {
  if (!partialLengths) return 0;

  return partialLengths[segment];
}

void Index::setPartialLength(int segment, uint partialLength) {
  if (partialLength > (uint)maxKeyLength) partialLength = maxKeyLength;
...
UCHAR Index::getPadByte(int index) { return fields[index]->indexPadByte; }

UCHAR Index::getPadByte(void) { return fields[numberFields - 1]->indexPadByte; }
...
void Index::checkMaxKeyLength(void) {
  ...
  for (int s = 0; s < numberFields; s++) {
    ...
    if ((fld->type == String) || (fld->type == Varchar) ||
        (fld->type == Char)) {
      len = getPartialLength(s);

      if (len == 0) len = fld->length;
    } else
      len = MAX(fld->length, 8);  // numbers can take up to 8 bytes encoded by
                                  // IndexKey::appendNumber().
  ...
  if (sumKeyLen > maxKeyLen) {
    ...
    throw SQLEXCEPTION(
        INDEX_OVERFLOW,
        "Maximum key length can be exceeded on index %s on table %s.%s",
        (const char *)name, table->schemaName, table->name);
  }
}
...
void Index::optimize(uint64 cardinality, Connection *connection) {
  PStatement update = database->prepareStatement(
      "update indexfields set records_per_value=? where schema=? and "
      "indexname=? and field=?");
  ...
  for (int n = 0; n < numberFields; ++n) {
    ...
    if ((n == numberFields - 1) && (type & (PrimaryKey | UniqueIndex)))
      ...
      sql.Format("select count(*) from (select distinct %s from %s.%s)",
                 (const char *)keys, (const char *)table->schemaName,
...
//*  static const int hashTableSizes[MAX_DI_HASH_TABLES] = {11, 101, 1009,
//10007, 100003};
...
void Index::addToDIHash(struct DIUniqueNode *uniqueNode) {
  ...
  if (DIHashTable == NULL) {
    ...
    //*		memset(DIHashTables[curHashTable], 0, sizeof(DIUniqueNode*) *
    //hashSize);
  ...
  DIHashTableCounts++;
  if (uniqueNode->collision == NULL) DIHashTableSlotsUsed++;
...
void Index::removeFromDIHash(struct DIUniqueNode *uniqueNode) {
  ...
  //*	for (int h = curHashTable; h >= 0; h--)
  {
    ...
    for (DIUniqueNode **uNode = &DIHashTable[slot]; uNode;
      ...
      if (*uNode == uniqueNode) {
        *uNode = uniqueNode->collision;
        ...
        DIHashTableCounts--;
        if (DIHashTable[slot] == NULL) DIHashTableSlotsUsed--;
...
void Index::scanDIHash(IndexKey *scanKey, int searchFlags, Bitmap *bitmap) {
  ...
  //*	for (int h = curHashTable; h >= 0; h--)
  {
    ...
    if (hashTable) {
      ...
      for (DIUniqueNode *uNode = hashTable[slot]; uNode;
           uNode = uNode->collision) {
        DINode *node = &uNode->node;
        if (scanKey->compareValues(node->key, node->keyLength, isPartial) ==
            0) {
