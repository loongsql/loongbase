...
void PageWriter::writePage(Dbb *dbb, int32 pageNumber, TransId transactionId) {
  ...
  // Get a dirty page block and link it into the linked list and page and
  // transaction hash tables
...
#ifdef _PTHREADS
  prctl(PR_SET_NAME, "cj_pagewriter");
#endif
...
void PageWriter::writer() {
  ...
  while (!thread->shutdownInProgress) {
    ...
    if (element) {
      ...
      if (bdb) {
        if (bdb->isDirty) cache->writePage(bdb, WRITE_TYPE_PAGE_WRITER);
...
void PageWriter::removeElement(DirtyPage *element) {
  ...
  if (!transaction->firstPage) release(transaction);
}
...
void PageWriter::waitForWrites(Transaction *transaction) {
  ...
  for (;;) {
    ...
    if (!dirtyTrans) return;

    if (!thread) thread = Thread::getThread("PageWriter::waitForWrites");
...
DirtyTrans *PageWriter::findDirtyTransaction(TransId transactionId) {
  ...
  for (DirtyTrans *trans = transactions[transactionId % dirtyHashSize]; trans;
       trans = trans->collision)
...
void PageWriter::release(DirtyTrans *transaction) {
  ...
  if (transaction->thread) transaction->thread->wake();
