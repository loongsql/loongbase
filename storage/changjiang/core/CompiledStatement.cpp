...
void CompiledStatement::addRef() { INTERLOCKED_INCREMENT(useCount); }
...
NNode *CompiledStatement::compile(Syntax *syntax) {
  ...
  switch (syntax->type) {
    ...
    case nod_number: {
      ...
      if (strlen(p) < 9) return new NConstant(this, atoi(syntax->getString()));
    }
...
Field *CompiledStatement::findField(Table *table, Syntax *syntax) {
  ...
  if (!field)
    throw SQLError(SYNTAX_ERROR, "can't find field %s in table %s", name,
                   table->getName());

  return field;
}

int CompiledStatement::getValueSlot() { return numberValues++; }
...
Context *CompiledStatement::compileContext(Syntax *syntax, int32 privMask) {
  ...
  if (alias) context->alias = alias->getString();
...
void *CompiledStatement::markContextStack() { return contextStack.mark(); }
...
NNode *CompiledStatement::compileField(Syntax *syntax) {
  ...
  switch (syntax->count) {
    case 1:
      ...
      if (field) return compileField(field, context);
      END_FOR;
      ...
      throw SQLEXCEPTION(SYNTAX_ERROR, "can't resolve field %s", name);
    ...
    case 2:
      ...
      if (strcmp (ctxName, "SQL") == 0 &&
              ...
              FOR_STACK (Context*, context, &contextStack)
                      if (context->table)
                              return new NConstant (this,
      context->table->schemaName); END_FOR
              }
      ***/
      // syntax->prettyPrint ("CompiledStatement.compileField");
      throw SQLEXCEPTION(SYNTAX_ERROR, "can't resolve field %s.%s", ctxName,
                         name);

    default:
      throw SQLEXCEPTION(SYNTAX_ERROR,
                         "compileField: too many field qualifiers");
  }

  return NULL;
}

Fsb *CompiledStatement::compileStream(Context *context, LinkedList &conjuncts,
                                      Row *row) {
  context->setComputable(true);
  LinkedList booleans;
  Fsb *stream = NULL;
  ...
  if (!booleans.isEmpty()) {
    ...
    switch (node->type) {
      ...
      case Or:
        if (isInvertible(context, node))
          inversion =
              mergeInversion(inversion, genInversion(context, node, &booleans));
      ...
      case Eql: {
        ...
        if (ref) inversion = mergeInversion(inversion, new NBitSet(this, ref));
      } break;
    ...
    if (invert) inversion = mergeInversion(inversion, invert);
    END_FOR;

    NNode *invert = genInversion(context, index, booleans);

    if (invert) inversions.append(invert);
    END_FOR;

    NNode *uniqueKey = NULL;

    FOR_OBJECTS(NNode *, invert, &inversions)
    if (invert->isUniqueIndex()) uniqueKey = invert;
    END_FOR;
    ...
    if (inversion) stream = new FsbInversion(context, inversion, row);

    context->setComputable(true);
  }

  if (!stream) stream = new FsbExhaustive(context, row);

  int count = booleans.count();

  if (count > 0) {
    NNode *boolean = nullptr;

    if (count > 1) boolean = new NNode(this, And, count);
...
bool CompiledStatement::contextComputable(int contextId) {
  FOR_STACK(Context *, context, &contextStack)
  if (context->contextId == contextId) return context->computable;
  END_FOR;

  return false;
}

NNode *CompiledStatement::genInversion(Context *context, Index *index,
                                       LinkedList &booleans) {
  ...
  FOR_OBJECTS(NNode *, node, &booleans)
  if (node->type != Starting) node->matchIndex(this, context, index, min, max);
  END_FOR;

  NBitmap *bitmap = NULL;

  if (min[0] || max[0]) bitmap = new NBitmap(this, index, count, min, max);
...
int CompiledStatement::getGeneralSlot() { return numberSlots++; }

int CompiledStatement::getSortSlot() { return numberSorts++; }

int CompiledStatement::getBitmapSlot() { return numberBitmaps++; }
...
Table *CompiledStatement::getTable(Connection *connection, Syntax *syntax) {
  ...
  if (syntax->count == 1) {
    ...
    throw SQLEXCEPTION(NO_SUCH_TABLE, "can't find table \"%s\"",
                       name->getString());
  }
...
Table *CompiledStatement::getTable(Syntax *syntax) {
  ...
  if (table && syntax->count == 1) noteUnqualifiedTable(table);
...
Table *CompiledStatement::findTable(Syntax *syntax, bool search) {
  ...
  if (syntax->count == 1) {
    ...
    if (table) noteUnqualifiedTable(table);
...
bool CompiledStatement::validate(Connection *connection) {
  ...
  FOR_OBJECTS(Context *, context, &contexts)
  if (context->select) FOR_OBJECTS(FilterSet *, filterSet, activeFilters)
  TableFilter *filter = filterSet->findFilter(context->table);

  if (filter) statementFilters.insertOrdered(filter);
  END_FOR;
  END_FOR;

  if (!statementFilters.equal(&filters)) return false;
...
NNode *CompiledStatement::compileField(Field *field, Context *context) {
  if (!context->viewContexts) return new NField(this, field, context);
...
bool CompiledStatement::isInvertible(Context *context, NNode *node) {
  ...
  if (node->isInvertible(this, context)) return true;
...
NNode *CompiledStatement::genInversion(Context *context, NNode *node,
                                       LinkedList *booleans) {
  // If this is a direct reference to a record number, we're invertible

  NNode *ref = isDirectReference(context, node);

  if (ref) return new NBitSet(this, ref);

  // Matching is always invertable

  if (node->type == Matching) return node;
  ...
  if (min[0] || max[0]) {
    if (booleans) FOR_OBJECTS(NNode *, expr, booleans)
...
Sequence *CompiledStatement::findSequence(Syntax *syntax, bool search) {
  ...
  if (syntax->count == 1) {
    ...
    if (sequence) {
      ...
      if (seq == sequence) return sequence;
      END_FOR;
...
void CompiledStatement::decomposeAdjuncts(Syntax *syntax,
                                          LinkedList &adjuncts) {
  if (syntax->type == nod_or) FOR_SYNTAX(child, syntax)
  decomposeAdjuncts(child, adjuncts);
  END_FOR
  else adjuncts.append(syntax);
}
...
int CompiledStatement::getValueSetSlot() { return numberValueSets++; }

int CompiledStatement::getRowSlot() { return numberRowSlots++; }
...
NNode *CompiledStatement::compileFunction(Syntax *syntax) {
  ...
  if (strcasecmp(name, "BlobRef") == 0 &&
      (qualifier == NULL || strcasecmp(qualifier, "SYSTEM") == 0)) {
    reqArgs = 1;
    ...
    throw SQLError(SYNTAX_ERROR, "function \"%s\" is undefined",
                   (const char *)getNameString(qualifiedName));
...
JString CompiledStatement::getNameString(Syntax *syntax) {
  ...
  for (int n = 0; n < syntax->count; ++n) {
    ...
    for (const char *q = syntax->getChild(n)->getString(); *q;) *p++ = *q++;
  }
