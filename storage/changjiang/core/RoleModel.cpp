...
static const char *ddl[] = {
    ...
    "upgrade index object_privileges on system.privileges(objectType, "
    "objectSchema, objectName)",
...
RoleModel::RoleModel(Database *db) {
  ...
  systemUser =
      new User(database, database->getSymbol("SYSTEM"), nullSchema, NULL, true);
  publicUser = new User(database, database->getSymbol("PUBLIC"), nullSchema,
                        NULL, false);
...
void RoleModel::createTables() {
  ...
  if (!table || !table->findIndex("OBJECT_PRIVILEGES")) {
    ...
    for (const char **ptr = ddl; *ptr; ++ptr) try {
...
User *RoleModel::createUser(const char *userName, const char *password,
                            bool encrypted, Coterie *coterie) {
  ASSERT(database->isSymbol(userName));

  if (!tablesCreated) createTables();

  JString encryptedPassword =
      (encrypted) ? (JString)password : User::encryptPassword(password);
...
#ifndef STORAGE_ENGINE
  if (coterie) statement->setString(3, coterie->name);
#endif
...
User *RoleModel::findUser(const char *name) {
  ...
  if (publicUser->name == name) return publicUser;
  ...
  if (!strcmp (userName, "PANIC"))
          {
          user = new User (database, userName, User::encryptPassword("panic"),
  NULL, true); insertUser (user); return user;
          }
  ***/
...
#ifndef STORAGE_ENGINE
  if (coterieName[0]) coterie = findCoterie(database->getSymbol(coterieName));
#endif
  ...
  statement = database->prepareStatement(
      "select roleSchema,roleName,options,defaultRole from system.userRoles "
      "where userName=?");
  statement->setString(1, userName);
  ...
  while (resultSet->next()) {
    ...
    if (role) user->addRole(role, defaultRole, options);
  }
...
Role *RoleModel::findRole(const char *schema, const char *name) {
  ...
  for (role = roles[slot]; role; role = role->collision)
    if (role->name == name && role->schemaName == schema) return role;
...
void RoleModel::addUserPrivilege(User *user, PrivilegeObject *object,
                                 int32 mask) {
  if (!tablesCreated) createTables();

  // The following is necessay to avoid assessing SYSTEM.PRIVILEGES while it is
  // being created
  ...
  if (updatePrivilege(user, object, mask)) return;

  insertPrivilege(user, object, mask);
}

void RoleModel::addUserRole(User *user, Role *role, bool defaultRole,
                            int options) {
  PreparedStatement *statement = database->prepareStatement(
...
User *RoleModel::getUser(const char *userName) {
  ...
  if (!user) throw SQLEXCEPTION(DDL_ERROR, "user %s does not exist", userName);

  return user;
}

bool RoleModel::updatePrivilege(Role *role, PrivilegeObject *object,
                                int32 mask) {
  PreparedStatement *statement = database->prepareStatement(
...
void RoleModel::changePassword(User *user, const char *password, bool encrypted,
                               Coterie *coterie) {
  ...
  if (count < 1) throw SQLEXCEPTION(DDL_ERROR, "couldn't change password");
...
void RoleModel::revokeUserRole(User *user, Role *role) {
  PreparedStatement *statement = database->prepareStatement(
      "delete from system.userRoles where userName=? and roleSchema=? and "
      "roleName=?");
  int n = 1;
  statement->setString(n++, user->name);
  statement->setString(n++, role->schemaName);
...
void RoleModel::insertPrivilege(Role *role, PrivilegeObject *object,
                                int32 mask) {
  PreparedStatement *statement = database->prepareStatement(
      "insert into system.privileges"
      "(holderType, "
      "holderSchema,holderName,objectType,objectSchema,objectName,"
      "privilegeMask)"
      "values (?,?,?,?,?,?,?)");
  int n = 1;
  statement->setInt(n++, role->getPrivilegeType());
...
User *RoleModel::getSystemUser() { return systemUser; }

void RoleModel::removePrivilege(Role *role, PrivilegeObject *object,
                                int32 mask) {
  Privilege *privilege = role->getPrivilege(object);
  privilege->privilegeMask &= ~mask;
...
void RoleModel::dropObject(PrivilegeObject *object) {
  ...
  PreparedStatement *statement = database->prepareStatement(
      "delete from system.privileges where objectType=? and objectSchema=? and "
      "objectName=?");
  int n = 1;
...
void RoleModel::initialize() { createTables(); }
...
Coterie *RoleModel::findCoterie(const char *name) {
  ...
  for (coterie = coteries; coterie; coterie = coterie->next)
    if (coterie->name == name) return coterie;
...
void RoleModel::updateCoterie(Coterie *coterie) {
  ...
  PreparedStatement *statement =
      database->prepareStatement("delete from system.coteries where coterie=?");
  ...
  for (CoterieRange *range = coteries->ranges; range;
       range = range->next, ++sequence) {
...
void RoleModel::dropCoterie(Coterie *coterie) {
  ...
  if (resultSet->next()) {
    ...
    throw SQLEXCEPTION(DDL_ERROR, "coterie %s is in use for user %s",
                       (const char *)coterie->name, (const char *)user);
  }
...
void RoleModel::dropUser(User *user) {
  ...
  PreparedStatement *statement =
      database->prepareStatement("delete from system.users where username=?");
...
void RoleModel::deletePrivileges(Role *role) {
  PreparedStatement *statement = database->prepareStatement(
      "delete from system.privileges where holderschema=? and holdername=? and "
      "holdertype=?");
  statement->setString(1, role->schemaName);
  statement->setString(2, role->name);
  ...
  statement = database->prepareStatement(
      "delete from system.privileges where objectschema=? and objectname=? and "
      "objecttype=?");
  statement->setString(1, role->schemaName);
  statement->setString(2, role->name);
  statement->setInt(3, role->getPrivilegeType());
...
void RoleModel::renameTable(Table *table, const char *newSchema,
                            const char *newName) {
  PreparedStatement *statement = database->prepareStatement(
