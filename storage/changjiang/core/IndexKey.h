...
// The current index design requires a minimum of three key entries on each
// page. MAX_INDEX_KEY_LENGTH = (((pageSize - keyPageOverhead(52)) /
// minKeysPerPage(3)) - keyOverhead(17)) * (RUN - 1) / RUN;
...
// static const uint MAX_PHYSICAL_KEY_LENGTH      = (MAX_INDEX_KEY_LENGTH_32K *
// RUN / (RUN - 1)) + sizeof(int64) + 1;
static const uint MAX_PHYSICAL_KEY_LENGTH =
    (((MAX_INDEX_KEY_LENGTH_32K * RUN / (RUN - 1)) + sizeof(int64) + 1) + 1) /
    2 * 2;
...
class IndexKey {
 public:
  ...
  inline unsigned char *keyEnd() { return key + keyLength; };
