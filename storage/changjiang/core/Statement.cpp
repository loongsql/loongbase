...
struct SyntaxPrivType {
  SyntaxType type;
  int32 privilege;
} privilegeTypes[] = {nod_priv_select,
                      PRIV_MASK(PrivSelect),
                      nod_priv_insert,
                      ...
                      PRIV_MASK(PrivAlter),
                      nod_priv_all,
                      PRIV_MASK(PrivSelect) | PRIV_MASK(PrivInsert) |
                          PRIV_MASK(PrivUpdate) | PRIV_MASK(PrivDelete) |
                          PRIV_MASK(PrivExecute) |
                          // PRIV_MASK (PrivGrant)|
                          PRIV_MASK(PrivAlter),
                      nod_not,
...
const static char *privObjectTypes[] = {
    "Table", "View", "Procedure", "User", "Role", "Coterie",
};
...
void Statement::createTable(Syntax *syntax) {
  ...
  if (tableSpaceName)
    tableSpace =
        database->tableSpaceManager->getTableSpace(tableSpaceName->getString());
  ...
  try {
    ...
    ForeignKey *key;
    if ((key = field->foreignKey)) key->create();
    END_FOR;
  ...
  if (!database->formatting) {
    ...
    table->create(
        (connection == database->systemConnection) ? "SYSTEM TABLE" : "TABLE",
        sysTransaction);
    table->save();
    database->roleModel->addUserPrivilege(connection->user, table,
                                          ALL_PRIVILEGES);
...
void Statement::createIndex(Syntax *syntax, bool upgrade) {
  ...
  if (syntax->getChild(0)) indexType = UniqueIndex;

  const char *name = syntax->getChild(1)->getString();
  Table *table = statement->getTable(syntax->getChild(2));

  // Make sure index doesn't exist on a different table or this table is
  // creation
  ...
  if (!database->formatting) {
    ...
    if (resultSet->next()) {
      ...
      if (oldIndex && oldIndex->numberFields == list->count &&
          oldIndex->type == indexType) {
        bool identical = true;
        ...
        for (int n = 0; n < oldIndex->numberFields; ++n) {
          ...
          if (oldIndex->fields[n] != field ||
              partialLength != oldIndex->getPartialLength(n)) {
            identical = false;
            break;
          }
        }
  ...
  if (partial) index->setPartialLength(n, partial->getNumber());
...
void Statement::allocParameters(int count) { parameters.alloc(count); }
...
void Statement::prepareStatement() {
  ...
  ***/
  ...
  if (statement->numberValues) values.alloc(statement->numberValues);
  ...
  if ((numberSorts = statement->numberSorts)) {
    if (!sorts) sorts = new Sort *[numberSorts];

    memset(sorts, 0, sizeof(Sort *) * numberSorts);
  }

  for (n = 0; n < numberBitmaps; ++n)
    if (!bitmaps[n]) bitmaps[n]->release();
...
ResultSet *Statement::executeQuery() { return NULL; }
...
void Statement::start(NNode *node) {
  ...
  // System transactions should hold syncSysDDL, and update verbs should hold it
  // exclusively
  ...
  ***/
  ...
  if (transaction) transaction->releaseSavepoint(savePoint);
}
...
void Statement::close() {
  ...
  if (resultSets) clearResults(true);
...
int Statement::getParameterCount() { return statement->numberParameters; }
...
void Statement::executeDDL() {
  ...
  switch (syntax->type) {
    ...
    case nod_set_namespace:
      while (!connection->nameSpace.isEmpty()) connection->popSchemaName();
...
Field *Statement::addField(Table *table, Syntax *syntax) {
  ...
  if (table->findField(fldName))
    throw SQLEXCEPTION(DDL_ERROR, "field %s already exists in table %s.%s",
                       fldName, (const char *)table->schemaName,
  ...
  Syntax *clauses = syntax->getChild(2);
  Field *field = table->addField(fldName, type.type, type.length,
                                 type.precision, type.scale, flags);

  if (clauses) FOR_SYNTAX(clause, clauses)
  switch (clause->type) {
    case nod_searchable:
    ...
    case nod_repository: {
      if (field->type != Asciiblob && field->type != Binaryblob)
        throw SQLEXCEPTION(DDL_ERROR,
                           "can't specify repository on non-blob field %s\n",
                           field->name);

      const char *repositoryName = clause->getChild(0)->getString();
      Repository *repository =
          database->getRepository(table->schemaName, repositoryName);
...
ForeignKey *Statement::addForeignKey(Table *foreignTable, Field *column,
                                     Syntax *syntax) {
  ...
  if (!primaryIndex)
    throw SQLEXCEPTION(DDL_ERROR, "no primary index for table %s",
  ...
  if (node) FOR_SYNTAX(option, node)
  switch (option->type) {
...
Context *Statement::getUpdateContext() {
  if (statement->node->type != Select) return NULL;

  NSelect *select = (NSelect *)statement->node;

  if (select->numberContexts != 1) return NULL;
...
void Statement::dropView(Syntax *syntax) {
  ...
  if (!table->view)
    throw SQLError(DDL_ERROR, "%s.%s is not a view", table->schemaName,
                   table->name);
...
void Statement::addRef() { INTERLOCKED_INCREMENT(useCount); }

void Statement::release() {
  ASSERT(useCount > 0);
  ASSERT(useCount > javaCount);

  if (INTERLOCKED_DECREMENT(useCount) == 0) close();
}
  ...
  switch (item->type) {
    ...
    case nod_primary_key: {
      ...
      if (index) {
        ...
        if (index->numberFields != fields->count) keyChangeError(table);
...
void Statement::reset() {
  ...
  if (bitmaps) {
    for (int n = 0; n < numberBitmaps; ++n)
      if (bitmaps[0]) bitmaps[n]->release();
  ...
  if (valueSets) {
    for (int n = 0; n < numberValueSets; ++n)
      if (valueSets[n]) delete valueSets[n];
...
void Statement::getFieldType(Syntax *node, FieldType *fieldType) {
  ...
  switch (node->type) {
    ...
    case nod_numeric: {
      ...
      // throw SQLError (DDL_ERROR, "decimal precision great than 19 digits not
      // yet supported");
      {
        type = Biginteger;
  ...
  if (precision == 0) precision = Field::getPrecision(type, length);
...
bool Statement::upgradeField(Table *table, Field *field, Syntax *item,
                             LinkedList &newIndexes) {
  bool changed = false;
  field->foreignKey = NULL;
  FieldType type;
  ...
  if (field->type != type.type || field->length != type.length ||
    ...
    case nod_primary_key: {
      ...
      if (index) {
        if (index->numberFields != 1 || index->fields[0] != field)
          // throw SQLEXCEPTION (DDL_ERROR, "can't change primary key with
          // UPGRADE");
          keyChangeError(table);
      } else {
        ...
        Index *index =
            table->addIndex(table->getPrimaryKeyName(), 1, PrimaryKey);
...
Index *Statement::createPrimaryKey(Table *table, Syntax *item) {
  ...
  Syntax *fields = item->getChild(0);
  Index *index =
      table->addIndex(table->getPrimaryKeyName(), fields->count, PrimaryKey);
  int n = 0;
...
void Statement::setTableFilter(const char *tableName) {
  ...
  if (!tableFilter) tableFilter = new Bitmap();
...
void Statement::setTableFilter(const char *schemaName, const char *tableName) {
  ...
  if (!table)
    throw SQLEXCEPTION(RUNTIME_ERROR, "can't find table %s.%s", schemaName,
                       tableName);
...
void Statement::setTableFilter(const WCString *schemaName,
                               const WCString *tableName) {
  setTableFilter(database->getSymbol(schemaName),
                 database->getSymbol(tableName));
}

bool Statement::getMoreResults() {
  if (currentResultSet < 0) return false;
...
void Statement::createUser(Syntax *syntax) {
  ...
  if (node) coterie = database->roleModel->getCoterie(node->getString());

  if (database->roleModel->findUser(name))
    throw SQLEXCEPTION(DDL_ERROR, "user %s already exists", name);

  User *user = database->roleModel->createUser(name, password,
                                               encrypted != NULL, coterie);
...
void Statement::grantPriv(Syntax *syntax) {
  ...
  switch (obj->type) {
    ...
    case nod_role:
      object = roleModel->getRole(getSchemaName(obj->getChild(0)),
                                  getName(obj->getChild(0)));
      break;
  ...
  if (syntax->getChild(3)) mask |= mask << GRANT_SHIFT;
...
void Statement::grantRole(Syntax *syntax) {
  ...
  if (!connection->checkAccess(GRANT_OPTION, role)) {
    ...
    throw SQLEXCEPTION(
        SECURITY_ERROR, "user %s does not have Grant authority to %s.%s",
        (const char *)connection->user->name, (const char *)role->schemaName,
  ...
  if (syntax->getChild(2)) options = 1;

  if (syntax->getChild(3)) defaultRole = true;

  database->roleModel->addUserRole(user, role, defaultRole, options);
}

const char *Statement::getSchemaName(Syntax *syntax) {
  if (syntax->count == 1) return connection->currentSchemaName();
...
void Statement::connectionClosed() {
  ...
  ***/
  ...
  if (javaCount == 0) {
    ...
    for (int n = useCount; n > 0; --n) release();
  }
}
...
void Statement::addJavaRef() {
  ...
  if (special) printf("%p addJavaRef to %d\n", this, javaCount);
}
...
void Statement::releaseJava() {
  ...
  ***/
  ...
  if (javaCount == 1)
    if (!connection)
      while (useCount > javaCount) release();
...
void Statement::alterUser(Syntax *syntax) {
  ...
  if (!user) throw SQLEXCEPTION(DDL_ERROR, "user %s doesn't exist", name);

  if (!connection->checkAccess(PrivAlter, user))
    throw SQLEXCEPTION(SECURITY_ERROR,
                       "user %s does not have alter authority to user %s",
  ...
  if ((child = syntax->getChild(1))) password = child->getString();

  database->roleModel->changePassword(user, password, encrypted != NULL,
                                      coterie);
}
...
void Statement::revokeRole(Syntax *syntax) {
  ...
  if (!(user->hasRole(role) & HAS_ROLE))
    throw SQLEXCEPTION(DDL_ERROR, "user %s doesn't have role %s.%s", userName,
                       schema, name);
...
void Statement::createView(Syntax *syntax, bool upgrade) {
  ...
  if (table) {
    if (!upgrade)
      throw SQLEXCEPTION(DDL_ERROR, "table %s.%s is already defined", schema,
                         name);
...
void Statement::createSequence(Syntax *syntax, bool upgrade) {
  ...
  if (sequence) {
    if (!upgrade)
      throw SQLEXCEPTION(DDL_ERROR, "sequence %s.%s already exists", schema,
                         name);

    int64 n = sequence->updatePhysical(0, transaction);

    if (base > n) sequence->updatePhysical(base - n, transaction);
...
void Statement::createTrigger(Syntax *syntax, bool update) {
  ...
  ChjTrigger *trigger = table->findTrigger(name);

  if (trigger && !update)
    throw SQLEXCEPTION(
        DDL_ERROR, "trigger \"%s\" already defined for table \"%s.%s\"", name,
  ...
  switch (child->type) {
    case nod_pre_insert:
      mask |= PreInsert;
      break;
    case nod_post_insert:
      mask |= PostInsert;
      break;
    case nod_pre_update:
      mask |= PreUpdate;
      break;
    case nod_post_update:
      mask |= PostUpdate;
      break;
    case nod_pre_delete:
      mask |= PreDelete;
      break;
    case nod_post_delete:
      mask |= PostDelete;
      break;
    case nod_post_commit:
      mask |= PostCommit;
      break;
    case nod_active:
      active = true;
  ...
  if (!dot) throw SQLEXCEPTION(DDL_ERROR, "invalid trigger method name");

  JString className(method, dot - method);
  JString methodName(dot + 1);

  if (trigger) {
    if (!changed && trigger->table == table &&
        trigger->className == className && trigger->methodName == methodName &&
        trigger->mask == mask && trigger->position == position &&
        trigger->active == active)
      return;

    table->dropTrigger(trigger);
    ChjTrigger::deleteTrigger(database, table->schemaName, name);
  }

  trigger = new ChjTrigger(name, table, mask, position, active, className,
                           methodName);
...
void Statement::dropTrigger(Syntax *syntax) {
  ...
  const char *name = getName(node);
  const char *schema = getSchemaName(node);
  JString tableName = ChjTrigger::getTableName(database, schema, name);
  ...
  if (trigger) table->dropTrigger(trigger);

  ChjTrigger::deleteTrigger(database, schema, name);
  database->commitSystemTransaction();
#endif
}
...
void Statement::dropIndex(Syntax *syntax) {
  ...
  if (!table)
    throw SQLEXCEPTION(DDL_ERROR, "table %s.%s not defined",
                       (const char *)tableName, schema);
...
void Statement::alterTrigger(Syntax *syntax) {
  ...
  if (!trigger)
    throw SQLEXCEPTION(
        DDL_ERROR, "trigger \"%s\" is not defined for table \"%s.%s\"", name,
        (const char *)table->schemaName, (const char *)table->name);
...
void Statement::createFilterSet(Syntax *syntax, bool upgrade) {
  ...
  if (filterSet && !upgrade)
    throw SQLEXCEPTION(DDL_ERROR, "filterset %s.%s already exists", schema,
                       name);
...
void Statement::createCoterie(Syntax *syntax, bool upgrade) {
  ...
  if (coterie && !upgrade) {
    ...
    if (!connection->checkAccess(PrivAlter, coterie))
      throw SQLEXCEPTION(SECURITY_ERROR,
                         "coterie %s does not have alter authority to user %s",
                         (const char *)connection->user->name, name);
  }
  ...
  if (!coterie) {
    ...
    database->roleModel->addUserPrivilege(connection->user, coterie,
                                          ALL_PRIVILEGES);
    database->commitSystemTransaction();
  }
...
char *Statement::getString(Syntax *node, char *buffer, int length) {
  ...
  switch (node->type) {
    ...
    case nod_ip_address:
    case nod_name: {
      for (const char *q = node->getString(); *q && p < end;) *p++ = *q++;
...
void Statement::clearConnection() {
  ...
  if (connection) connection->deleteStatement(this);
...
void Statement::reIndex(Syntax *syntax) {
  ...
  if (!index) throw SQLEXCEPTION(DDL_ERROR, "can't find index \"%s\"", name);

  table->rebuildIndex(index, database->getSystemTransaction());
  database->commitSystemTransaction();
}

bool Statement::isPreparedResultSet() { return false; }
...
void Statement::dropUser(Syntax *syntax) {
  ...
  if (!connection->checkAccess(PrivDelete, user))
    throw SQLEXCEPTION(SECURITY_ERROR,
                       "user %s does not have drop authority to user %s",
                       (const char *)connection->user->name, name);
...
bool Statement::execute(const char *sqlString, bool isQuery) {
  ...
  if (!statement->node) return false;
...
void Statement::invalidate() {
  ...
  for (int n = 0; n < numberContexts; ++n) contexts[n].close();
}

void Statement::keyChangeError(Table *table) {
  throw SQLEXCEPTION(DDL_ERROR,
                     "can't change primary key for %s.%s with UPGRADE statment",
...
void Statement::checkAlterPriv(PrivilegeObject *object) {
  if (!connection->checkAccess(PrivAlter, object))
    throw SQLEXCEPTION(SECURITY_ERROR,
                       "user %s does not have alter authority to %s %s.%s",
...
void Statement::createRepository(Syntax *syntax, bool upgrade) {
  ...
  if ((node = syntax->getChild(1))) sequenceName = node->getString();

  const char *fileName = NULL;
  const char *rollovers = NULL;
  Sequence *sequence = NULL;

  if ((node = syntax->getChild(2))) fileName = node->getString();

  if ((node = syntax->getChild(3))) volume = node->getNumber();
  ...
  if (!upgrade && repository)
    throw SQLEXCEPTION(DDL_ERROR, "repository %s.%s already exists", schema,
                       name);
  ...
  if (!upgrade && !sequence)
    if (sequenceName)
      throw SQLEXCEPTION(DDL_ERROR, "sequence %s.%s isn't defined", schema,
                         sequenceName);
    else
      throw SQLEXCEPTION(
          DDL_ERROR, "a sequence name is required for repository %d", schema);

  if (repository) {
    if (sequence) repository->setSequence(sequence);

    if (fileName) repository->setFilePattern(fileName);

    if (volume) repository->setVolume(volume);

    if (rollovers) repository->setRollover(rollovers);

    repository->save();
  } else
    repository = database->createRepository(name, schema, sequence, fileName,
                                            volume, rollovers);
...
void Statement::upgradeSchema(Syntax *syntax, bool upgrade) {
  ...
  if ((node = syntax->getChild(1))) schema->setInterval(node->getNumber());

  if ((node = syntax->getChild(2))) schema->setSystemId(node->getNumber());
}
...
void Statement::dropRepository(Syntax *syntax) {
  ...
  PreparedStatement *statement = database->prepareStatement(
      "select field,tableName from system.fields where schema=? and "
      "repositoryName=?");
  ...
  if (resultSet->next()) {
    ...
    throw SQLError(DDL_ERROR, "repository %s is in use in field %s.%s\n", name,
                   (const char *)fieldName, (const char *)tableName);
  }
...
ResultList *Statement::findResultList(int32 handle) {
  for (ResultList *resultList = resultLists; resultList;
       resultList = resultList->sibling)
    if (resultList->handle == handle) return resultList;

  return NULL;
}

ValueSet *Statement::getValueSet(int slot) { return valueSets[slot]; }
...
void Statement::renameTables(Syntax *syntax) {
  for (int n = 0; n < syntax->count; ++n) {
    ...
    if (statement->findTable(to, false))
      throw SQLError(DDL_ERROR, "rename table target \"%s.%s\" already exists",
                     name, schema);
...
void Statement::createTableSpace(Syntax *syntax) {
  ...
  if (tableSpace) {
    if (syntax->type == nod_create_tablespace)
      throw SQLError(TABLESPACE_EXIST_ERROR, "tablespace \"%s\" already exists",
                     name);

    if (syntax->type == nod_upgrade_tablespace) {
      if (!tableSpace->fileNameEqual(fileName))
        throw SQLError(DDL_ERROR,
                       "filename change not supported for tablespaces");
...
void Statement::dropTableSpace(Syntax *syntax) {
  ...
  if (!tableSpace)
    throw SQLError(TABLESPACE_NOT_EXIST_ERROR,
                   "table space \"%s\" is not defined", name);
  ...
  if (resultSet->next())
    throw SQLError(TABLESPACE_NOT_EMPTY, "table space \"%s\" is not empty",
                   name);
