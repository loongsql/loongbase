...
bool Index2RootPage::addIndexEntry(Dbb *dbb, int32 indexId, IndexKey *key,
...
Bdb *Index2RootPage::findLeaf(Dbb *dbb, int32 indexId, int32 rootPage,
...
Bdb *Index2RootPage::findInsertionLeaf(Dbb *dbb, int32 indexId,
  ...
  if (page->level == 0) {
    ...
    if (!(bdb = findRoot(dbb, indexId, 0, Exclusive, transId))) return NULL;
...
Bdb *Index2RootPage::findRoot(Dbb *dbb, int32 indexId, int32 rootPage,
...
void Index2RootPage::scanIndex(Dbb *dbb, int32 indexId, int32 rootPage,
  ...
  if (highKey && node.node < end) {
    ...
    node.expandKey(&key);
    offset = page->computePrefix(node.offset + node.length, key.key,
                                 highKey->keyLength, highKey->key);
  }
...
bool Index2RootPage::splitIndexPage(Dbb *dbb, int32 indexId, Bdb *bdb,
  ...
  if (!page->parentPage) {
    ...
    Bdb *parentBdb = Index2Page::createNewLevel (dbb,
                    splitPage->level + 1, page->version, bdb->pageNumber,
    splitBdb->pageNumber, &splitKey, transId);
    ***/
  ...
  for (;;) {
    ...
    if (splitIndexPage(dbb, indexId, bdb, transId, result, &splitKey,
                       splitPageNumber))
      return true;
  }
}

bool Index2RootPage::deleteIndexEntry(Dbb *dbb, int32 indexId, IndexKey *key,
                                      int32 recordNumber, TransId transId) {
  Index2Page *page;
  IndexKey searchKey(key);
...
void Index2RootPage::debugBucket(Dbb *dbb, int indexId, int recordNumber,
...
void Index2RootPage::redoIndexPage(Dbb *dbb, int32 pageNumber, int32 parentPage,
                                   int level, int32 priorPage, int32 nextPage,
  ...
  if (!dbb->trialRead(bdb) || indexPage->pageType != PAGE_btree ||
      indexPage->parentPage != parentPage || indexPage->level == level) {
...
void Index2RootPage::setIndexRoot(Dbb *dbb, int indexId, int32 pageNumber,
                                  TransId transId) {
  int sequence = indexId / dbb->pagesPerSection;
  int slot = indexId % dbb->pagesPerSection;
  Bdb *bdb =
      Section::getSectionPage(dbb, INDEX_ROOT, sequence, Exclusive, transId);
  BDB_HISTORY(bdb);
  bdb->mark(transId);
  SectionPage *sections = (SectionPage *)bdb->buffer;
  ...
  if (!dbb->streamLog->recovering && !dbb->noLog)
    dbb->streamLog->logControl->sectionPage.append(
        dbb, transId, bdb->pageNumber, pageNumber, slot, INDEX_ROOT, sequence,
        0);
...
void Index2RootPage::indexMerge(Dbb *dbb, int indexId,
    ...
    for (;;) {
      if (number == recordNumber && nextKey.compare(&key) == 0)
        ...
        if ((UCHAR *)node.node == (UCHAR *)page + page->length)
          ...
          node.expandKey(&nextKey);
          offset2 = Index2Page::computePrefix(
              key.keyLength, key.key, node.offset + node.length, nextKey.key);
      ...
      node.parseNode(Index2Page::findInsertionPoint(
          0, &key, recordNumber, &priorKey, node.node, bucketEnd));
      nextKey.setKey(0, node.offset, priorKey.key);
      node.expandKey(&nextKey);
      number = node.getNumber();
...
void Index2RootPage::analyzeIndex(Dbb *dbb, int indexId,
