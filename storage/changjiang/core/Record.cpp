  ...
  if (*p > 0) {
    ...
    if (stream->decompressedLength != format->length)
      throw SQLEXCEPTION(RUNTIME_ERROR,
                         "wrong length record (got %d, expected %d), table "
                         "%s.%s, section %d, record %d",
                         stream->decompressedLength, format->length,
                         table->getSchema(), table->getName(),
                         table->dataSection, recordNumber);
  } else {
...
Record::Record(Table *tbl, Format *fmt) {
  ...
  if (!(format = fmt)) format = tbl->getCurrentFormat();
  ...
  /*
          ...
          Recordversion is created, the record buffer is allocated in
     setEncodedRecord() setAgeGroup() will be called later when data.record is
     allocated.
  */
...
void Record::setValue(TransactionState *transaction, int id, Value *value,
                      bool cloneFlag, bool copyFlag) {
  ASSERT(id < format->maxId);

  switch (encoding) {
    case noEncoding:
      if (data.record != NULL) DELETE_RECORD(data.record);

      data.record = (char *)NEW Value[format->count];
      encoding = valueVector;
    case valueVector:
      ((Value *)data.record)[format->format[id].physicalId].setValue(value,
                                                                     copyFlag);
      return;
  ...
  switch (ff->type) {
    ...
    case Char: {
      ...
      if (length < ff->length) memset(ptr + length, 0, ff->length - length);
    } break;
    ...
    case Asciiblob:
    case Binaryblob:
      *(int32 *)ptr = format->table->getBlobId(value, *(int32 *)ptr, cloneFlag,
                                               transaction);
      break;
...
int Record::getFormatVersion() {
  switch (encoding) {
    ...
    case shortVector:
      return -*(short *)(data.record + ((USHORT *)data.record)[0] -
                         sizeof(short));
...
Record *Record::fetchVersion(Transaction *transaction) { return this; }
...
void Record::getRawValue(int fieldId, Value *value) {
  ...
  if (fieldId >= format->maxId) return;

  // If chilled, restore the record data from the stream log
  ...
  switch (encoding) {
    ...
    case valueVector:
      value->setValue(
          ((Value *)data.record) + format->format[fieldId].physicalId, false);
      return;
  ...
  if (!(data.record[NULL_BYTE(ff)] & NULL_BIT(ff))) return;
...
bool Record::isVersion() { return false; }
...
void Record::scavenge(TransId targetTransactionId,
                      int oldestActiveSavePointId) {}

Record *Record::getPriorVersion() { return NULL; }
...
TransactionState *Record::getTransactionState() const { return NULL; }

bool Record::isSuperceded() { return false; }

void Record::addRef() { INTERLOCKED_INCREMENT(useCount); }
...
int Record::getBlobId(int fieldId) {
  if (fieldId >= format->maxId) return -1;

  switch (encoding) {
    case traditional: {
      FieldFormat *ff = format->format + fieldId;

      if (!(data.record[NULL_BYTE(ff)] & NULL_BIT(ff))) return -1;

      if (ff->type != Asciiblob && ff->type != Binaryblob) return -1;
...
TransId Record::getTransactionId() { return 0; }

int Record::getSavePointId() { return 0; }
...
void Record::getEncodedValue(int fieldId, Value *value) {
  switch (encoding) {
    case shortVector: {
      ...
      const UCHAR *q = EncodedDataStream::decode(
          (UCHAR *)data.record + vector[index], value, false);
...
int Record::setEncodedRecord(Stream *stream, bool interlocked) {
  ...
  if (interlocked) {
    ...
    // If data.record has changed since allocating the new buffer, then free the
    // new buffer
...
bool Record::isNull(int fieldId) {
  if (fieldId >= format->maxId) return true;
  ...
  switch (encoding) {
    ...
    case valueVector:
      return ((Value **)data.record)[format->format[fieldId].physicalId]
          ->isNull();
...
Record *Record::clearPriorVersion(void) { return NULL; }
...
void Record::deleteData(void) { deleteData(false); }
...
void Record::print(void) {
  printf("  %p\tId %d, enc %d, state %d, use %d, grp " I64FORMAT "\n", this,
         recordNumber, encoding, state, useCount, generation);
}
...
void Record::validateData(void) { ASSERT(data.record); }
...
Record *Record::getGCPriorVersion(void) { return NULL; }

void Record::expungeRecord(void) { format->table->expungeRecord(recordNumber); }
...
void Record::serialize(Serialize *stream) {
  ...
  if (data.record)
    stream->putData(getEncodedSize(),
                    (UCHAR *)data.record + ((USHORT *)data.record)[0]);
  else
    stream->putData(0, NULL);
}

int Record::getSize(void) { return sizeof(*this); }
...
void Record::ShowHistory(void) {
  ...
  for (int a = historyOffset; a >= 0; a--) {
    if (history[a].threadId == 0) break;
    historicState = (int)history[a].state;
    Log::log("%d ThreadId=%d state=%d useCount=%d+(%d)=%d  File=%s Line=%d\n",
             history[a].counter, history[a].threadId, historicState,
             history[a].useCount, history[a].delta,
             history[a].useCount + history[a].delta, history[a].file,
             history[a].line);
  }
