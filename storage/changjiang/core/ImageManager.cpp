...
static const char *ddl[] = {
    ...
    "grant select,insert,update,delete on images to public", NULL};
...
ImageManager::ImageManager(Database *db)
    : TableAttachment(PRE_INSERT | PRE_UPDATE | PRE_DELETE | POST_COMMIT) {
  database = db;
...
Images *ImageManager::getImages(const char *name, Application *extends) {
  ...
  for (images = hashTable[slot]; images; images = images->collision)
    if (images->name == applicationName) return images;
...
void ImageManager::tableAdded(Table *table) {
  if (table->isNamed("SYSTEM", "IMAGES")) table->addAttachment(this);
}
...
void ImageManager::preUpdate(Table *table, RecordVersion *record) {
  ...
  if (before.compare(&after) != 0) assignAlias(table, record);
}

void ImageManager::assignAlias(Table *table, RecordVersion *record) {
  ValueEx type(record, typeId);
  JString alias;
  alias.Format("NF%d_%d.%s", database->sequence, ++nextAlias, type.getString());
  Value value;
...
void ImageManager::insertCommit(Table *table, RecordVersion *record) {
  ...
  Image *image =
      new Image(database->getSymbol(name.getString()), width.getInt(),
                height.getInt(), alias.getString(), images);
...
Images *ImageManager::findImages(const char *name) {
  ...
  for (Images *images = hashTable[slot]; images; images = images->collision)
    if (images->name == imageName) return images;
...
void ImageManager::checkAccess(Table *table, RecordVersion *recordVersion) {
  ...
  Record *record = (recordVersion->hasRecord())
                       ? recordVersion
