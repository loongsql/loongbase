...
RepositoryVolume::RepositoryVolume(Repository *repo, int volume, JString file) {
  ...
  JString volumeName;
  volumeName.Format("%s.%s.%d", repository->schema, repository->name,
                    volumeNumber);
  tableSpace = database->tableSpaceManager->findTableSpace(volumeName);

  if (!tableSpace)
    tableSpace = database->tableSpaceManager->createTableSpace(
        volumeName, fileName, true, NULL);
...
void RepositoryVolume::storeBlob(BlobReference *blob,
                                 TransactionState *transaction) {
  storeBlob(blob->blobId, blob->getStream(), transaction);
}

void RepositoryVolume::storeBlob(int64 blobId, Stream *stream,
                                 TransactionState *transaction) {
  ...
  if (!section) section = dbb->findSection(VOLUME_SECTION_ID);
  ...
  if (recordNumber == 0)
    IndexRootPage::deleteIndexEntry(dbb, VOLUME_INDEX_ID, &indexKey,
  ...
  if (recordNumber > 0) {
    ...
    if (compare(stream, &stream2) == 0) return;

    throw SQLError(INCONSISTENT_BLOB,
                   "inconsistent values for repository blobId\n");
  }

  int recordId = dbb->insertStub(section, transaction);
  dbb->updateRecord(section, recordId, stream, transaction, true);
  IndexRootPage::addIndexEntry(dbb, VOLUME_INDEX_ID, &indexKey, recordId + 1,
                               transaction->transactionId);
}
...
void RepositoryVolume::open() {
  ...
  try {
    ...
    if (dbb->doesFileExist(fileName)) throw;
  ...
  try {
    ...
    if (header.fileType != HdrRepositoryFile)
      throw SQLError(
          RUNTIME_ERROR,
          "repository file \"%s\" has wrong page type (expeced %d, got %d)\n",
          (const char *)fileName, HdrRepositoryFile, header.fileType);

    if (header.pageSize != dbb->pageSize)
      throw SQLError(
          RUNTIME_ERROR,
          "repository file \"%s\" has wrong page size (expeced %d, got %d)\n",
    ...
    if (volumeNumber == 0)
      ...
      throw SQLError(RUNTIME_ERROR,
                     "repository file \"%s\" has wrong volume number (expected "
                     "%d, got %d)\n",
    ...
    if (name != "" && name != repository->name)
      throw SQLError(RUNTIME_ERROR,
                     "repository file \"%s\" has wrong repository name "
                     "(expected %s, got %s)\n",
                     (const char *)fileName, (const char *)repository->name,
                     (const char *)name);
  } catch (...) {
    isOpen = false;
    isWritable = false;
...
void RepositoryVolume::create() {
  ...
  header->putHeaderVariable(dbb, hdrRepositoryName,
                            (int)strlen(repository->name), repository->name);
...
#ifdef _BIG_ENDIAN
  for (UCHAR *q = stuff.chars; q < stuff.chars + 8;) *p++ = *q++;
#else
  for (UCHAR *q = stuff.chars + 8; q > stuff.chars;) *p++ = *--q;
#endif
...
#ifdef _BIG_ENDIAN
  for (UCHAR *q = stuff.chars, *p = key; q < stuff.chars + 8;) *q++ = *p++;
#else
  for (UCHAR *q = stuff.chars + 8, *p = key; q > stuff.chars;) *--q = *p++;
#endif
...
void RepositoryVolume::getBlob(BlobReference *blob) {
  ...
  if (recordNumber == 0)
    throw SQLError(DELETED_BLOB,
                   "blob previously deleted from repository file \"%s\"\n",
                   (const char *)fileName);
...
int RepositoryVolume::getRecordNumber(IndexKey *indexKey) {
  if (!rootPage) rootPage = IndexRootPage::getIndexRoot(dbb, VOLUME_INDEX_ID);

  Bitmap bitmap;
  // dbb->scanIndex (VOLUME_INDEX_ID, VOLUME_INDEX_VERSION, indexKey, indexKey,
  // false, bitmap);
  IndexRootPage::scanIndex(dbb, VOLUME_INDEX_ID, rootPage, indexKey, indexKey,
                           false, NO_TRANSACTION, &bitmap);
...
void RepositoryVolume::deleteBlob(int64 blobId, Transaction *transaction) {
  ...
  if (recordNumber == 0)
    throw SQLError(DELETED_BLOB,
                   "blobId already deleted repository file \"%s\"\n",
                   (const char *)fileName);

  IndexKey indexKey;
  makeKey(blobId, &indexKey);
  IndexRootPage::addIndexEntry(dbb, VOLUME_INDEX_ID, &indexKey, 0,
                               transaction->transactionId);
  IndexRootPage::deleteIndexEntry(dbb, VOLUME_INDEX_ID, &indexKey,
                                  recordNumber + 1, transaction->transactionId);
}
...
void RepositoryVolume::scavenge() {
  if (!isOpen || lastAccess + TIMEOUT > database->timestamp) return;
...
void RepositoryVolume::synchronize(Transaction *transaction) {
  ...
  try {
    ...
    bdb = IndexRootPage::findLeaf(dbb, VOLUME_INDEX_ID, 0, &indexKey, Shared,
                                  transaction->transactionId);
    ...
    for (;;) {
      ...
      for (IndexNode node(page); node.node < bucketEnd;
           node.getNext(bucketEnd)) {
...
void RepositoryVolume::synchronize(int64 id, Stream *stream,
  ...
  if (!stream) {
    ...
    if (recordNumber > 0) deleteBlob(id, transaction);
  ...
  if (stream2.compare(stream))
    throw SQLError(INCONSISTENT_BLOB,
                   "inconsistent blob %ld in repository \"%s.%s\" volume %d\n",
                   id, repository->schema, repository->name, volumeNumber);
}

void RepositoryVolume::reportStatistics() { dbb->reportStatistics(); }
