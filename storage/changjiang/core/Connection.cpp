...
static const char *ddl[] = {"grant all on system.sequences to %s",
...
Connection::Connection(Configuration *config) { init(config); }
...
Connection::~Connection() {
  if (logFile) fclose((FILE *)logFile);

  if (database) detachDatabase();
...
void generateDigest(const char *string, char *digest) {
  ...
  EncryptTransform<StringTransform, SHATransform, Base64Transform> encrypt(
      string, 0);
...
Transaction *Connection::getTransaction() {
  if (!transaction) transaction = database->startTransaction(this);
...
void Connection::commit() {
  ...
  for (int n = 0; (trans = transaction); ++n) {
    if (n > 5) throw SQLEXCEPTION(RUNTIME_ERROR, "non-converging transaction");
...
void Connection::transactionEnded() {
  if (resultSets || statements) {
    ...
    for (ResultSet *resultSet = resultSets; resultSet;
         resultSet = resultSet->connectionNext)
      resultSet->transactionEnded();

    for (Statement *statement = statements; statement;
         statement = statement->connectionNext)
...
void Connection::prepare(int xidSize, const UCHAR *xid) {
  ...
  if (transaction) transaction->prepare(xidSize, xid);
}

void Connection::close() { release(); }
...
void Connection::shutdown() {
  ...
  if (server) {
    try {
      server->addRef();
      Thread *thread =
          new Thread("Connection::shutdown", NULL);  // shutdown, this, NULL);
...
Statement *Connection::findStatement(const char *cursorName) {
  ...
  for (Statement *statement = statements; statement;
       statement = statement->connectionNext)
    if (statement->cursorName == cursorName) return statement;
...
Clob *Connection::genHTML(TemplateContext *context, int32 genHeaders) {
  ...
  try {
    ...
    const char *defaultApplication =
        context->getValue("DEFAULT_APPLICATION", "UnknownApplication");

    if (!applicationName) applicationName = defaultApplication;
...
#ifdef FORCE_ALIASES
    if (application->name == defaultApplication) context->enableAliases();
#endif
    ...
    ***/
    ...
    if (!session)
      if (session =
              sessionManager->findSession(application, context, httpCookie))
        session->addRef();
    ...
    if (!session->activate(false)) session->activate(true);

#ifdef LICENSE
    if (!licenseToken)
      if (licenseToken = session->licenseToken) licenseToken->addRef();
#endif
    ...
    if (licenseToken)
      ...
      database->genHTML(NULL, application->name, "licenseExceeded", context,
                        &stream, NULL);
#else
    sessionManager->execute(this, application, context, &stream);
#endif
  ...
  if (genHeaders) {
    ...
    if (!cookie && session) {
      ...
      char expiration[100];
      formatDateString("%a %d-%b-%y %H:%M:%S GMT", sizeof(expiration), date,
                       expiration);
      setCookie.Format("set-cookie: %s=%s; expires=%s;\r\n", COOKIE_NAME,
                       (const char *)session->sessionId, expiration);
    ...
    if (newSession) {
      ...
      if (p) {
        setCookie.Format("set-cookie: %s=%s; expires=%s; path=%.*s\r\n",
                         applicationName, (const char *)session->sessionId,
                         expiration, p + 1 - scriptName, scriptName);
        headers->putSegment(setCookie);
      }
    }
  ...
  if (stream.totalLength == 0)
    stream.putSegment(
        "<html><body>Well, almost nothing got generated.  "
        "Sorry.</body></html>");
...
void Connection::genNativeMethod(const char *className, const char *fileName,
                                 GenOption option) {
  // JavaGenNative::genNativeMethod (database->java, className, fileName,
  // option);
}
...
void Connection::popSchemaName() {
  if (!nameSpace.isEmpty()) nameSpace.pop();
}
...
DatabaseMetaData *Connection::getMetaData() {
  if (!metaData) metaData = new DatabaseMetaData(this);

  return metaData;
}

void Connection::addRef() { INTERLOCKED_INCREMENT(useCount); }

void Connection::release() {
  if (INTERLOCKED_DECREMENT(useCount) == 0) closeConnection();
}

void Connection::closeConnection() {
#ifndef STORAGE_ENGINE
  if (activeDebugger) database->detachDebugger();
#endif
...
char *Connection::getCookie(const char *cookie, const char *name, char *temp,
                            int length) {
...
void Connection::validatePrivilege(PrivilegeObject *object, PrivType priv) {
  ...
  if (user && (user->getPrivileges(object) & mask)) return;

  FOR_OBJECTS(Role *, role, &activeRoles)
  if (role->getPrivileges(object) & mask) return;
  END_FOR;
...
void Connection::addRole(Role *role) {
  if (activeRoles.appendUnique(role)) role->addRef();
}
...
void Connection::addImage(Image *image) {
  ...
  if (!images.appendUnique(image)) image->release();
}
...
Role *Connection::findRole(const WCString *schemaName,
                           const WCString *roleName) {
  return database->findRole(schemaName, roleName);
}
...
int Connection::hasRole(Role *role) {
  ...
  if (activeRoles.isMember(role)) mask |= ROLE_ACTIVE;

  if (user->system) mask |= GRANT_OPTION;
...
void Connection::validate(int optionMask) {
  ...
  ***/
  ...
  if (database) database->validate(optionMask);
}

bool Connection::assumeRole(Role *role) {
  int mask = user->hasRole(role);

  if (!(mask & HAS_ROLE)) return false;

  if (!(mask & ROLE_ACTIVE)) addRole(role);

  return true;
}

void Connection::openDatabase(const char *dbName, Parameters *parameters,
                              Threads *parent) {
...
void Connection::openDatabase(const char *dbName, const char *filename,
                              const char *account, const char *password,
                              const char *address, Threads *parent) {
  char dbFileName[256];
  ...
  if (filename)
    ...
    if (!registry.findDatabase(dbName, sizeof(dbFileName), dbFileName))
      throw SQLEXCEPTION(CONNECTION_ERROR, "can't find database \"%s\"",
                         dbName);
  }
  ...
  if (!user)
    throw SQLEXCEPTION(SECURITY_ERROR,
                       "\"%s\" is not a known user for database \"%s\"",
                       account, dbName);

  if (user->coterie) {
    if (!address || !user->validateAddress(atoi(address)))
      throw SQLEXCEPTION(SECURITY_ERROR,
                         "invalid address for user \"%s\" in database \"%s\"",
                         account, dbName);
  }

  if (!user->validatePassword(password))
    throw SQLEXCEPTION(SECURITY_ERROR,
                       "invalid password for user \"%s\" in database \"%s\"",
                       account, dbName);

  setUser(user);
}

void Connection::createDatabase(const char *dbName, Parameters *parameters,
                                Threads *parent) {
  if (database)
    throw SQLEXCEPTION(CONNECTION_ERROR, "database is already open");
...
Database *Connection::createDatabase(const char *dbName, const char *fileName,
                                     const char *account, const char *password,
                                     Threads *threads) {
  char dbFileName[1024];
  ...
  if (!firstDatabase) syncDatabaseList.setName("Connection::syncDatabaseList");
...
void Connection::requiresLicense() {
  ...
  if (licenseNotRequired) return;

  if (database->systemConnection == this) return;

  LicenseProduct *product =
      database->licenseManager->getProduct(SERVER_PRODUCT);
...
int64 Connection::getSequenceValue(const char *sequenceName) {
  ...
  if (!sequence)
    throw SQLEXCEPTION(RUNTIME_ERROR, "Sequence \"%s\" is not defined",
                       sequenceName);
...
int64 Connection::getSequenceValue(const char *schemaName,
                                   const char *sequenceName) {
  Sequence *sequence = getSequence(database->getSymbol(schemaName),
                                   database->getSymbol(sequenceName));

  return sequence->update(1, getTransaction());
}

int64 Connection::getSequenceValue(const WCString *schemaName,
                                   const WCString *sequenceName) {
  Sequence *sequence = getSequence(database->getSymbol(schemaName),
                                   database->getSymbol(sequenceName));
...
const char *Connection::getAttribute(const char *name,
                                     const char *defaultValue) {
  if (!strcasecmp(name, "account")) return user->name;
...
bool Connection::checkAccess(int32 privMask, PrivilegeObject *object) {
  if (database->systemConnection == this) return true;

  if (user->system) return true;

  int32 mask = user->getPrivileges(object);

  if (!(privMask & ~mask)) return true;
...
bool Connection::dropRole(Role *role) {
  if (!activeRoles.deleteItem(role)) return false;
...
void Connection::formatDateString(const char *format, int length, int32 date,
                                  char *buffer) {
  struct tm time;
...
void Connection::setAutoCommit(bool flag) { autoCommit = flag; }

bool Connection::getAutoCommit() { return autoCommit; }

void Connection::updateSchedule(const char *application, const char *eventName,
                                const char *schedule) {
...
void Connection::setUser(User *usr) {
  if ((user = usr))
    for (UserRole *userRole = user->roleList; userRole;
         userRole = userRole->next)
      if (userRole->defaultRole) addRole(userRole->role);
}
...
void Connection::disableFilterSet(FilterSet *filterSet) {
  if (filterSets.deleteItem(filterSet)) filterSet->release();
}
...
JString Connection::analyze(int mask) { return database->analyze(mask); }
...
void Connection::checkSitePassword(Parameters *parameters) {
  ...
  if (!sitePassword)
    throw SQLEXCEPTION(CONNECTION_ERROR,
                       "a site password is required for server operator");
...
void Connection::clearStatements() {
  Sync global(&database->syncConnectionStatements,
              "Connection::clearStatements");
...
void Connection::setLicenseNotRequired(bool flag) { licenseNotRequired = flag; }

bool Connection::enableFilterSet(const WCString *schemaName,
                                 const WCString *filterSetName) {
  const char *schema = database->getSymbol(schemaName);
  const char *name = database->getSymbol(filterSetName);
  FilterSet *filterSet =
      database->filterSetManager->findFilterSet(schema, name);
...
void Connection::disableFilterSet(const WCString *schemaName,
                                  const WCString *filterSetName) {
  const char *schema = database->getSymbol(schemaName);
  const char *name = database->getSymbol(filterSetName);
  FilterSet *filterSet =
...
Sequence *Connection::getSequence(const char *schemaName,
                                  const char *sequenceName) {
  SequenceManager *manager = database->sequenceManager;
  Sequence *sequence = manager->findSequence(schemaName, sequenceName);

  if (!sequence)
    throw SQLEXCEPTION(RUNTIME_ERROR, "Sequence \"%s.%s\" is not defined",
...
void Connection::deleteRepositoryBlob(const char *schema,
                                      const char *repositoryName, int volume,
                                      int64 blobId) {
  database->deleteRepositoryBlob(schema, repositoryName, volume, blobId,
                                 getTransaction());
}

PreparedStatement *Connection::findRegisteredStatement(StatementType type,
                                                       void *owner) {
  for (RegisteredStatement *reg = registeredStatements; reg; reg = reg->next)
    if (reg->type == type && reg->owner == owner) return reg->statement;

  return NULL;
}

void Connection::registerStatement(StatementType type, void *owner,
...
Statement *Connection::findStatement(int32 handle) {
  for (Statement *statement = statements; statement;
       statement = statement->connectionNext)
    if (statement->handle == handle) return statement;

  return NULL;
}

int32 Connection::getNextHandle() { return ++nextHandle; }

ResultSet *Connection::findResultSet(int32 handle) {
  for (ResultSet *resultSet = resultSets; resultSet;
       resultSet = resultSet->connectionNext)
    if (resultSet->handle == handle) return resultSet;
...
Database *Connection::getDatabase(const char *dbName, const char *dbFileName,
                                  Threads *threads) {
...
Sequence *Connection::findSequence(const char *schema, const char *name) {
  return database->sequenceManager->findSequence(database->getSymbol(schema),
                                                 database->getSymbol(name));
}

void Connection::dropDatabase() {
  if (!database) throw SQLEXCEPTION(CONNECTION_ERROR, "database isn't open");
...
void Connection::detachDatabase() {
...
#ifndef STORAGE_ENGINE
  for (JavaConnection *connection = javaConnections; connection;
       connection = connection->next)
    connection->clear();
#endif

#ifdef LICENSE
  if (licenseToken) licenseToken->release();
#endif
  ...
  if (resultSets) clearResultSets();

  if (statements) clearStatements();
...
void Connection::setTransactionIsolation(int level) { isolationLevel = level; }

int Connection::getTransactionIsolation() { return isolationLevel; }

void Connection::commitByXid(int xidLength, const UCHAR *xid) {
  if (database) database->commitByXid(xidLength, xid);
}

void Connection::rollbackByXid(int xidLength, const UCHAR *xid) {
  if (database) database->rollbackByXid(xidLength, xid);
}
...
void Connection::setRecordMemoryMax(uint64 value) {
  if (database) database->setRecordMemoryMax(value);
}

void Connection::setRecordScavengeThreshold(int value) {
  if (database) database->setRecordScavengeThreshold(value);
}

void Connection::setRecordScavengeFloor(int value) {
  if (database) database->setRecordScavengeFloor(value);
}
