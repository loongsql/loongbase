...
int DataPage::updateRecord(Section *section, int lineNumber, Stream *stream,
  ...
  */
  ...
  for (int n = 0; idx < end; ++idx, ++n) {
    ASSERT(idx->offset == 0 || (idx->offset >= indexEnd &&
                                idx->offset + idx->length <= dbb->pageSize));

    if (idx->offset && n != lineNumber) {
      used += ABS(idx->length);

      if (idx->offset < highWater) highWater = idx->offset;
    }
  }
  ...
  if (available < (int)sizeof(int32)) return 0;

  if (available < length) {
    overflowPageNumber =
        section->storeTail(stream, available, &length, transId, earlyWrite);
    length += sizeof(int32);
...
int DataPage::compressPage(Dbb *dbb) {
  ...
  for (LineIndex *index = lineIndex, *end = index + maxLine; index < end;
       ++index)
    if (index->offset) {
...
bool DataPage::fetchRecord(Dbb *dbb, int slot, Stream *stream) {
  ...
  ASSERT(offset >= OFFSET(DataPage *, lineIndex) &&
         offset + length <= dbb->pageSize);
  ...
  while (overflowPageNumber) {
    ...
    stream->putSegment(page->length, (char *)page + sizeof(DataOverflowPage),
                       true);
...
int DataPage::storeRecord(Dbb *dbb, Bdb *bdb, RecordIndex *index, int length,
                          Stream *stream, int32 overflowPageNumber,
                          TransId transId, bool earlyWrite) {
  if (overflowPageNumber) length += sizeof(int32);

  short id = -1;
  short highWater = dbb->pageSize;
  int spaceRemaining = dbb->pageSize - OFFSET(DataPage *, lineIndex) -
                       maxLine * sizeof(LineIndex) - length;
  LineIndex *line, *end;

  for (line = lineIndex, end = line + maxLine; line < end; ++line)
    if (line->offset) {
      if (line->offset < highWater) highWater = line->offset;
...
void DataPage::validate(Dbb *dbb) {
  ...
  for (int n = 0; n < maxLine; ++n) {
    ...
    ASSERT(index->offset == 0 ||
           (index->offset >= indexEnd &&
            index->offset + index->length <= dbb->pageSize));
  }
}

void DataPage::deleteOverflowPages(Dbb *dbb, int32 overflowPageNumber,
                                   TransId transId) {
  ...
  while (overflowPageNumber) {
    ...
    Bdb *bdb =
        dbb->fetchPage(overflowPageNumber, PAGE_data_overflow, Exclusive);
    BDB_HISTORY(bdb);
    DataOverflowPage *page = (DataOverflowPage *)bdb->buffer;
    overflowPageNumber = page->nextPage;
  ...
  for (int n = 0; n < maxLine; ++n)
    if (lineIndex[n].offset && lineIndex[n].length < 0) {
      ...
      while (overflowPageNumber) {
        ...
        if (validation->isPageType(bdb, PAGE_data_overflow,
                                   "DataOverFlowPage")) {
...
int DataPage::computeSpaceAvailable(int pageSize) {
  ...
  for (int n = 0; n < maxLine; ++n)
    if (lineIndex[n].offset) spaceUsed += ABS(lineIndex[n].length);
...
void DataPage::deletePage(Dbb *dbb, TransId transId) {
  for (int line = 0; line < maxLine; ++line)
    if (lineIndex[line].length < 0) {
      ...
      if (overflowPageNumber && !dbb->streamLog->recovering)
        deleteOverflowPages(dbb, overflowPageNumber, transId);
    }
}
