...
class IndexNode {
 public:
  ...
  inline static int nodeLength(int offset, int length, int32 number) {
    return ((offset >= 128) ? 2 : 1) + ((length >= 128) ? 2 : 1) + length +
           RECORD_NUMBER_LENGTH(number);
  }
...
inline Btn *IndexNode::parseNode(Btn *indexNode) {
  ...
  if ((offset = *key++) >= 128) offset = ((offset & 0x7f) << 8) | *key++;

  if ((length = *key++) >= 128) length = ((length & 0x7f) << 8) | *key++;
...
inline int32 IndexNode::getNumber() { return number; }
