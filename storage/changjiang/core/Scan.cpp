...
bool Scan::start(Database *database) {
  ...
  if (dbb->debug & (DEBUG_PAGES | DEBUG_SCAN_INDEX)) page->printPage(bdb);
...
bool Scan::hit(int32 wordNumber) {
  ...
  if (numbers[WORD_NUMBER] != wordNumber) return false;

  if (next) return next->hit(wordNumber + 1);
...
void Scan::print() {
  Log::debug("%s tbl %d, rec %d, fld %d, word %d, mask %x, hit %d\n", key,
             numbers[0], numbers[1], numbers[2], numbers[3], caseMask, hit());
}
...
bool Scan::hit() {
  if (!frequentWord) {
    ...
    if (searchMask & ~caseMask) return false;
  }

  if (next && !next->frequentTail) {
    if (next->compare(3, numbers) != 0) return false;
...
int Scan::compare(int count, int32 *nums) {
  ...
  for (int n = 0; n < count; ++n)
    if (nums[n] != numbers[n]) return (numbers[n] < nums[n]) ? -1 : 1;
...
int32 *Scan::getNumbers() {
  if (!frequentWord) return numbers;

  if (next) return next->getNumbers();
...
bool Scan::validate(Scan *prior) {
  if (!frequentWord) {
    if (next) return next->validate(this);
    return true;
  }
  ...
  *p++ = 0;
  ...
  for (int n = 0; n < 4; ++n) Inv::encode(numbers[n], &p);
  ...
  if (length != expandedLength) return false;

  if (memcmp(key, expandedKey, length)) return false;
...
bool Scan::validateWord(int32 *wordNumbers) {
  ...
  *p++ = 0;
  ...
  for (int n = 0; n < 3; ++n) Inv::encode(wordNumbers[n], &p);

  int length = (int)(p - key);
  IndexKey indexKey(length, key);

  if (!(bdb = inversion->findInversion(&indexKey, Shared))) return false;
  ...
  if (compare(3, wordNumbers)) return false;

  if (next) return next->validateWord(wordNumbers);
