...
AddNodeResult Index2Page::addNode(Dbb *dbb, IndexKey *indexKey,
                                  int32 recordNumber) {
  ...
  if ((UCHAR *)node.node == (UCHAR *)this + length)
    ...
    offset2 = computePrefix(indexKey->keyLength, indexKey->key,
                            node.offset + node.length, nextKey);
  ...
  if (length + delta > dbb->pageSize) {
    ...
    if (offset2 == -1 || keyCompare(node.keyLength(), nextKey,
                                    indexKey->keyLength, indexKey->key) >= 0)
      return NextPage;
  ...
  ***/
  ...
  if (offset2 >= 0) {
    ...
    if (tailLength > 0) memmove(tail + delta, tail, tailLength);
  }
  ...
  if (offset2 >= 0)
    newNode.insert(newNode.nextNode, offset2, nextLength - offset2, nextKey,
                   nextNumber);
...
Btn *Index2Page::findNode(IndexKey *indexKey, IndexKey *expandedKey) {
  ...
  if (indexKey->keyLength == 0) {
    if (expandedKey) expandedKey->keyLength = 0;
  ...
  for (;; node.getNext()) {
    if (node.node >= bucketEnd) {
      ...
      if (expandedKey) expandedKey->keyLength = priorLength;
...
Bdb *Index2Page::splitIndexPageMiddle(Dbb *dbb, Bdb *bdb, IndexKey *splitKey,
                                      TransId transId) {
  ...
  Btn *midpoint =
      (Btn *)((UCHAR *)nodes + (length - OFFSET(Index2Page *, nodes)) / 2);
...
Btn *Index2Page::findNodeInBranch(IndexKey *indexKey) {
  ...
  for (Index2Node node(this); node.node < bucketEnd;
       prior = node.node, node.getNext()) {
    UCHAR *q = node.key;

    if (node.offset < offset) return prior;
    ...
    for (;;) {
      if (p == keyEnd) return prior;  // searchKey <= node, return prior

      if (q == nodeEnd || *p > *q) break;  // searchKey > node, keep looking

      if (*p++ < *q++) return prior;  // searchKey <= node, return prior
    }
...
void Index2Page::validate(void *before) {
  ...
  for (Index2Node node(this);; node.getNext()) {
    if (node.node >= bucketEnd) {
      if (node.node != bucketEnd) {
        if (before) printPage((Index2Page *)before, 0, false);
        printPage(this, 0, false);
...
Bdb *Index2Page::createNewLevel(Dbb *dbb, int level, int version, int32 page1,
...
Bdb *Index2Page::findLevel(Dbb *dbb, Bdb *bdb, int level, IndexKey *indexKey,
                           int32 recordNumber) {
  ...
  for (bool first = true; node.node < bucketEnd;
    ...
    if (number == recordNumber) {
      ...
      if (next.node >= bucketEnd)
        ...
        int prefix = computePrefix(priorKey.keyLength, priorKey.key, nextLength,
                                   nextKey);
        ...
        if (tailLength > 0) memcpy(newTail, tail, tailLength);
...
void Index2Page::printPage(Index2Page *page, int32 pageNumber, bool inversion) {
  ...
  Log::debug(
      "Btree Page %d, level %d, length %d, prior %d, next %d, parent %d\n",
      pageNumber, page->level, page->length, page->priorPage, page->nextPage,
      page->parentPage);
  ...
  for (; node.node < end; node.getNext()) {
    Log::debug(
        "   %d. offset %d, length %d, %d: ", (char *)node.node - (char *)page,
...
void Index2Page::validate(Dbb *dbb, Validation *validation, Bitmap *pages,
...
void Index2Page::validateNodes(Dbb *dbb, Validation *validation,
    ...
    if (!children->isSet(pageNumber)) {
      ...
      if (level == 1 && bdb->buffer->pageType == PAGE_inversion) {
        ...
        if (indexPage->parentPage == parentPageNumber)
          ...
          validation->error(
              "lost index child page %d, index id %d has wrong parent",
...
/***
void Index2Page::validateInsertion(int keyLength, UCHAR *key, int32
recordNumber)
{
        Btn *bucketEnd = (Btn*) ((char*) this + length);
        Index2Node node (this);
...
Bdb *Index2Page::splitIndexPageEnd(Dbb *dbb, Bdb *bdb, TransId transId,
  ...
  int offset = computePrefix(insertKeyLength, insertKey->key, tempKey.keyLength,
                             tempKey.key);

  // nodeOverhead assumes END_BUCKET is the recordNumber.
  int nodeOverhead = 3 + (offset >= 128) + (insertKeyLength - offset >= 128);
  char *nextNextNode = (char *)node.nextNode - node.length + insertKeyLength -
                       offset + nodeOverhead;
  ...
  // printf ("splitIndexPage: level %d, %d -> %d\n", level, bdb->pageNumber,
  // nextPage); validate( splitBdb->buffer);
...
Btn *Index2Page::appendNode(int newLength, UCHAR *newKey, int32 recordNumber,
                            int pageSize) {
  UCHAR key[MAX_PHYSICAL_KEY_LENGTH];
  ...
  for (; node.node < end; node.getNext()) keyLength = node.expandKey(key);
...
Btn *Index2Page::appendNode(IndexKey *newKey, int32 recordNumber,
                            int pageSize) {
  ...
  for (; node.node < end; node.getNext()) node.expandKey(&key);
...
int Index2Page::keyCompare(int length1, UCHAR *key1, int length2, UCHAR *key2) {
  ...
  for (; length1 && length2; --length1, --length2)
    if (*key1++ != *key2++) return (key1[-1] > key2[-1]) ? -1 : 1;
...
Btn *Index2Page::findInsertionPoint(IndexKey *indexKey, int32 recordNumber,
  ...
  for (;; node.getNext()) {
    ...
    if (node.offset > offset) continue;
    ...
    for (;;) {
      if (p == keyEnd) {
        if (level == 0 && recordNumber > number &&
            (node.offset + node.length == indexKey->keyLength))
          break;

        goto exit;
      }

      if (q == nodeEnd || *p > *q) break;
...
int32 Index2Page::getRecordNumber(const UCHAR *ptr) {
  ...
  for (int n = 0; n < 4; ++n) number = number << 8 | *ptr++;
...
void Index2Page::logIndexPage(Bdb *bdb, TransId transId) {
  ...
  dbb->streamLog->logControl->indexPage.append(
      dbb, transId, INDEX_VERSION_0, bdb->pageNumber, indexPage->level,
      indexPage->nextPage, indexPage->length - OFFSET(Index2Page *, nodes),
      (const UCHAR *)indexPage->nodes);
}

Btn *Index2Page::findInsertionPoint(int level, IndexKey *indexKey,
  ...
  if (node.offset) {
    for (; offset < node.offset &&
           indexKey->key[offset] == expandedKey->key[offset];
         ++offset)
      ;
