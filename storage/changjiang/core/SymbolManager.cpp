...
int init() {
  ...
  for (char c = 'a'; c <= 'z'; ++c) caseTable[(int)c] = c - 'a' + 'A';
...
const char *SymbolManager::findSymbol(const char *string) {
  ...
  for (Symbol *symbol = hashTable[value % SYMBOL_HASH_SIZE]; symbol;
       symbol = symbol->collision) {
    const char *s = symbol->symbol;
    const char *q = string;
    for (; *s && *q && *p == caseTable[(int)*s]; ++s, ++q)
      ;
    if (*s == *q) return symbol->symbol;
  }
...
const char *SymbolManager::getSymbol(const char *string) {
  for (SymbolSection *section = sections; section; section = section->next)
    if (section->space <= string &&
        string < section->space + sizeof(section->space))
      return string;
...
const char *SymbolManager::getString(const char *string) {
  ...
  for (p = string; (c = *p++);) value = value * HASH_MULTIPLIER + c;
...
const char *SymbolManager::getSymbol(const WCString *string) {
  ...
  sync.lock(Shared);
  int value = 0, c;
  // int length = Unicode::getUtf8Length(string->count, (UCHAR*)
  // string->string);
  ...
  for (symbol = hashTable[slot]; symbol; symbol = symbol->collision) {
    ...
    for (; *s && q < end && *s == caseTable[(*q) & 0xff]; ++s, ++q)
      ;
    if (*s == 0 && q == end) return symbol->symbol;
  }
  ...
  while (p < end) *s++ = caseTable[(*p++) & 0xff];
