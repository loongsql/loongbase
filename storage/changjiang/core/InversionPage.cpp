...
bool InversionPage::addNode(Dbb *dbb, IndexKey *indexKey) {
  ...
  if (l2 == keyLength && l2 == (node->offset + node->length)) return true;

  /* If new node doesn't fit on page, split the page now */

  if (length + delta > dbb->pageSize) return false;
...
Inv *InversionPage::findNode(int keyLength, UCHAR *key, UCHAR *expandedKey,
                             int *expandedLength) {
  int offset = 0;
  int priorLength = 0;
  ...
  for (node = nodes;; node = NEXT_INV(node)) {
    if (node >= bucketEnd) {
      ...
      if (expandedLength) *expandedLength = priorLength;
    ...
    if (node->offset > offset || l == 0) continue;
...
Bdb *InversionPage::splitInversionPage(Dbb *dbb, Bdb *bdb, IndexKey *indexKey,
                                       TransId transId) {
  ...
  for (node = nodes;; node = NEXT_INV(node)) {
    ...
    if (node > midpoint && (node->offset || node->length)) break;
  }
  ...
  // dbb->streamLog->logControl->inversionPage.append(splitBdb->pageNumber,
  // parentPage, split->priorPage, nextPage);
  ...
  // validate();
  split->length =
      (int)((newNode->key + newNode->length + tailLength) - (UCHAR *)split);
  // split->validate();
...
void InversionPage::printPage(Bdb *bdb) {
  ...
  Log::debug("Inversion Page %d length %d, prior %d, next %d, parent %d\n",
             ((bdb) ? bdb->pageNumber : 0), length, priorPage, nextPage,
             parentPage);
  ...
  for (node = nodes; node < end; node = NEXT_INV(node)) {
    Log::debug("   %x. offset %d, length %d:\t", (char *)node - (char *)this,
...
void Inv::printKey(UCHAR *expandedKey) {
  ...
  for (n = 0; n < l; ++n) {
    UCHAR c;
    if (n == offset) Log::debug("*");
    if (n < offset)
      c = expandedKey[n];
    else
      c = key[n - offset];
    if (!c) null = true;
    if (!null && (ISLOWER(c) || c == '@' || c == '/' || c == ':' || ISDIGIT(c)))
      Log::debug("%c", c);
    else
      Log::debug("\\%x", c);
  }
  ...
  for (n = 0; n < 4; ++n) numbers[n] = Inv::decode(&p);

  Log::debug(" (%d,%d,%d,%d)", numbers[0], numbers[1], numbers[2], numbers[3]);

  while (p < end) Log::debug(" 0x%x", *p++);
}
...
int32 Inv::decode(UCHAR **ptr) {
  ...
  if (c & 0x80) {
    number = c & 0x1f;
    int count = c >> 5;
    for (int n = 3; n < count; ++n) number = number << 8 | *p++;
  } else
    number = c & 0x7f;
...
void InversionPage::removeNode(Dbb *dbb, int keyLength, UCHAR *key) {
  ...
  if (node->offset + node->length != keyLength) return;

  memcpy(nextKey, priorKey, priorLength);
  memcpy(nextKey + node->offset, node->key, node->length);
  // Inv::printKey (keyLength, nextKey);

  if (memcmp(nextKey, key, keyLength) != 0) return;
...
void Inv::printKey(int length, UCHAR *key) {
  ...
  for (n = 0; n < length; ++n) {
    ...
    if (c && (ISLOWER(c) || c == '@' || c == '/' || c == ':' || ISDIGIT(c)))
      Log::debug("%c", c);
    else
      Log::debug("\\%x", c);
  }
...
void InversionPage::logPage(Bdb *bdb) {
  ...
  dbb->streamLog->logControl->inversionPage.append(
      dbb, bdb->pageNumber, page->parentPage, page->priorPage, page->nextPage,
      page->length - OFFSET(IndexPage *, nodes), (const UCHAR *)page->nodes);
}
