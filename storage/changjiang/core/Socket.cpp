...
Socket *Socket::acceptSocket() {
  ...
  if (sock < 0) throw SQLEXCEPTION(NETWORK_ERROR, "ACCEPT failed");
...
Protocol *Socket::acceptProtocol() {
  ...
  for (;;) {
    ...
    if (FD_ISSET(socket, &fd_reads)) break;
  }
...
int Socket::read() {
  ...
  if (n <= 0) throw SQLEXCEPTION(NETWORK_ERROR, "recv failed");
...
int Socket::flush() {
  ...
  if (length) {
    ...
    if (n <= 0) throw SQLEXCEPTION(NETWORK_ERROR, "send failed");
  }
...
void Socket::putLong(int value) {
  if (swap) swapBytes(sizeof(value), &value);
...
void Socket::connect(const char *host, int port) {
  ...
  if (host) {
    const char *p;
    for (p = host; (c = *p++) && c != ':';) *q++ = c;

    *q = 0;

    if (c) portNumber = atoi(p);
  }
  ...
  if (host) {
    ...
    if (!hp) {
      ...
      throw SQLEXCEPTION(NETWORK_ERROR,
                         "gethostbyname for %s (%s) failed with %d", hostname,
                         host, getSocketError());
    }
  ...
  if (::connect(socket, (struct sockaddr *)&address, sizeof(address)) < 0) {
    close();
    throw SQLEXCEPTION(NETWORK_ERROR,
                       "connection to \"%s\" on port %d failed with error %d",
                       hostname, portNumber, getSocketError());
  }
...
int Socket::initialize() {
  ...
  if (n) throw SQLError(NETWORK_ERROR, "WSAStartup failed with %d\n", n);
#endif
...
void Socket::putHandle(int32 handle) { putBytes(sizeof(handle), &handle); }
...
int Socket::getInetFamily() { return AF_INET; }

void Socket::create() {
  socket = ::socket(AF_INET, SOCK_STREAM, 0);

  if (socket < 0) throw SQLEXCEPTION(NETWORK_ERROR, "socket allocation failed");
}
...
void Socket::setSwapBytes(bool flag) { swap = true; }
...
void Socket::putByte(char c) { putBytes(1, &c); }

int32 Socket::getPartnerAddress() { return ntohl(address.sin_addr.s_addr); }
...
int32 Socket::translateAddress(const char *hostname) {
  ...
  if (!hp)
    throw SQLEXCEPTION(NETWORK_ERROR, "can't resolve network address %s",
                       hostname);
...
void Socket::close(socket_t socket) {
  if (socket == CLOSED_SOCKET) return;
