...
Repository::Repository(const char *repositoryName, const char *repositorySchema,
  ...
  if (rollovers) setRollover(rollovers);
...
void Repository::storeBlob(BlobReference *blob, TransactionState *transaction) {
  ...
  if (maxSize && volumeNumber == currentVolume &&
      volume->getRepositorySize() > maxSize) {
    ++currentVolume;
    save();
  }
}
...
RepositoryVolume *Repository::getVolume(int volumeNumber) {
  ...
  if ((volume = findVolume(volumeNumber))) return volume;
...
RepositoryVolume *Repository::findVolume(int volumeNumber) {
  for (RepositoryVolume *volume = volumes[volumeNumber % VOLUME_HASH_SIZE];
    ...
    if (volume->volumeNumber == volumeNumber) return volume;
    ...
    if (c == '%' && *p) {
      ...
      if (c == '%')
        *q++ = c;
        ...
        for (const char *s = schema; *s;) *q++ = *s++;
      else if (c == 'r')
        for (const char *s = name; *s;) *q++ = *s++;
      else
        throw SQLError(DDL_ERROR,
                       "illegal substitution character \"%c\" in pattern "
                       "\"%s\" in repository %s\n",
...
int64 Repository::getFileSize(const char *token) {
  ...
  while (ISDIGIT(*p)) size = size * 10 + *p++ - '0';
...
RolloverPeriod Repository::getRolloverPeriod(const char *token) {
  if (strcmp(token, "MONTHLY") == 0) return RolloverMonthly;

  if (strcmp(token, "WEEKLY") == 0) return RolloverWeekly;

  throw SQLError(DDL_ERROR, "don't recognize rollover type \"%s\"\n", token);
}

void Repository::setRollover(const char *string) {
  if (rolloverString == string) return;
...
void Repository::deleteBlob(int volumeNumber, int64 blobId,
...
void Repository::scavenge() {
  for (int n = 0; n < VOLUME_HASH_SIZE; ++n)
    for (RepositoryVolume *volume = volumes[n]; volume;
         volume = volume->collision)
      volume->scavenge();
}
