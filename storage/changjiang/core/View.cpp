...
void View::compile(CompiledStatement *statement, Syntax *viewSyntax) {
  ...
  if (boolean) predicate = statement->compile(boolean);

  Syntax *sortClause = syntax->getChild(1);

  if (sortClause) sort = statement->compile(sortClause);

  if (branch->getChild(0)) distinct = true;

  for (n = 0; n < numberTables; ++n) statement->popContext();
    ...
    for (int m = n + 1; m < numberColumns; ++m)
      if (columnNames[n] == columnNames[m])
        throw SQLEXCEPTION(SYNTAX_ERROR,
                           "duplication column name \"%s\" in view %s.%s",
                           (const char *)columnNames[n], (const char *)schema,
                           (const char *)name);
...
void View::gen(Stream *stream) {
  ...
  if (distinct) stream->putSegment("distinct ");

  for (n = 0; n < numberColumns; ++n) {
    if (n != 0) stream->putCharacter(',');
  ...
  for (n = 0; n < numberTables; ++n) {
    if (schema == tables[n]->schemaName)
      stream->format("%s%s ctx%d", sep, (const char *)tables[n]->name, n);
    else
      stream->format("%s%s.%s ctx%d", sep, (const char *)tables[n]->schemaName,
...
void View::createFields(Table *table) {
  for (int n = 0; n < numberColumns; ++n) {
    ...
    int flags = 0;
    // Field *field =
    table->addField(columnNames[n], type.type, type.length, type.precision,
                    type.scale, flags);
  }
}
...
void View::save(Database *database) {
  PreparedStatement *statement = database->prepareStatement(
      "insert into system.view_tables "
      "(viewName,viewSchema,sequence,tableName,schema) values (?,?,?,?,?)");
...
bool View::isEquiv(View *view) {
  if (numberTables != view->numberTables ||
      numberColumns != view->numberColumns || distinct != view->distinct)
    return false;

  if (!equiv(predicate, view->predicate) || !equiv(columns, view->columns) ||
  ...
  for (n = 0; n < numberTables; ++n)
    if (tables[n] != view->tables[n]) return false;

  for (n = 0; n < numberColumns; ++n)
    if (columnNames[n] != view->columnNames[n]) return false;
