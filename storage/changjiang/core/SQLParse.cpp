...
static const PRIV privileges[] = {
    "SELECT",  nod_priv_select,  "INSERT", nod_priv_insert,
    "UPDATE",  nod_priv_update,  "DELETE", nod_priv_delete,
    "EXECUTE", nod_priv_execute, "ALTER",  nod_priv_alter,
    "GRANT",   nod_priv_grant,   "ALL",    nod_priv_all,
    NULL,      nod_priv_all};
...
SQLParse::SQLParse() {
  ...
  for (p = PUNCTUATION_CHARS; *p; ++p) characters[(int)*p] |= PUNCT;

  for (p = WHITE_SPACE; *p; ++p) characters[(int)*p] |= WHITE;

  for (p = MULTI_CHARS; *p; ++p) characters[(int)*p] |= MULTI_CHAR;
...
void SQLParse::getToken() {
  ...
  if (ipAddress && c >= '0' && c <= '9') {
    ...
    *t = 0;
    if (pos == length) error("unterminated quoted string");
    ++pos;
    ...
    for (; pos < length; ++pos) {
      c = string[pos];
      if (characters[(int)c] & TERM) break;
    }
...
Syntax *SQLParse::parseAlterNameSpace() {
  ...
  if (match("PUSH")) return makeNode(nod_push_namespace, parseName());

  if (match("POP")) return makeNode(nod_pop_namespace);
  ...
  for (;;) {
    if (match("ADD")) {
      ...
      if (match("PRIMARY")) {
        ...
        Syntax *table = NULL;
        if (match("REFERENCES")) table = parseIdentifier();
...
Syntax *SQLParse::parseAnd() {
  ...
  while (match("AND")) expr = makeNode(nod_and, expr, parseNot());
...
Syntax *SQLParse::parseCreateIndex(bool unique, SyntaxType nodeType) {
  ...
  if (unique) syntax->setChild(0, makeNode(nod_unique));

  syntax->setChild(1, parseName());

  if (!match("ON")) error("ON table");
...
Syntax *SQLParse::parseCreateTable(SyntaxType nodeType) {
  ...
  if (match("TABLESPACE")) tableSpace = parseName();
...
Syntax *SQLParse::parseCreateView(SyntaxType nodeType) {
  ...
  if (!match("AS")) return error("AS SELECT");

  if (!match("SELECT")) return error("SELECT");
...
Syntax *SQLParse::parseDataType() {
  ...
  } else if (match("NUMERIC") || match("NUMBER")) {
    parseParen();
    Syntax *size = parseNumber();
    Syntax *scale = NULL;

    if (match(",")) scale = parseNumber();
...
Syntax *SQLParse::parseDecimalNumber() {
  ...
  if (tokenType != DECIMAL_NUMBER) error("number");
...
Syntax *SQLParse::parseDelete() {
  ...
  if (!match("FROM")) return error("FROM");
...
void SQLParse::parseEndParen() {
  ...
  if (!match(")")) error("closing parenthesis");
}
...
Syntax *SQLParse::parseFieldClause() {
  ...
  if (match("NULL")) return makeNode(nod_null);
  ...
  if (match("DEFAULT")) return makeNode(nod_default_value, parseExpr());

  if (match("SEARCHABLE")) return makeNode(nod_searchable);

  if (match("COLLATION")) return makeNode(nod_collation, parseName());

  if (match("UNIQUE")) return makeNode(nod_unique);
  ...
  if (match("REFERENCES")) return parseForeignKey(NULL);
  ...
  if (match("CONSTRAINT")) {
    ...
    if (match("REFERENCES")) return parseForeignKey(constraintName);

    error("field constraint");
  }

  if (match("REPOSITORY")) return makeNode(nod_repository, parseName());
...
Syntax *SQLParse::parseForeignKey() {
  ...
  ***/
  ...
  if (!match("REFERENCES")) error("REFERENCES");
...
Syntax *SQLParse::parseGrant() {
  ...
  while (match(",")) {
    Syntax *privilege = parsePrivilege();
    if (!privilege) error("privilege");
    privs.append(privilege);
  }

  if (!match("ON")) error("ON");
...
Syntax *SQLParse::parseIdentifier() {
  ...
  if (tokenType != NAME && tokenType != QUOTED_NAME) error("identifier");
...
Syntax *SQLParse::parseInsert(SyntaxType nodType) {
  ...
  if (keyword("(")) syntax->setChild(1, parseNames());

  if (match("VALUES")) {
    if (!match("(")) error("parenthesis");
...
Syntax *SQLParse::parseNot() {
  ...
  if (match("NOT")) return makeNode(nod_not, parseNot());
...
Syntax *SQLParse::parseNumber() {
  ...
  if (tokenType != NUMBER) error("number");
...
Syntax *SQLParse::parseOr() {
  ...
  while (match("OR")) expr = makeNode(nod_or, expr, parseAnd());
...
Syntax *SQLParse::parseQuotedString() {
  ...
  if (tokenType != QUOTED_STRING) error("quoted string");
...
Syntax *SQLParse::parseSelect() {
  ...
  for (;;) {
    ...
    if (!match("UNION")) break;

    match("ALL");

    if (!match("SELECT")) error("SELECT");
  }
...
Syntax *SQLParse::parseSelectClause() {
  ...
  if (!match("*")) {
    ...
    for (;;) {
      ...
      if (match("AS")) expr = new Syntax(nod_alias, expr, parseName());
  ...
  if (!match("FROM")) error("FROM clause");

  syntax->setChild(2, parseJoin());

  if (match("WHERE")) syntax->setChild(3, parseBoolean());
  ...
  if (match("GROUP")) {
    ...
    if (match("HAVING")) syntax->setChild(5, parseBoolean());
  }
...
Syntax *SQLParse::parseSimpleBoolean() {
  ...
  if (match("EXISTS")) {
    if (!match("(")) error("parenthesised select express");
    if (!match("SELECT")) error("select expression");
  ...
  if (match(">")) {
    ...
    if (match("ACTIVE_ROLE")) return makeNode(nod_is_active_role, expr1);
    type = nod_is_null;
    if (match("NOT")) type = nod_not_null;
    if (!match("NULL")) error("NULL or ACTIVE_ROLE");
    return makeNode(type, expr1);
  } else if (match("IN")) {
    Syntax *syntax = parseIn(expr1);
    if (negate) syntax = makeNode(nod_not, syntax);
    return syntax;
  } else
...
Syntax *SQLParse::parseSimpleExpr() {
  ...
  if (match("?")) return makeNode(nod_parameter);
  ...
  if (match("CASE")) {
    ...
    if (keyword("WHEN")) {
      ...
      if (match("ELSE")) list.append(parseExpr());
      require("END");
  ...
  if (match("SELECT")) return makeNode(nod_select_expr, parseSelect());

  if (match("CAST")) {
    if (!match("(")) error("parenthesis");
    Syntax *value = parseExpr();
    if (!match("AS")) error("AS");
  ...
  if (match("NEXT_VALUE")) return makeNode(nod_next_value, parseIdentifier());
...
Syntax *SQLParse::parseStatement() {
  ...
  if (match("CREATE")) {
    if (match("DATABASE"))
      return makeNode(nod_create_database, parseName(), parseQuotedString());
    else if (match("TABLE"))
    ...
    ***/
    error(
        "database | table | view | index | unique index | sequence | trigger | "
        "filterset");
  }

  if (match("UPGRADE")) {
    if (match("TABLE")) return parseCreateTable(nod_upgrade_table);
  ...
  if (match("ALTER")) {
    if (match("TABLE")) return parseAlterTable();
    if (match("NAMESPACE")) return parseAlterNameSpace();
    if (match("USER")) return parseAlterUser();
    if (match("TRIGGER")) return parseAlterTrigger();
  ...
  if (match("DROP")) {
    ...
    ***/
    if (match("INDEX")) return makeNode(nod_drop_index, parseIdentifier());

    if (match("TABLE")) return makeNode(nod_drop_table, parseIdentifier());

    if (match("VIEW")) return makeNode(nod_drop_view, parseIdentifier());

    if (match("USER")) return makeNode(nod_drop_user, parseName());

    if (match("COTERIE")) return makeNode(nod_drop_coterie, parseName());

    if (match("ROLE")) return makeNode(nod_drop_role, parseIdentifier());

    if (match("SEQUENCE"))
      return makeNode(nod_drop_sequence, parseIdentifier());

    if (match("TRIGGER")) return makeNode(nod_drop_trigger, parseIdentifier());
    ...
    if (match("TABLESPACE")) return makeNode(nod_drop_tablespace, parseName());

    error("DATABASE | TABLE | VIEW | INDEX | CONSTRAINT | USER | ROLE");
  }

  if (match("RENAME")) {
    if (match("TABLE")) return renameTable();
    error("TABLE");
  }

  if (match("INSERT")) return parseInsert(nod_insert);

  if (match("REPLACE")) return parseInsert(nod_replace);
  ...
  if (match("REPAIR")) return parseRepair();

  if (match("GRANT")) return parseGrant();
  if (match("REVOKE")) return parseRevoke();
...
Syntax *SQLParse::parseTable(bool alias) {
  ...
  if (alias && tokenType == NAME && !keyword("WHERE") && !keyword("GROUP") &&
      !keyword("ORDER") && !keyword("LEFT") && !keyword("RIGHT") &&
      !keyword("FULL") && !keyword("JOIN") && !keyword("ON") &&
      !keyword("UNION"))
    synonym = parseName();
...
Syntax *SQLParse::parseUpdate() {
  ...
  if (!match("SET")) return error("SET clause");
...
int SQLParse::saveBegin() { return tokenStart; }
...
Syntax *SQLParse::parseRevoke() {
  ...
  return makeNode(nod_revoke, makeNode(privs), makeNode(objects),
                  makeNode(users));
}
...
Syntax *SQLParse::parseCreateUser() {
  ...
  if (match("ENCRYPTED")) encrypted = makeNode(nod_encrypted);

  if (!match("PASSWORD")) error("PASSWORD");

  // The following crock is for backward compatibility
  // Syntax *password = (tokenType == QUOTED_STRING) ? parseQuotedString() :
  // parseName();

  Syntax *password = parseQuotedString();
  Syntax *coterie = NULL;

  if (match("COTERIE")) coterie = parseName();
...
Syntax *SQLParse::parseAlterUser() {
  ...
  if (match("PASSWORD")) password = parseQuotedString();
...
Syntax *SQLParse::parseIn(Syntax *expr) {
  if (!match("(")) error("parenthesised value list");
...
Syntax *SQLParse::parseJoin() {
  ...
  if (match("ON")) boolean = parseBoolean();
  ...
  if (!match("JOIN")) {
    if (type == nod_inner) {
      if (boolean) return makeNode(nod_join_term, node, boolean);
...
Syntax *SQLParse::parseCreateTrigger(SyntaxType nodeType) {
  ...
  if (!match("USING")) error("USING 'trigger method'");
...
Syntax *SQLParse::parseAlterTrigger() {
  ...
  for (;;) {
    ...
    if (match ("INSERT"))
            {
            options.append (makeNode((before) ? nod_pre_insert :
    nod_post_insert)); verbPending = false;
            }
    else if (match ("UPDATE"))
            {
            options.append (makeNode((before) ? nod_pre_update :
    nod_post_update)); verbPending = false;
            }
    else if (match ("DELETE"))
            {
            options.append (makeNode((before) ? nod_pre_delete :
    nod_post_delete)); verbPending = false;
            }
    else if (match ("COMMIT"))
            {
            options.append (makeNode((before) ? nod_pre_commit :
    nod_post_commit)); verbPending = false;
            }
    else if (verbPending)
            error ("INSERT | UPDATE | DELETE");
    else if (match ("BEFORE"))
...
Syntax *SQLParse::parseCreateFilterSet(SyntaxType nodeType) {
  ...
  if (!match("(")) error("parenthesised filter list");
...
Syntax *SQLParse::parseAddress() {
  ...
  if (match("*")) {
    ...
    if (!match(".")) error("dot after node wildcard");
  }
...
Syntax *SQLParse::parseRepair() {
  ...
  if (match("WHERE")) node = parseBoolean();
...
Syntax *SQLParse::parseForeignKey(Syntax *constraint) {
  ...
  return makeNode(nod_foreign_key, constraint, identifier, names,
                  makeNode(options));
}
...
Syntax *SQLParse::parseCreateTableSpace(SyntaxType nodeType) {
  ...
  if (!match("FILENAME")) error("FILENAME");
