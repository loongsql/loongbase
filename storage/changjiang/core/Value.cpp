  ...
  switch (type) {
    ...
    case Varchar:
      if (data.string.length > bufferSize)
        throw SQLEXCEPTION(
            TRUNCATION_ERROR,
            "string truncation of %d bytes into %d bytes from \"%.*s\"",
            data.string.length, bufferSize, data.string.length,
            data.string.string);
    ...
    case Short:
      if (bufferSize < 6)
        throw SQLEXCEPTION(TRUNCATION_ERROR,
                           "string truncation during conversion");
...
double Value::getDouble() {
  switch (type) {
    ...
    case Timestamp:
      return (double)data.time.getSeconds() + (double)data.timestamp.getNanos();
...
int Value::compare(Value *value) {
  ...
  if (type == value->type) switch (type) {
      case Null:
        return 0;

      case Short:
        if (scale != value->scale) break;

        if (data.smallInt > value->data.smallInt) return 1;
      ...
      case Int32:
        ...
        if (data.integer > value->data.integer) return 1;
      ...
      case Double:
        if (data.dbl > value->data.dbl) return 1;
      ...
      case Int64:
        ...
        if (data.quad > value->data.quad) return 1;
...
short Value::getShort(int scl) {
  switch (type) {
    case Short:
      if (scale == scl) return data.smallInt;

      break;

    case Int32:
      if (scale == scl) return (short)data.integer;
  ...
  for (const char *p = string, *end = p + length; p < end;) {
    ...
    if (c >= '0' && c <= '9') {
      ...
      throw SQLEXCEPTION(CONVERSION_ERROR,
                         "error converting to numeric from '%*s'", length,
                         string);
  }
...
TimeStamp Value::getTimestamp() {
  if (type == Timestamp) return data.timestamp;
...
void Value::setNull() { clear(); }

bool Value::isNull(Type conversionType) {
  if (type == Null) return true;

  if (conversionType == Date) switch (type) {
      case Char:
      case String:
      case Varchar:
        if (data.string.length == 0) return true;
...
void Value::add(int value) { setValue((int)(getInt() + value)); }
...
int Value::convert(int64 value, int scale, char *string) {
  ...
  for (n = 0; number; number /= 10, --n) {
    if (scale && scale == digits++) *p++ = '.';

    *p++ = '0' + (char)(number % 10);
  }

  if (scale && digits <= scale) {
    for (; digits < scale; ++digits) *p++ = '0';
...
int64 Value::reScale(int64 number, int from, int to) {
  ...
  if (delta > 0)
    for (int n = 0; n < delta; ++n) number *= 10;
  else if (delta < 0)
    for (int n = 0; n > delta; --n) number /= 10;
...
double Value::scaleDouble(double d, int scale) {
  ...
  ***/
  ...
  if (scale > 0) {
    for (; n > 10; n -= 10) d /= powersOfTen[10];
...
int Value::getNanos(void) {
  if (type == Timestamp) return data.timestamp.getNanos();

  return 0;
}

BigInt *Value::getBigInt(void) {
  if (type == Biginteger) return data.bigInt;
