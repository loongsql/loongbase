...
bool IndexKey::isEqual(IndexKey *indexKey) {
  if (indexKey->keyLength != keyLength) return false;
...
void IndexKey::appendRecordNumber(int32 recordNum) {
  ...
  for (int length = len; length > 2; --length) value = (value << 8) | *p++;
...
int IndexKey::compareValues(IndexKey *indexKey) {
  ...
  if (indexKey->recordNumber) len2 -= indexKey->getAppendedRecordNumberLength();
...
int IndexKey::compareValues(UCHAR *key2, uint len2, bool isPartial) {
  ...
  uint len1 = keyLength;
  if (recordNumber) len1 -= getAppendedRecordNumberLength();

  uint len = MIN(len1, len2);

  int ret = memcmp(key2, key, len);
  if (ret == 0 && !isPartial) ret = len2 - len1;
...
int32 IndexKey::getRecordNumber() { return recordNumber; }
