...
//#define STUFF(number,ptr,shift)	if (n = number >> shift) *ptr++ = n |
//0x80
...
int init() {
  if (!characters[(int)' ']) {
    ...
    for (p = white; *p;) characters[(int)*p++] |= WHITE;

    for (p = punctuation; *p;) characters[(int)*p++] |= PUNCTUATION;

    for (p = delimiters; *p;) characters[(int)*p++] |= DELIMITER;

    for (p = junk; *p;) characters[(int)*p++] |= JUNK;

    int n;

    for (n = 'A'; n <= 'Z'; ++n) characters[n] |= ALPHA;

    for (n = 'a'; n <= 'z'; ++n) characters[n] |= ALPHA;

    for (n = '0'; n <= '9'; ++n) characters[n] |= ALPHA;
  }
...
Search::Search(const char *string, SearchWords *words) {
  ...
  for (const char *ptr = string; getToken(&ptr, word);) switch (word[0]) {
      case '+':
        type = Required;
        break;
  ...
  switch (scan->type) {
    ...
    // case Phrase:
    case Tail:
      if (!scan->frequentWord) valid = true;
      break;
...
Search::~Search() {
  if (scans) {
    for (int n = 0; n < count; ++n) delete scans[n];
    delete[] scans;
  }
}
...
void Search::search(ResultList *resultList) {
  ...
  for (;;) {
      ...
      for (int n = 0; n < count; ++n) {
        ...
        if (scan->type == Required) {
          ...
          if (!required)
            ...
            if (ret) {
              if (ret > 0) required = scan->numbers;
              again = true;
            }
          }
          ...
          if (current) {
            if (required) {
              while (scan->compare(2, required) < 0)
                if (!scan->getNextValid()) break;
            }
            if (scan->compare(2, current) < 0) current = scan->numbers;
          } else
    ...
    if (eof || !current || !streamsActive) break;
      ...
      for (n = 0; n < count; ++n) {
        ...
        if (scan->type == Excluded) {
          ...
          while (scan->compare(2, current) < 0)
            if (!scan->getNextValid()) break;
          if (scan->eof) continue;
          if (excluded) {
            if (scan->compare(2, excluded->numbers) < 0) excluded = scan;
          } else
            excluded = scan;
        }
      }
    ...
    if (!reject && (!excluded || excluded->compare(2, current) != 0)) {
      ...
      if (hit) {
        for (n = 0; n < count; ++n) {
          Scan *scan = scans[n];
          if (scan->type != Excluded && scan->numbers[0] == current[0] &&
              scan->numbers[1] == current[1] && scan->numbers[2] == current[2])
            wordMask |= 1 << n;
        }
    ...
    for (int n = 0; n < count; ++n) {
      Scan *scan = scans[n];
      while (!scan->eof && !scan->frequentTail && scan->compare(4, temp) < 0)
        scan->getNextValid();
    }
  }
...
bool Search::getToken(const char **ptr, char *token) {
  ...
  for (;;) {
    ...
    *t++ = c;
    ...
    if (type & ALPHA)
      while ((c = *p) && !(characters[(int)c] & DELIMITER)) *t++ = *p++;
...
Scan *Search::parsePhrase(ScanType type, const char **ptr) {
  ...
  if (!getToken(ptr, word)) return NULL;
