...
//page->validateSpaceSlots(maxLine);
...
static Bdb *allocSectionPage(Dbb *dbb, int transId, int parentPage, int slot,
                             int sectionId, int sequence, int level) {
  Bdb *newBdb = dbb->allocPage(PAGE_sections, transId);

  if (!dbb->streamLog->recovering && !dbb->noLog)
    dbb->streamLog->logControl->sectionPage.append(dbb, transId, parentPage,
                                                   newBdb->pageNumber, slot,
                                                   sectionId, sequence, level);
  return newBdb;
}
...
Section::~Section() {
  if (reservedRecordNumbers) reservedRecordNumbers->release();

  if (freeLines) freeLines->release();
}
  ...
  for (int32 id = dbb->nextSection;; ++id) {
    ...
    if (sections->pages[slot] == 0) {
      if (dbb->sectionInUse(id)) {
        if (!sectionSkipped) sectionSkipped = id;
      } else {
        Bdb *sectionBdb = allocSectionPage(dbb, transId, sections->pageNumber,
                                           slot, id, 0, 0);
...
void Section::createSection(Dbb *dbb, int32 sectionId, TransId transId) {
  ...
  Bdb *sectionsBdb =
      getSectionPage(dbb, SECTION_ROOT, sequence, Exclusive, transId);
  BDB_HISTORY(sectionsBdb);
  SectionPage *sections = (SectionPage *)sectionsBdb->buffer;
  int slot = sectionId % dbb->pagesPerSection;

  if (sections->pages[slot] == 0) {
    // If we're in recovery and slot is empty, something went wrong , page must
    // have been logged and rebuilt already (every allocation is redone)
    ASSERT(!(dbb->streamLog && dbb->streamLog->recovering));

    sectionsBdb->mark(transId);
    Bdb *sectionBdb = allocSectionPage(dbb, transId, sectionsBdb->pageNumber,
                                       slot, sectionId, sequence, 0);
    BDB_HISTORY(sectionBdb);
...
Bdb *Section::getSectionPage(Dbb *dbb, int32 root, int32 sequence,
                             LockType requestedLockType, TransId transId) {
  LockType lockTypes[4];

  for (int n = 0; n < 4; ++n) lockTypes[n] = Shared;
  ...
  for (;;) {
    ...
    if (page->level == 0 && sequence == 0) return bdb;
    ...
    if (!decomposeSequence(dbb, sequence, level, slots)) {
      ...
      if (!dbb->streamLog->recovering && !dbb->noLog)
        dbb->streamLog->logControl->sectionPromotion.append(
            dbb, page->section, bdb->pageNumber, dbb->pageSize,
    ...
    for (;;) {
      ...
      if (pageNumber) {
        if (page->level == 1) {
          bdb = dbb->handoffPage(bdb, pageNumber, PAGE_sections,
                                 requestedLockType);
          BDB_HISTORY(bdb);
        ...
        Bdb *newBdb =
            allocSectionPage(dbb, transId, bdb->pageNumber, slot, page->section,
                             sequence, page->level - 1);
        BDB_HISTORY(newBdb);
        SectionPage *newPage = (SectionPage *)newBdb->buffer;
        ...
        if (newPage->level == 0) return newBdb;
...
int32 Section::insertStub(TransId transId) {
  ...
  for (int32 line = (freeLines) ? 0 : nextLine;; ++line) {
    ...
    if (reservedRecordNumbers && reservedRecordNumbers->isSet(line)) continue;
    ...
    if (bdb == NULL || indexSequence != segment) {
      ...
      if (pages->pages[indexSlot]) {
        bdb = dbb->handoffPage(bdb, pages->pages[indexSlot],
                               PAGE_record_locator, Exclusive);
        BDB_HISTORY(bdb);
      } else {
        ...
        if (!dbb->streamLog->recovering && !dbb->noLog)
          dbb->streamLog->logControl->recordLocator.append(
              dbb, transId, sectionId, page->sequence, bdb->pageNumber);
      }
    ...
    if (!index->page && !index->line) {
      // OK, reserve the slot by setting the line leaving the page number as
      // zero
      ...
      // We have our line.  Find the next potential line, and if it isn't in
      // this section, mark this section as full.
      ...
      if (reservedRecordNumbers || freeLines) {
        ...
        if (nextSequence > sequence) markFull(true, sequence, transId);

        if (!freeLines) nextLine = line + 1;
      } else {
        nextLine = line + 1;

        if (nextLine % linesPerSection == 0) markFull(true, sequence, transId);
      }
...
void Section::updateRecord(int32 recordNumber, Stream *stream, TransId transId,
                           bool earlyWrite) {
#ifdef STOP_SECTION
  if (sectionId == stopSection)
    Log::debug("UpdateRecord %d in section %d/%d\n", recordNumber, sectionId,
               dbb->tableSpaceId);
#endif

  // Since different porpoise threads are processing different
  ...
  if (!dbb->streamLog->recovering && table &&
      !table->validateUpdate(recordNumber, transId))
    return;
  ...
  if (!stream) reserveRecordNumber(recordNumber);
  ...
  if (index->page) {
    ...
    if (!stream) {
      if (earlyWrite)
        dbb->streamLog->logControl->blobDelete.append(
            dbb, bdb->pageNumber, line, index->page, index->line);

      if (line < locatorPage->maxLine) {
        int spaceAvailable = deleteLine(dataBdb, index->line, bdb->pageNumber,
                                        transId, locatorPage, line);
      ...
      if (isFull) markFull(false, sequence, transId);

      return;
    }

    int spaceAvailable =
        dataPage->updateRecord(this, index->line, stream, transId, earlyWrite);
    ...
    deleteLine(dataBdb, index->line, bdb->pageNumber, transId, locatorPage,
               line);
  } else if (!stream) {
  ...
  if (stream) {
    storeRecord(locatorPage, bdb->pageNumber, index, stream, transId,
                earlyWrite);
  }
...
Bdb *Section::fetchLocatorPage(int32 root, int32 recordNumber,
...
int Section::deleteLine(Bdb *bdb, int line, int32 sectionPageNumber,
                        TransId transId, RecordLocatorPage *locatorPage,
                        int locatorLine) {
  bdb->mark(transId);
  ...
  for (int n = 0; n < locatorPage->maxLine; ++n)
    if (n != locatorLine && locatorPage->elements[n].page == bdb->pageNumber) {
      bdb->release(REL_HISTORY);
      Log::debug(
          "Section::deleteLine -- locator page %d line %d points to empty data "
          "page %d:%d\n",
          sectionPageNumber, n, locatorPage->elements[n].page,
          locatorPage->elements[n].line);

      return spaceAvailable;
    }

  if (!dbb->streamLog->recovering && !dbb->noLog)
    dbb->streamLog->logControl->sectionLine.append(dbb, sectionPageNumber,
                                                   bdb->pageNumber);
...
void Section::storeRecord(RecordLocatorPage *recordLocatorPage,
                          int32 indexPageNumber, RecordIndex *index,
                          Stream *stream, TransId transId, bool earlyWrite) {
  ...
  if (length > maxRecord) {
    overflowPageNumber =
        storeTail(stream, maxRecord, &length, transId, earlyWrite);
  ...
  for (int slot = -1; (slot = recordLocatorPage->nextSpaceSlot(slot)) >= 0;) {
    ...
    if (recordLocator->spaceAvailable >= effectiveLength) {
      ...
      RecordIndex temp;
      int spaceAvailable =
          page->storeRecord(dbb, bdb, &temp, length, stream, overflowPageNumber,
                            transId, earlyWrite);
      VALIDATE_SPACE_SLOTS(recordLocatorPage, dbb->linesPerPage);

      if (spaceAvailable > 0) {
        recordLocatorPage->setIndexSlot(indexSlot, temp.page, temp.line,
                                        spaceAvailable);
        VALIDATE_SPACE_SLOTS(recordLocatorPage, dbb->linesPerPage);

        if (!dbb->streamLog->recovering && !dbb->noLog) {
          if (earlyWrite) {
            dbb->streamLog->logControl->largeBlob.append(
                dbb, transId, indexPageNumber, indexSlot, temp.page, temp.line);
            bdb->setWriter();
          } else
            dbb->streamLog->setPhysicalBlock(transId);
        }
  ...
  VALIDATE_SPACE_SLOTS(recordLocatorPage, dbb->linesPerPage);
  RecordIndex temp;
  int spaceAvailable = page->storeRecord(
      dbb, bdb, &temp, length, stream, overflowPageNumber, transId, earlyWrite);
  ...
  if (!dbb->streamLog->recovering && !dbb->noLog) {
    ...
    // the stream log because it will be written directly to disk
    // on commit without going through the stream log.
    ...
    if (earlyWrite) {
      ...
      dbb->streamLog->logControl->dataPage.append(
          dbb, transId, sectionId, indexPageNumber, bdb->pageNumber);
      dbb->streamLog->setPhysicalBlock(transId);
    }
  }
...
int Section::storeTail(Stream *stream, int maxRecord, int *pLength,
  ...
  while (length + (int)sizeof(int32) > maxRecord) {
    ...
    stream->getSegment(offset, overflowPage->length,
                       (char *)overflowPage + sizeof(DataOverflowPage));
...
int32 Section::findNextRecord(int32 pageNumber, int32 startingRecord,
                              Stream *stream) {
  ...
  if (locatorPage->pageType != PAGE_record_locator &&
      locatorPage->pageType != PAGE_sections) {
  ...
  if (locatorPage->pageType == PAGE_record_locator) {
    ...
    for (int slot = startingRecord % dbb->linesPerPage;
         slot < locatorPage->maxLine; ++slot)
      if (locatorPage->elements[slot].page) {
        if (stream) {
          ...
          try {
            ...
            Log::log(
                "Corrupted data page, section %d, slot %d, index page %d, "
                "corrupt page %d\n",
                sectionId, slot, bdb->pageNumber,
                locatorPage->elements[slot].page);
            Log::log("-%s\n", exception.getText());
          ...
          if (line >= dataPage->maxLine ||
              dataPage->lineIndex[line].offset == 0) {
  ...
  for (int n = 0; n < sectionPage->level; ++n) factor *= dbb->pagesPerSection;
...
void Section::deleteSection(Dbb *dbb, int32 sectionId, TransId transId) {
  ...
  if (!dbb->streamLog->recovering && !dbb->noLog)
    dbb->streamLog->logControl->sectionPage.append(
        dbb, transId, sectionsBdb->pageNumber, 0, slot, sections->section,
        sections->sequence, sections->level);

  sectionsBdb->release(REL_HISTORY);

  // Section is officially gone.  Now recover the pages

  if (pageNumber) deleteSectionLevel(dbb, pageNumber, transId);
}
...
void Section::deleteSectionLevel(Dbb *dbb, int32 pageNumber, TransId transId) {
  ...
  if (page->level > 0) {
    for (int n = 0; n < dbb->pagesPerSection; ++n) {
      if (!dbb->streamLog->recovering && !dbb->noLog)
        dbb->streamLog->logControl->sectionPage.append(
            dbb, transId, pageNumber, 0, n, page->section, page->sequence,
            page->level);

      if (page->pages[n]) deleteSectionLevel(dbb, page->pages[n], transId);
    }
  } else {
    for (int n = 0; n < dbb->pagesPerSection; ++n)
      if (page->pages[n]) {
        Bdb *recordLocatorBdb =
            dbb->fetchPage(page->pages[n], PAGE_record_locator, Exclusive);
        BDB_HISTORY(recordLocatorBdb);
        RecordLocatorPage *locatorPage =
            (RecordLocatorPage *)recordLocatorBdb->buffer;
...
int32 Section::getMaxPage(int32 root, Dbb *dbb) {
  ...
  for (int n = dbb->pagesPerSection - 1; n >= 0; --n)
    if (page->pages[n]) {
      ++n;
      for (int level = page->level; level; --level) n *= dbb->pagesPerSection;
      return n;
    }
...
void Section::validate(Dbb *dbb, Validation *validation, int sectionId,
                       int pageNumber) {
  ...
  if (bdb->buffer->pageType == PAGE_record_locator) {
    ...
    if (recordLocatorPage->section != sectionId)
      validation->error(
          "RecordLocatorPage %d, section %d, sequence %d has wrong section "
          "(%d)",
          pageNumber, sectionId, 0, recordLocatorPage->section);
    else if (recordLocatorPage->sequence != 0)
      validation->error(
          "RecordLocatorPage %d, section %d, sequence %d has wrong sequence "
          "(%d)",
          pageNumber, sectionId, 0, recordLocatorPage->sequence);
    else if (!recordLocatorPage->validate(dbb, validation, sectionId, 0,
                                          &dataPages) &&
             validation->isRepair()) {
      bdb->release(REL_HISTORY);
    ...
    if (sectionPage->section != sectionId)
      validation->error(
          "SectionPage %d, section %d, sequence %d has wrong section (%d)",
    ...
    /***
            ...
            if (sectionPage->level == 1)
                    for (int n = 0; n < dbb->pagesPerSection; ++n)
                            ...
                            if (pageNumber)
                                    {
                                    Bdb *bdb2 = dbb->fetchPage (pageNumber,
    PAGE_any, Shared); BDB_HISTORY(bdb2); validation->isPageType(bdb2,
    PAGE_sections, "Sequence Page, relative page %d", n);
...
bool Section::decomposeSequence(Dbb *dbb, int32 sequence, int level,
                                int *slots) {
...
void Section::expungeRecord(int32 recordNumber) {
  ...
  if (!freeLines) freeLines = new Bitmap;
...
void Section::redoSectionPage(Dbb *dbb, int32 parentPage, int32 pageNumber,
                              int slot, int sectionId, int sequence,
                              int level) {
...
int32 Section::getSectionRoot() {
  Bdb *bdb = getSectionPage(dbb, SECTION_ROOT, sectionId / dbb->pagesPerSection,
                            Shared, NO_TRANSACTION);
  BDB_HISTORY(bdb);
  SectionPage *sectionPage = (SectionPage *)bdb->buffer;
  int slot = sectionId % dbb->pagesPerSection;
...
void Section::redoRecordLocatorPage(int sequence, int32 pageNumber,
                                    bool isPostFlush) {
  Bdb *bdb = getSectionPage(sequence / dbb->pagesPerSection, Exclusive,
                            NO_TRANSACTION);
  BDB_HISTORY(bdb);
  SectionPage *sectionPage = (SectionPage *)bdb->buffer;
  ...
  if (pageNumber) {
    ...
    if (!isPostFlush) {
      Bdb *locatorBdb =
          dbb->trialFetch(pageNumber, PAGE_record_locator, Shared);
      BDB_HISTORY(locatorBdb);

      if (locatorBdb) {
        RecordLocatorPage *locatorPage =
            (RecordLocatorPage *)locatorBdb->buffer;
        ASSERT(locatorPage->section == sectionId || locatorPage->section == 0);

        if (locatorPage->section != sectionId ||
...
void Section::redoSectionPromotion(Dbb *dbb, int sectionId,
  ...
  if (page->section == sectionId &&
      page->level == ((SectionPage *)pageData)->level + 1) {
...
void Section::redoDataPage(int32 pageNumber, int32 locatorPageNumber) {
  ...
  BDB_HISTORY(bdb);
  DataPage *dataPage = (DataPage *)bdb->buffer;
  // memset(dataPage, 0, dbb->pageSize);			// page may have been dirtied
  // by partial read dataPage->pageType = PAGE_data;
  memset(&dataPage->maxLine, 0, dbb->pageSize - OFFSET(DataPage *, maxLine));

  Bdb *locatorBdb =
      dbb->fetchPage(locatorPageNumber, PAGE_record_locator, Shared);
  BDB_HISTORY(locatorBdb);
  RecordLocatorPage *locatorPage = (RecordLocatorPage *)locatorBdb->buffer;
  VALIDATE_SPACE_SLOTS(locatorPage, dbb->linesPerPage);
  ...
  for (int n = 0; n < locatorPage->maxLine; ++n)
    if (locatorPage->elements[n].page == pageNumber) {
      ...
      if (line >= dataPage->maxLine) dataPage->maxLine = line + 1;
...
Bdb *Section::getSectionPage(int sequence, LockType lockType, TransId transId) {
  ...
  if (pageNumber) {
    ...
    // There is a chance that the first page has been promoted upwards.  If so,
    // fix it.

    if (page->level == 0) return bdb;
...
void Section::redoSectionLine(Dbb *dbb, int32 pageNumber,
...
void Section::redoBlobUpdate(Dbb *dbb, int32 locatorPage, int locatorLine,
  ...
  if (page->maxLine <= locatorLine) page->maxLine = locatorLine + 1;

  page->setIndexSlot(locatorLine, dataPage, dataLine,
                     DATA_PAGE_MAX_AVAILABLE_SPACE(dbb->pageSize));
  bdb->release(REL_HISTORY);
}
...
void Section::redoBlobDelete(Dbb *dbb, int32 locatorPage, int locatorLine,
                             int32 dataPageNumber, int dataLine,
                             bool dataPageActive) {
  ...
  bdb->mark(NO_TRANSACTION);
  RecordLocatorPage *page = (RecordLocatorPage *)bdb->buffer;
  page->setIndexSlot(locatorLine, 0, 0,
                     DATA_PAGE_MAX_AVAILABLE_SPACE(dbb->pageSize));
  bdb->release(REL_HISTORY);
}
...
void Section::reserveRecordNumber(int32 recordNumber) {
  ...
  if (!reservedRecordNumbers) reservedRecordNumbers = new Bitmap;
