...
void Log::print(int mask, const char *text, void *arg) { printf("%s", text); }
...
void Log::logMessage(int mask, const char *text) {
  ...
  if ((mask & LogScrub) && symbols) {
    ...
    if (l > (int)sizeof(temp)) scrubbed = new char[l];
  ...
  if (inCS) LEAVE_CRITICAL_SECTION;

  if (scrubbed != temp) delete[] scrubbed;
}
...
bool Log::isActive(int mask) { return (mask & activeMask) != 0; }
