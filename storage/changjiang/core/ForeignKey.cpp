...
ForeignKey::~ForeignKey() {
  ...
  ***/
  ...
  if (primaryFields) delete[] primaryFields;

  if (foreignFields) delete[] foreignFields;

  if (primaryFieldIds) delete[] primaryFieldIds;

  if (foreignFieldIds) delete[] foreignFieldIds;
}
...
void ForeignKey::loadRow(Database *database, ResultSet *resultSet) {
  ...
  if (resultSet->wasNull()) deleteRule = importedKeyNoAction;
}
...
void ForeignKey::loadPrimaryKeys(Database *database, Table *table) {
  ...
  while (resultSet->next()) {
    ...
    if (!key || tableId != primaryTableId || position <= lastPosition) {
      if (key) table->addForeignKey(key);
      key = new ForeignKey;
...
bool ForeignKey::isMember(Field *field, bool foreign) {
  if (foreign) {
    for (int n = 0; n < numberFields; ++n)
      if (foreignFields[n] == field) return true;
  } else
    for (int n = 0; n < numberFields; ++n)
      if (primaryFields[n] == field) return true;

  return false;
}

bool ForeignKey::matches(ForeignKey *key, Database *database) {
  if (numberFields != key->numberFields) return false;
  ...
  ***/
  ...
  if (foreignTable != key->foreignTable || primaryTable != key->primaryTable)
    return false;

  for (int n = 0; n < numberFields; ++n) {
    if (primaryFields[n] != key->primaryFields[n]) return false;
    if (foreignFields[n] != key->foreignFields[n]) return false;
  }
...
void ForeignKey::setDeleteRule(int rule) { deleteRule = rule; }

void ForeignKey::cascadeDelete(Transaction *transaction, Record *oldRecord) {
  Connection *connection = transaction->connection;
  PreparedStatement *statement =
      connection->findRegisteredStatement(statementDeleteCascade, this);

  if (!statement) {
    if (deleteStatement.IsEmpty()) {
      deleteStatement.Format("delete from %s.%s where ",
  ...
  if (active) statement = connection->prepareStatement(deleteStatement);
