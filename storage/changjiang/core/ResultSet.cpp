...
ResultSet::ResultSet(Statement *stmt, NSelect *node, int count) {
  ...
  if (connection) connection->addResultSet(this);
}

ResultSet::ResultSet(int count) { init(count); }

ResultSet::ResultSet() { init(0); }
...
ResultSet::~ResultSet() {
  ...
  if (metaData) metaData->resultSetClosed();
}
...
int ResultSet::getInt(int id) { return getValue(id)->getInt(); }
...
short ResultSet::getShort(int id) { return getValue(id)->getShort(); }
...
int64 ResultSet::getLong(int id) { return getValue(id)->getQuad(); }
...
double ResultSet::getDouble(int id) { return getValue(id)->getDouble(); }
...
float ResultSet::getFloat(int id) { return (float)getValue(id)->getDouble(); }
...
ResultSetMetaData *ResultSet::getMetaData() {
  /***
  if (!statement)
          throw SQLEXCEPTION (RUNTIME_ERROR, "ResultSet's statement has been
  closed");
  ***/

  if (!metaData) metaData = new ResultSetMetaData(this);

  return metaData;
}

Field *ResultSet::getField(int index) {
  if (!select) throw SQLEXCEPTION(RUNTIME_ERROR, "ResultSet is not active");
...
Database *ResultSet::getDatabase() { return database; }

bool ResultSet::isMember(Table *table) { return select->isMember(table); }

bool ResultSet::isMember(Field *field) {
  for (int n = 0; n < numberColumns; ++n)
    if (select->getField(n) == field) return true;
...
void ResultSet::addRef() { INTERLOCKED_INCREMENT(useCount); }

void ResultSet::release() {
  ASSERT(useCount > 0);

  if (useCount == 1 && connection) clearConnection();
...
void ResultSet::releaseJavaRef() {
  ...
  if (javaCount == 1) {
    ...
    if (connection) {
      if (useCount == javaCount) clearConnection();
    } else
...
void ResultSet::clearStatement() {
  ...
  if (conversions) {
    for (int n = 0; n < numberColumns; ++n)
      if (conversions[n]) delete conversions[n];
    delete[] conversions;
    conversions = NULL;
  }
...
int ResultSet::findColumn(const char *name) {
  ...
  if (id < 0)
    throw SQLEXCEPTION(RUNTIME_ERROR, "column %s not defined in ResultSet",
                       name);
...
int ResultSet::getColumnIndex(const WCString *name) {
  ...
  if (n < 0)
    throw SQLEXCEPTION(RUNTIME_ERROR,
                       "invalid column name (\"%s\") for result set",
                       columnName);

  return n;
}

int ResultSet::getIndex(Field *field) { return select->getIndex(field) + 1; }

void ResultSet::clearConnection() {
  Sync sync(&database->syncResultSets, "ResultSet::clearConnection");
  sync.lock(Shared);

  if (connection) connection->deleteResultSet(this);

  connection = NULL;
}

void ResultSet::connectionClosed() {
  if (statement) clearStatement();
...
DateTime ResultSet::getDate(int id) { return getValue(id)->getDate(); }
  ...
  for (int n = 0; n < numberColumns; ++n) {
    ...
    switch (value->getType()) {
      ...
      case Quad: {
        ...
        // blob->putSegment (sizeof (int64), (const char*) &value->data.quad,
        // true);
      ...
      case Date: {
        ...
        blob->putSegment(sizeof(milliseconds), (const char *)&milliseconds,
                         true);
      } break;
      ...
      case ClobPtr: {
        ...
        if (data->isBlobReference()) {
          ...
          if (length > 0) blob->putSegment(data);
        } else {
          blob->putCharacter(terString);
...
void ResultSet::transactionEnded() { clearStatement(); }

Statement *ResultSet::getStatement() {
  if (!statement)
    throw SQLError(RUNTIME_ERROR,
                   "no statement currently associated with ResultSet");
...
void ResultSet::print(Stream *stream) {
  stream->format("%8x ResultSet uc %d, jc %d, jsc %d, stat %x, cs %x, sib %x\n",
                 this, useCount, javaCount, javaStatementCount, statement,
                 compiledStatement, sibling);
}

Time ResultSet::getTime(int id) { return getValue(id)->getTime(); }
