...
IndexNode::IndexNode(Btn *node) { parseNode(node); }
...
Btn *IndexNode::insert(Btn *where, int offst, int len, UCHAR *fullKey,
                       int32 recordNumber) {
  node = where;
  key = (UCHAR *)node;

  if ((offset = offst) >= 128) *key++ = (UCHAR)((offset >> 8) | 0x80);

  *key++ = (UCHAR)offset;

  if ((length = len) >= 128) *key++ = (UCHAR)((length >> 8) | 0x80);
  ...
  *key++ = (UCHAR)length;
  ...
  if (recordNumber > 0) {
    ...
    for (; shift > 0; shift -= 7) *key++ = (recordNumber >> shift) & 0x7f;
...
/**
@brief		Return the value of an appended record number starting at the
byte sent in.
**/
int32 IndexNode::getAppendedRecordNumber(const UCHAR *ptr) {
  ...
  for (int length = len1; length > 2; --length) number = (number << 8) | *ptr++;
