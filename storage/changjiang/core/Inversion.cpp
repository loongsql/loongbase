...
#define STUFF(number, ptr, shift) \
  if (n = number >> shift) *ptr++ = n | 0x80
...
Inversion::~Inversion() { delete[] sortBuffer; }

int32 Inversion::addInversion(InversionFilter *filter, TransId transId,
                              bool insertFlag) {
  ...
  for (int iteration = 0;; ++iteration) {
    ...
    if (page->parentPage)
      ...
      Bdb *parentBdb = Index2Page::createNewLevel(
          dbb, 1, INVERSION_VERSION_NUMBER, bdb->pageNumber,
...
Bdb *Inversion::findRoot(LockType lockType) {
  ...
  if (!header->inversion) return NULL;
...
void Inversion::propagateSplit(int level, IndexKey *indexKey, int32 pageNumber,
                               TransId transId) {
  for (int iteration = 0;; ++iteration) {
    ...
    if (page->parentPage) {
      ...
      Bdb *parentBdb = Index2Page::createNewLevel(
          dbb, level + 1, INVERSION_VERSION_NUMBER, bdb->pageNumber,
          splitBdb->pageNumber, &splitKey, transId);
      Index2Page::logIndexPage(parentBdb, transId);
      int32 pageNumber = parentBdb->pageNumber;
      page->parentPage = pageNumber;
...
void Inversion::sort(UCHAR **records, int size) {
  ...
  while (stack > 0) {
    ...
    ***/
    ...
    i = r + 1;
    for (;;) {
      while (compare(records[i], key) <= 0 && i < limit) ++i;
      while (compare(records[j], key) >= 0 && j > r) --j;
