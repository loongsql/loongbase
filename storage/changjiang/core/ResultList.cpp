...
SearchHit *ResultList::add(int32 tableId, int32 recordNumber, int32 fieldId,
                           double score) {
  if (tableFilter && !tableFilter->isSet(tableId)) return NULL;

  if (fieldFilter &&
      !(fieldFilter->id == fieldId && fieldFilter->table->tableId == tableId))
    return NULL;
...
void ResultList::sort() {
  ...
  for (SearchHit *hit = hits.next; hit != &hits; hit = hit->next)
    if (database->getTable(hit->tableId)) *ptr++ = hit;
  ...
  while (stack > 0) {
    ...
    for (;;) {
      while (records[i]->score >= key->score && i < limit) ++i;

      while (records[j]->score <= key->score && j > r) --j;
...
void ResultList::addRef() { INTERLOCKED_INCREMENT(useCount); }
...
void ResultList::print() {
  for (SearchHit *hit = hits.next; hit != &hits; hit = hit->next) {
    ...
    if (!table) database->getTable(hit->tableId);
    Log::debug("Table %s (%d) record %d score %f\n",
               (table) ? table->getName() : "???", hit->tableId,
...
bool ResultList::next() {
  if (nextHit->next == &hits) return false;
...
ResultSet *ResultList::fetchRecord() {
  if (!nextHit) throw SQLEXCEPTION(RUNTIME_ERROR, "no current search hit");
...
void ResultList::close() { delete this; }

int ResultList::getCount() { return count; }

double ResultList::getScore() {
  if (nextHit) return nextHit->score;
