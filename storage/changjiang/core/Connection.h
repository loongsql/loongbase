...
#define IS_REPEATABLE_READ(_level)          \
  (_level == TRANSACTION_WRITE_COMMITTED || \
   _level == TRANSACTION_CONSISTENT_READ)
#define IS_SERIALIZABLE(_level) (_level == TRANSACTION_SERIALIZABLE)

// TRANSACTION_WRITE_COMMITTED is a Changjiang name for InnoDB's version of
...
class Connection {
 ...
 public:
  ...
  virtual void updateSchedule(const char *application, const char *eventName,
                              const char *schedule);
  virtual void shutdown();
  virtual PreparedStatement *prepareStatement(const char *sqlString);
  virtual Statement *createStatement();
  virtual void openDatabase(const char *dbName, Parameters *parameters,
                            Threads *parent);
  virtual void createDatabase(const char *dbName, Parameters *parameters,
                              Threads *parent);
  virtual void close();
  ...
  virtual void genNativeMethod(const char *className, const char *fileName,
                               GenOption option);
  ...
  void formatDateString(const char *format, int length, int32 date,
                        char *buffer);
  ...
  void registerStatement(StatementType typ, void *own,
                         PreparedStatement *statement);
  ...
  int64 getSequenceValue(const WCString *schemaName,
  ...
  Database *createDatabase(const char *dbName, const char *dbFileName,
                           const char *account, const char *password,
                           Threads *threads);
  Database *getDatabase(const char *dbName, const char *filename,
                        Threads *threads);
  void openDatabase(const char *dbName, const char *filename,
                    const char *account, const char *password,
  ...
  bool enableFilterSet(const WCString *schemaName,
                       const WCString *filterSetName);
  void disableFilterSet(FilterSet *filterSet);
  void disableFilterSet(const WCString *schemaName,
                        const WCString *filterSetName);
