...
int Stream::getSegment(int offset, int len, void *ptr) {
  ...
  for (Segment *segment = segments; segment;
       n += segment->length, segment = segment->next)
...
void Stream::setMinSegment(int length) { minSegment = length; }
  ...
  for (Segment *segment = segments; segment;
    ...
    if (n + segment->length >= offset) {
      ...
      for (char *end = p + l; p < end;) {
        char c = *address++ = *p++;
        --length;
        if (c == delimiter) return len - length;
      }
...
void Stream::putSegment(const char *string) {
  if (string[0]) putSegment((int)strlen(string), string, true);
}
...
char *Stream::decompress(int tableId, int recordNumber) {
  ...
  for (Segment *segment = segments; segment; segment = segment->next) {
    ...
    if (decompressedLength == 0) {
      ...
      if (decompressedLength <= 0) {
        Log::log(
            "corrupted record, table %d, record %d, decompressed length %d\n",
    ...
    while (p < end) {
      ...
      if (n == 0 && run == 0) {
        Log::log("corrupted record (zero run), table %d, record %d\n", tableId,
                 recordNumber);
        printShorts("Zero run", (segment->length + 1) / 2,
        ...
        *q++ = n;
        ...
        if (q + run > limit) {
          ...
          Log::log("corrupted record (overrun), table %d, record %d\n", tableId,
                   recordNumber);
          printShorts("Compressed", (segment->length + 1) / 2,
                      (short *)segment->address);
          printChars("Compressed", segment->length, segment->address);
          //#endif
          if (q == limit) return data;

          throw SQLEXCEPTION(RUNTIME_ERROR,
                             "corrupted record, table %d, record %d", tableId,
                             recordNumber);
        }
      }
    }
...
void Stream::clear() {
  ...
  if (useMalloc)
    while ((segment = segments)) {
      ...
      if (segment != &first) free(segment);
    }
  else
    while ((segment = segments)) {
      segments = segment->next;

      if (segment != &first) delete[](char *) segment;
    }
...
void Stream::putSegment(int length, const WCHAR *chars) {
  ...
  for (int n = 0; n < length; ++n) *p++ = (char)*wc++;
}

int Stream::getLength() { return totalLength; }
...
void Stream::putSegment(Stream *stream) {
  ...
  if (seg.remaining > 0) seg.copy(alloc(seg.remaining), seg.remaining);
#ifdef CHANGJIANGDB
...
void Stream::transfer(Stream *stream) {
  ...
  if (segment == &stream->first) {
    ...
    if (current && segment->next) current->next = segment->next;
...
void Stream::indent(int spaces) {
  for (int n = 0; n < spaces; ++n) putCharacter(' ');
}
