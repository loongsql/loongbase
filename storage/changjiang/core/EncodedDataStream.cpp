...
EncodedDataStream::EncodedDataStream() { stream = NULL; }

EncodedDataStream::EncodedDataStream(Stream *strm) { stream = strm; }
  ...
  switch (type) {
    ...
    case Varchar: {
      ...
      if (len <= edsUtf8Len39 - edsUtf8Len0)
        ...
        while (--count >= 0) stream->putCharacter(len >> (lengthShifts[count]));
      }

      if (len) stream->putSegment(len, data, true);
    ...
    case Int32: {
      ...
      while (--count >= 0) stream->putCharacter(val >> (lengthShifts[count]));
    } break;
    ...
    case Timestamp: {
      ...
      INT64 val = timestamp.getMilliseconds() * 1000 + timestamp.getNanos();
      int count = BYTE_COUNT64(val);
    ...
    case ClobPtr:
      ...
      case Asciiblob:
              ...
              while (--count >= 0)
                      stream->putCharacter((char) (val >> (lengthShifts
      [count]))); break;
              }
...
const UCHAR *EncodedDataStream::decode(const UCHAR *ptr, Value *value,
  ...
  switch (code) {
    ...
    case edsUtf8Count4: {
      ...
      for (int n = 0; n < count; ++n) length = length << 8 | *p++;
...
#ifdef _BIG_ENDIAN
      for (int n = 0; n < count; ++n) u.chars[n] = *p++;
#else
      for (int n = 0; n < count; ++n) u.chars[7 - n] = *p++;
#endif
    ...
    case edsClobLen4: {
      ...
      for (int n = 0; n < l; ++n) blobId = (blobId << 8) | *p++;
...
void EncodedDataStream::encodeAsciiBlob(Value *value) { unsupported(value); }

void EncodedDataStream::encodeBinaryBlob(Value *value) { unsupported(value); }
...
DataStreamType EncodedDataStream::decode() {
  ...
  switch (code) {
    ...
    case edsIntLen4: {
      ...
      for (int n = 0; n < l; ++n) val = (val << 8) | *ptr++;
    ...
    case edsUtf8Count4: {
      ...
      for (int n = 0; n < count; ++n) length = length << 8 | *ptr++;
...
#ifdef _BIG_ENDIAN
      for (int n = 0; n < count; ++n) u.chars[n] = *ptr++;
#else
      for (int n = 0; n < count; ++n) u.chars[7 - n] = *ptr++;
#endif
    ...
    case edsClobLen4: {
      ...
      for (int n = 0; n < l; ++n) blobId = (blobId << 8) | *ptr++;
...
void EncodedDataStream::skip() { ptr = skip(ptr); }
...
void EncodedDataStream::encodeNull() { stream->putCharacter(edsNull); }
...
void EncodedDataStream::encodeUtf8(int length, const char *string) {
  ...
  if (len) stream->putSegment(len, string, true);
}
...
void EncodedDataStream::setStream(Stream *newStream) { stream = newStream; }
...
void EncodedDataStream::encodeDouble(double dbl) {
  ...
  while (count && u.chars[count - 1] == 0) count--;

  stream->putCharacter(edsDoubleLen0 + count);

  for (int n = 0; n < count; n++) stream->putCharacter(u.chars[n]);
#else
  int count = 0;

  while (count < 8 && u.chars[count] == 0) ++count;

  stream->putCharacter(edsDoubleLen0 + 8 - count);

  for (int n = 7; n >= count; --n) stream->putCharacter(u.chars[n]);
#endif
}
