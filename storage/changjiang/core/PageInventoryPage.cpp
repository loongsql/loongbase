...
void PageInventoryPage::create(Dbb *dbb, TransId transId) {
  ...
  space->freePages[0] &= ~(MASK(PIP_PAGE) | MASK(SECTION_ROOT) |
                           MASK(HEADER_PAGE) | MASK(INDEX_ROOT));
  bdb->release(REL_HISTORY);
}

Bdb *PageInventoryPage::createInventoryPage(Dbb *dbb, int32 pageNumber,
                                            TransId transId) {
  Bdb *bdb = dbb->fakePage(pageNumber, PAGE_inventory, transId);
  BDB_HISTORY(bdb);
  PageInventoryPage *page = (PageInventoryPage *)bdb->buffer;

  for (int n = 0; n < dbb->pipSlots; ++n) page->freePages[n] = -1;

  if (dbb->database->streamLog && !dbb->database->streamLog->recovering &&
      (pageNumber != PIP_PAGE))
    dbb->database->streamLog->logControl->inventoryPage.append(dbb, pageNumber);

  return bdb;
}

Bdb *PageInventoryPage::allocPage(Dbb *dbb, PageType pageType,
                                  TransId transId) {
  StreamLog *streamLog = dbb->database->streamLog;
  ASSERT(!(streamLog->recovering && (streamLog->recoveryPhase == 2)));
        ...
        for (int n = 0; n < PIP_BITS; ++n)
          if (mask & MASK(n)) {
            ...
            if (n + slot * PIP_BITS == dbb->pagesPerPip - 1)
              createInventoryPage(dbb, pageNumber, transId)
                  ->release(REL_HISTORY);
            else {
              bdb->release(REL_HISTORY);
...
void PageInventoryPage::markPageInUse(Dbb *dbb, int32 pageNumber,
                                      TransId transId) {
  int32 pip = pageNumber / dbb->pagesPerPip;
  int32 n = pageNumber % dbb->pagesPerPip;
          ...
          if (bdb) {
            ...
            switch (page->pageType) {
              case PAGE_record_locator:
                validation->warning(
                    "orphan section index page %d/%d, section %d, seq %d",
              ...
              case PAGE_btree: {
                ...
                validation->warning(
                    "orphan index page %d/%d, level %d, next %d", pageNumber,
                    tableSpaceId, ipg->level, ipg->nextPage);
              } break;

              case PAGE_data: {
                DataPage *pg = (DataPage *)page;
                validation->warning("orphan data page %d/%d, maxLine %d",
                                    pageNumber, tableSpaceId, pg->maxLine);
              } break;

              case PAGE_data_overflow: {
                DataOverflowPage *pg = (DataOverflowPage *)page;
                validation->warning(
                    "orphan data overflow page %d/%d, section=%d, next=%d",
                    pageNumber, tableSpaceId, pg->section, pg->nextPage);
              } break;

              case PAGE_sections: {
                SectionPage *pg = (SectionPage *)page;
                validation->warning(
                    "orphan section page %d/%d, section=%d, seq=%d, level=%d, "
                    "flgs=%d",
                    pageNumber, tableSpaceId, pg->section, pg->level,
                    pg->flags);
              } break;
              ...
              default:
                validation->warning("orphan page %d/%d, type %d", pageNumber,
                                    tableSpaceId, page->pageType);
            }
          } else
            validation->warning("possible unwritten orphan page %d/%d",
                                pageNumber, tableSpaceId);
...
int32 PageInventoryPage::getLastPage(Dbb *dbb) {
  for (int32 pip = dbb->lastPageAllocated / dbb->pagesPerPip;; ++pip) {
    Bdb *bdb =
        dbb->trialFetch((pip == 0) ? PIP_PAGE : pip * dbb->pagesPerPip - 1,
    ...
    if (!bdb) {
      ...
      if (pip == 0)
        throw SQLError(DATABASE_CORRUPTION,
                       "Transaction inventory page not found");

      bdb = dbb->trialFetch(pip * dbb->pagesPerPip - 1, PAGE_inventory,
                            Exclusive);
      BDB_HISTORY(bdb);
      ...
      if ((mask = page->freePages[slot]) != -1)
        for (int n = PIP_BITS - 1; n >= 0; --n)
          if (!(mask & MASK(n))) {
            ...
            if (n + slot * PIP_BITS == dbb->pagesPerPip - 1) goto nextPip;
...
void PageInventoryPage::reallocPage(Dbb *dbb, int32 pageNumber) {
  ...
  if (!bdb) bdb = createInventoryPage(dbb, pip, NO_TRANSACTION);
...
void PageInventoryPage::analyzePages(Dbb *dbb, PagesAnalysis *pagesAnalysis) {
  for (int32 pip = 0;; ++pip) {
    Bdb *bdb =
        dbb->fetchPage((pip == 0) ? PIP_PAGE : pip * dbb->pagesPerPip - 1,
                       PAGE_inventory, Shared);
    BDB_HISTORY(bdb);
    PageInventoryPage *page = (PageInventoryPage *)bdb->buffer;
    ...
    for (int slot = 0; slot < dbb->pipSlots; ++slot) {
      ...
      if (maxPage > pagesAnalysis->maxPage) pagesAnalysis->maxPage = maxPage;
    }

    bdb->release(REL_HISTORY);

    if (clump & MASK(PIP_BITS - 1)) break;
  }
}
