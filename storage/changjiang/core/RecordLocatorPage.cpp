...
bool RecordLocatorPage::validate(Dbb *dbb, Validation *validation,
                                 int sectionId, int sequence,
                                 Bitmap *dataPages) {
  bool ok = true;
  ...
  for (int n = 0; n < maxLine; ++n) {
    ...
    if (!index->page && index->line) {
      if (validation->minutia())
        validation->error(
            "Abandoned list slot, section %d, sequence %d, line %d", sectionId,
            sequence, n);
      ok = false;
    }
    ...
    if (index->page) {
      ...
      if (validation->isPageType(bdb, PAGE_data,
                                 "DataPage, section %d, sequence %d", sectionId,
                                 sequence)) {
        ...
        if (index->line >= dataPage->maxLine)
          validation->error(
              "Missing data record, section %d, sequence %d, line %d, data "
              "page %d",
...
void RecordLocatorPage::repair(Dbb *dbb, int sectionId, int sequence) {
  for (int n = 0; n < maxLine; ++n) {
    ...
    if (!index->page && index->line) index->line = 0;
    ...
    if (index->page) {
      ...
      if (dataPage->pageType == PAGE_data) {
        ...
        Log::log(
            "RecordLocatorPage::repair: dropping DataPage %d from section %d, "
            "sequence %d\n",
...
void RecordLocatorPage::analyze(Dbb *dbb, SectionAnalysis *analysis,
                                int sectionId, int Sequence,
                                Bitmap *dataPages) {
  for (int n = 0; n < maxLine; ++n) {
    RecordIndex *index = elements + n;
...
int RecordLocatorPage::nextSpaceSlot(int priorSlot) {
  for (int slot = priorSlot + 1; slot < maxLine; ++slot) {
    if (elements[slot].spaceAvailable > 0) return slot;
...
int RecordLocatorPage::findSpaceSlot(int32 pageNumber) {
  ...
  for (slot = -1; (slot = nextSpaceSlot(slot)) >= 0;)
    if (elements[slot].page == pageNumber) return slot;
...
void RecordLocatorPage::printPage(void) {
  ...
  for (int n = 0; n < maxLine; ++n, ++element)
    Log::debug("%d.  page %d, line %d, space %d\n", n, element->page,
               element->line, element->spaceAvailable);
}

void RecordLocatorPage::setIndexSlot(int slot, int32 pageNumber, int line,
                                     int availableSpace) {
  ...
  if (pageNumber == 0) {
    if (element->page && element->spaceAvailable > 0) {
      ...
      if (spaceSlot >= 0) elements[spaceSlot].spaceAvailable = availableSpace;
    }
...
void RecordLocatorPage::insertSpaceSlot(int slot, int availableSpace) {
  ...
  for (priorSlot = -1;
       (nextSlot = nextSpaceSlot(priorSlot)) >= 0 && nextSlot < slot;
       priorSlot = nextSlot)
    ;

  elements[slot].spaceAvailable = availableSpace;

  if (slot > 0) linkSpaceSlot(priorSlot, slot);
...
void RecordLocatorPage::expungeDataPage(int32 pageNumber) {
  ...
  for (int slot = 0; slot < maxLine; ++slot, ++element)
    if (element->page == pageNumber) {
      if (element->spaceAvailable > 0) unlinkSpaceSlot(slot);
