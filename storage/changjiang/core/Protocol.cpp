...
MsgType Protocol::getMsg() { return (MsgType)getLong(); }
...
void Protocol::sendSuccess() { putMsg(Success); }
...
void Protocol::putValue(Database *database, Value *value) {
  ...
  switch (type) {
    ...
    case Short: {
      ...
      if (protocolVersion >= PROTOCOL_VERSION_3) putShort(scale);
    ...
    case BlobPtr: {
      ...
      if (protocolVersion >= PROTOCOL_VERSION_11) {
        ...
        if (length > 0) putBytes(length, temp);
      }
      ...
      if (length > 0)
        for (Segment *segment = blob->getStream()->segments; segment;
    ...
    case ClobPtr: {
      ...
      if (length > 0)
        for (Segment *segment = clob->getStream()->segments; segment;
             segment = segment->next)
          putBytes(segment->length, segment->address);
    ...
    case Biginteger: {
      ...
      for (int n = 0; n < bigInt->length; ++n) putLong(bigInt->words[n]);
    } break;
...
void Protocol::getValue(Value *value) {
  ...
  switch (type) {
    ...
    case Short:
      if (protocolVersion >= PROTOCOL_VERSION_3) scale = getShort();
    ...
    case ClobPtr: {
      ...
      if (protocolVersion >= PROTOCOL_VERSION_11) getRepository(clob);
      int32 length = getLong();
      if (length >= 0) {
    ...
    case BlobPtr: {
      ...
      if (protocolVersion >= PROTOCOL_VERSION_11) getRepository(blob);
    ...
    case Biginteger: {
      ...
      for (int n = 0; n < bigInt->length; ++n) bigInt->words[n] = getLong();
    } break;
...
Protocol::Protocol() { protocolVersion = 0; }
...
void Protocol::putValues(Database *database, int count, Value *values) {
  ...
  for (int n = 0; n < count; ++n) putValue(database, values + n);
}
...
void Protocol::getRepository(BlobReference *blob) {
  ...
  if (length > (int)sizeof(temp)) {
    // Log::log ("Protocol::getRepository: invalid repository block (%d
    // bytes)\n", length);
    for (; length > 0; --length) getByte();
    return;
  }
