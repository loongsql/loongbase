...
int64 ScaledBinary::getInt64FromBinaryDecimal(const char *ptr, int precision,
                                              int scale) {
  int position = 0;
...
int ScaledBinary::numberBytes(int digits) {
  return (digits / DIGITS_PER_INT) * sizeof(decimal_digit_t) +
         bytesByDigit[digits % DIGITS_PER_INT];
}

void ScaledBinary::putBinaryDecimal(int64 number, char *ptr, int precision,
                                    int scale) {
  ...
  if (scale) putBinaryNumber(absNumber % powersOfTen[scale], scale, &p, true);

  // Handle sign

  if (number < 0)
    for (char *q = ptr; q < p; ++q) *q ^= -1;

  *ptr ^= 0x80;
}

int64 ScaledBinary::getBinaryNumber(int start, int digits, const char *ptr,
                                    bool isFraction) {
  int64 value = 0;
  int position = start;
  ...
  for (int n = 0; n < groups; ++n) {
    value =
        value * BASE + getBinaryGroup(position, sizeof(decimal_digit_t), ptr);
  ...
  if (partialBytes)
    value = value * powersOfTen[partialDigits] +
            getBinaryGroup(position, partialBytes, ptr);

  return value;
}

void ScaledBinary::putBinaryNumber(int64 number, int digits, char **ptr,
                                   bool isFraction) {
  int groups = digits / DIGITS_PER_INT;
  int partialDigits = digits % DIGITS_PER_INT;
  char *p = *ptr;
  ...
  for (int n = 0; n < groups; ++n) {
    ...
    if (index)
      putBinaryGroup((uint)(scaleInt64(number, index) % BASE), DIGITS_PER_INT,
                     &p);
    else
  ...
  if (partialDigits)
    putBinaryGroup((uint)(number % powersOfTen[partialDigits]), partialDigits,
                   &p);
...
void ScaledBinary::putBinaryGroup(uint number, int digits, char **ptr) {
  ...
  for (int n = (bytes - 1) * 8; n >= 0; n -= 8) *p++ = (char)(number >> n);

  *ptr = p;
}

void ScaledBinary::getBigIntFromBinaryDecimal(const char *ptr, int precision,
...
void ScaledBinary::getBigNumber(int start, int digits, const char *ptr,
  ...
  // Store remaining digits by collapsing 9-digit groups into int32, then
  // accumulate
...
void ScaledBinary::putBigInt(BigInt *bigInt, char *ptr, int precision,
                             int scale) {
  ...
  if (partialDigits)
    putBinaryGroup((number.length) ? number.divide(BASE) : 0, partialDigits,
                   &p);
  ...
  if (scale <= 9)
    ...
    uint32 partial =
        (uint32)bigFraction.divide((uint32)powersOfTen[partialDigits]);
    ...
    while (--n >= 0) putBinaryGroup(data[n], DIGITS_PER_INT, &p);

    // Write the trailing digits

    if (partialDigits) putBinaryGroup(partial, partialDigits, &p);
  }
...
int64 ScaledBinary::scaleInt64(int64 number, int delta) {
  ...
  if (n > 0) {
    for (; n > 9; n -= 9) value /= powersOfTen[9];

    if (n) value /= powersOfTen[n];
  } else if (n < 0) {
    for (; n < -9; n += 9) value *= powersOfTen[9];
