...
ResultSet *DatabaseMetaData::getTables(const char *catalog,
  ...
  if (types)
    for (const char **ptr = types; *ptr; ++ptr) ++count;
...
ResultSet *DatabaseMetaData::getTables(const char *catalog,
                                       const char *schemaPattern,
                                       const char *tableNamePattern,
                                       int typeCount, const char **types) {
...
ResultSet *DatabaseMetaData::getIndexInfo(const char *catalog,
                                          const char *schema,
                                          const char *tableName, bool b1,
                                          bool v2) {
  PreparedStatement *statement = connection->prepareStatement(
      "select '' as table_cat,\n"                                   // 1
      "\tschema as table_schem,\n"                                  // 2
...
ResultSet *DatabaseMetaData::getPrimaryKeys(const char *catalog,
                                            const char *schema,
...
ResultSet *DatabaseMetaData::getImportedKeys(const char *catalog,
                                             const char *schema,
                                             const char *tableName) {
  PreparedStatement *statement = connection->prepareStatement(
      "select \n"
...
ResultSet *DatabaseMetaData::getColumns(const char *catalog, const char *schema,
                                        const char *tableName,
  ...
  PreparedStatement *statement = connection->prepareStatement(
      ...
      ***/
      "\tcase\n"  // 6
      "when domainname is not null and domainname <> '' then schema || '.' || "
      "domainname\n"
      "\telse case dataType\n"  // 6
      "\t   when  1 then 'CHAR'\n"
      "\t   when  2 then 'CHAR'\n"
...
ResultSet *DatabaseMetaData::getUsers(const char *catalog,
...
ResultSet *DatabaseMetaData::getRoles(const char *catalog, const char *schema,
...
ResultSet *DatabaseMetaData::getUserRoles(const char *catalog,
                                          const char *schema,
                                          const char *rolePattern,
                                          const char *user) {
  JString sql =
  ...
  if (schema) statement->setString(n++, FUDGE_CASE(schema));

  if (rolePattern) statement->setString(n++, FUDGE_CASE(rolePattern));

  if (user) statement->setString(n++, FUDGE_CASE(user));
...
ResultSet *DatabaseMetaData::getObjectPrivileges(const char *catalog,
                                                 const char *schemaPattern,
  ...
  PreparedStatement *statement = connection->prepareStatement(
      ...
      "objectName like ? "
      "order by "
      "objectType,objectSchema,objectName,holderType,HolderSchema,holderName");
...
ResultSet *DatabaseMetaData::getSequences(const char *catalog,
                                          const char *schema,
                                          const char *sequence) {
  PreparedStatement *statement = connection->prepareStatement(
      "select\n"
...
ResultSet *DatabaseMetaData::getTriggers(const char *catalog,
...
ResultSet *DatabaseMetaData::getHolderPrivileges(const char *catalog,
                                                 const char *schemaPattern,
                                                 const char *namePattern,
                                                 int objectType) {
  PreparedStatement *statement = connection->prepareStatement(
...
ResultSet *DatabaseMetaData::getExportedKeys(const char *catalog,
                                             const char *schema,
                                             const char *tableName) {
  PreparedStatement *statement = connection->prepareStatement(
      "select \n"
...
const char *DatabaseMetaData::getDatabaseProductName() { return NETFRASERVER; }

const char *DatabaseMetaData::getDatabaseProductVersion() {
  if (databaseVersion.IsEmpty())
    databaseVersion.Format("%s ODS %d", VERSION_STRING,
                           connection->database->odsVersion);

  return databaseVersion;
}

ResultSet *DatabaseMetaData::getDomains(const char *catalog, const char *schema,
                                        const char *fieldName) {
  PreparedStatement *statement = connection->prepareStatement(
...
ResultSet *DatabaseMetaData::getRepositories(const char *catalog,
                                             const char *schema,
                                             const char *namePattern) {
  PreparedStatement *statement = connection->prepareStatement(
      "select\n"
