...
void PreparedStatement::setRecord(int length, const char *bytes) {
  ...
  *p++;
  ...
  if (count > statement->numberParameters)
    throw SQLEXCEPTION(
        RUNTIME_ERROR,
        "more fields (%d) in record than parameters in statement (%d):\n%s",
        count, statement->numberParameters, (const char *)statement->sqlString);

  for (int n = 1; n <= count; ++n) {
    if (p >= end) throw SQLEXCEPTION(RUNTIME_ERROR, "overflow in setRecord");
  ...
  if (p < end) throw SQLEXCEPTION(RUNTIME_ERROR, "underflow in setRecord");
}
...
bool PreparedStatement::isPreparedResultSet() { return true; }
