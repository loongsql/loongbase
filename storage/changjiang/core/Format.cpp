...
Format::Format(Table *tbl, int newVersion) {
  ...
  for (int n = 0; n < maxId; ++n) {
    ...
    if (ff->offset != 0) ff->nullPosition = index++;
  }
...
Format::Format(Table *tbl, ResultSet *resultSet) {
  ...
  for (n = 0; n < maxId; ++n) format[n].fieldId = -1;
...
bool Format::validate(Table *table) {
  FOR_FIELDS(field, table)
  if (field->id >= maxId) return false;

  int id = field->id;
  FieldFormat *ff = format + id;

  if (field->type != ff->type || field->getPhysicalLength() != ff->length)
    return false;
  END_FOR;
...
void Format::save(Table *table) {
  ...
  PreparedStatement *statement = database->prepareStatement(
      "insert Formats (version, tableId, fieldId, datatype, offset, length, "
      "scale, maxId) values (?,?,?,?,?,?,?,?)");
