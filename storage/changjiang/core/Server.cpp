...
Server::~Server() {
  ...
  if (acceptLogSocket) acceptLogSocket->close();
...
void Server::startAgent() {
  thread =
      parent->threads->startWhenever("Server::startAgent", agentLoop, this);
}
...
void Server::agentLoop() {
  ...
  if (parent) parent->agentShutdown(this);
...
void Server::next() {
  ...
  if (b) try {
      if (count > 20) values = new Value *[count];
      for (int n = 1; n <= count; ++n) {
        Value *value = values[n - 1] = resultSet->getValue(n);
        value->getStringLength();
      }
    } catch (...) {
      if (values != vector) delete[] values;
      throw;
    }
  ...
  for (int n = 0; n < count; ++n) protocol->putValue(database, values[n]);
...
void Server::genConnectionHtml() {
  ...
  try {
    ...
    /***
    for (int offset = 0, length; length = clob->getSegmentLength (offset);
    offset += length) protocol->putBytes (length, clob->getSegment (offset));
    ***/
...
void Server::getTables() {
  ...
  if (typeCount) {
    ...
    for (int n = 0; n < typeCount; ++n) types[n] = protocol->getString();
  }

  ResultSet *resultSet = metaData->getTables(string1, string2, string3,
                                             typeCount, (const char **)types);
  protocol->sendSuccess();
  sendResultSet(resultSet);

  if (typeCount) {
    for (int n = 0; n < typeCount; ++n) delete[] types[n];
    delete[] types;
  }
}
...
void Server::getColumns() {
  ...
  ResultSet *resultSet =
      metaData->getColumns(string1, string2, string3, string4);
  protocol->sendSuccess();
  sendResultSet(resultSet);
}
...
void Server::getIndexInfo() {
  ...
  ResultSet *resultSet =
      metaData->getIndexInfo(string1, string2, string3, unique, approximate);
  protocol->sendSuccess();
  sendResultSet(resultSet);
}
...
void Server::ping() { protocol->sendSuccess(); }
...
void Server::hasRole() {
  ...
  if (role) mask = connection->hasRole(role);
...
void Server::getObjectPrivileges() {
  ...
  ResultSet *resultSet =
      metaData->getObjectPrivileges(string1, string2, string3, objectType);
  protocol->sendSuccess();
  sendResultSet(resultSet);
}
...
void Server::getUserRoles() {
  ...
  ResultSet *resultSet =
      metaData->getUserRoles(string1, string2, string3, string4);
  protocol->sendSuccess();
  sendResultSet(resultSet);
}
...
void Server::getTriggers() {
  ...
  ResultSet *resultSet =
      metaData->getTriggers(string1, string2, string3, string4);
  protocol->sendSuccess();
  sendResultSet(resultSet);
}
...
void Server::addListener() {
  ...
  if (!logSocket) {
    ...
    parent->threads->start("Server::addListener", &Server::acceptLogConnection,
                           this);
...
void Server::getHolderPrivileges() {
  ...
  ResultSet *resultSet =
      metaData->getHolderPrivileges(string1, string2, string3, objectType);
  protocol->sendSuccess();
  sendResultSet(resultSet);
}
...
#ifdef LICENSE
  for (ScanDir dir(Registry::getInstallPath() + "/" + "licenses", "*.lic");
       dir.next();) {
...
void Server::deleteBlobData() {
  ...
  if (length > sizeof(repositoryInfo)) {
    for (uint n = 0; n < length; ++n) protocol->getByte();
  ...
  connection->deleteRepositoryBlob(string1, ref.repositoryName,
                                   ref.repositoryVolume, ref.blobId);
  protocol->sendSuccess();
}
...
bool Server::isActive() { return active; }
...
void Server::notInStorageEngine() {
  throw SQLError(FEATURE_NOT_YET_IMPLEMENTED,
                 "feature not implemented in storage engine");
}
