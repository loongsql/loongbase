...
void ResultSetMetaData::addRef() {
  ...
  if (resultSet) resultSet->addRef();
}

void ResultSetMetaData::addJavaRef() {
  INTERLOCKED_INCREMENT(useCount);

  if (resultSet) resultSet->addJavaRef();
}

void ResultSetMetaData::release() {
  ASSERT(useCount > 0);

  if (resultSet) resultSet->release();
...
void ResultSetMetaData::checkResultSet() {
  if (!resultSet)
    throw SQLEXCEPTION(RUNTIME_ERROR,
                       "ResultSet for MetaDataResultSet has been closed");
}
...
const char *ResultSetMetaData::getCollationSequence(int index) {
  ...
  if (!field || !field->collation) return NULL;
...
const char *ResultSetMetaData::getDefaultValue(int index) {
  ...
  if (field->defaultValue) return field->defaultValue->getString();
...
const char *ResultSetMetaData::getRepositoryName(int index) {
  ...
  if (!field || !field->repository) return NULL;
