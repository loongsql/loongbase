...
void Synchronize::sleep() {
  ...
  while (!wakeup) pthread_cond_wait(&condition, &mutex);
...
bool Synchronize::sleep(int milliseconds) { return sleep(milliseconds, NULL); }

bool Synchronize::sleep(int milliseconds, Mutex *callersMutex) {
  sleeping = true;

#ifdef _WIN32
  if (callersMutex) callersMutex->release();

  int n = WaitForSingleObject(event, milliseconds);

  if (callersMutex) callersMutex->lock();
...
#ifdef CHANGJIANGDB
      Log::debug(
          "Synchronize::sleep(milliseconds): unexpected wakeup, ret %d\n", ret);
#else
