...
Schedule::Schedule(const char *scheduleString) {
  ...
  for (int n = 0; *p && n < SCHEDULE_ELEMENTS; ++n) {
    ...
    if (*p == '*')
      ...
      for (ScheduleElement *element = elements[n]; element;
           element = element->next)
        if (element->from >= max || element->to >= max)
          throw SQLEXCEPTION(RUNTIME_ERROR, "Invalid schedule string \"%s\"",
                             (const char *)string);
    } else
      throw SQLEXCEPTION(RUNTIME_ERROR, "Invalid schedule string \"%s\"",
                         (const char *)string);
...
void Schedule::getNextEvent() {
  ...
  for (int cycles = 0;; ++cycles) {
    ...
    if (bump >= 0) {
      ...
      for (int n = 0; n < bump; ++n) values[n] = 0;
...
void Schedule::addRef() { INTERLOCKED_INCREMENT(useCount); }
