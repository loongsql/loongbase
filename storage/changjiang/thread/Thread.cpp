...
Thread::Thread(const char *desc) { init(desc); }

// Thread::Thread(const char *desc, Threads *threads, void (*fn)(void*), void
// *arg, Threads *barn)
...
Thread::~Thread() {
  ...
  if (threadBarn) setThreadBarn(NULL);
  ...
  try {
    while (!shutdownInProgress) {
      ...
      if (function) {
        ...
        if (!shutdownInProgress && threadBarn) threadBarn->inactive(this);
      }
...
#ifdef _WIN32
  if (!thread->threadId) thread->threadId = GetCurrentThreadId();
#endif

#ifdef _PTHREADS
  if (!thread->threadId) thread->threadId = pthread_self();
#endif
...
void Thread::createThread(void (*fn)(void *), void *arg) {
  ...
  ***/
  ...
  if (!(threadHandle = CreateThread(NULL, 0, thread, this, 0, &threadId))) {
    ...
    if (threadBarn) threadBarn->print();
...
void Thread::addRef() { INTERLOCKED_INCREMENT(useCount); }
...
void Thread::findLocks(LinkedList &threads, LinkedList &syncObjects) {
  if (threads.appendUnique(this)) {
    ...
    if (lockPending) lockPending->findLocks(threads, syncObjects);
  }
}
...
void Thread::print() {
  ...
  LOG_DEBUG(
      "  Thread %p (%d) sleep=%d, grant=%d, locks=%d, who %d, parent=%p\n",
      this, threadId, sleeping, lockGranted, activeLocks, lockGranted,
      threadBarn);
#else
  LOG_DEBUG(
      "  Thread %p (%d) sleep=%d, grant=%d, locks=%d, who %d, javaThread %p\n"
      "      prior=%p, next=%p, parent=%p\n",
      this, threadId, sleeping, lockGranted, activeLocks, lockGranted,
      javaThread, prior, next, threadBarn);
#endif

  for (Sync *sync = locks; sync; sync = sync->prior) sync->print("    Holding");

  if (lockPending) lockPending->print("    Pending");
}
...
{
        marked = true;
        LOG_DEBUG ("%*sThread %d (%x) sleeping=%d, granted=%d, locks=%d, who
%d\n", level * 2, "", threadId, this, sleeping, lockGranted, activeLocks,
lockGranted); Sync *sync;
        ++level;
...
void Thread::print(const char *label) {
  LOG_DEBUG("%s %x (%d), type %d; %s\n", label, this, threadId, lockType,
            getWhere());
}

const char *Thread::getWhere() {
  if (lockPending && lockPending->location) return lockPending->location;
...
void Thread::setThreadBarn(Threads *newBarn) {
  if (newBarn) newBarn->checkInactive(this);

  if (newBarn == threadBarn) return;
...
void Thread::lockExitMutex(void) { exitMutex.lock(); }

void Thread::unlockExitMutex(void) { exitMutex.unlock(); }
