...
static const char *ddl[] = {
    ...
    "grant select on Schedule to public", NULL};
...
void Scheduler::schedule() {
  ...
  for (;;) {
    ...
    while (next && next->eventTime <= currentTime) {
      ...
      next = schedule->next;
      sync.unlock();
      if (!schedule->deleted) schedule->execute(this);
...
#ifdef _PTHREADS
  prctl(PR_SET_NAME, "cj_scheduler");
#endif
...
void Scheduler::addEvent(Schedule *schedule) {
  ...
  *ptr = schedule;
  ...
  if (thread && !shutdownInProgress) thread->wake();
}

void Scheduler::start() {
  if (!thread)
    thread = database->threads->start("Scheduler::start", &Scheduler::schedule,
                                      this);

  if (!database->findTable("SYSTEM", "SCHEDULE")) initialize();
  ...
  while (resultSet->next()) {
    ...
    if (!application) {
      Log::debug("No application \"%s\" for schedule event \"%s\"", appName,
...
void Scheduler::updateSchedule(const char *appName, const char *eventName,
                               User *user, const char *schedule) {
  ...
  PreparedStatement *statement = database->prepareStatement(
      "replace into system.schedule (application,eventName,userName,schedule) "
      "values (?,?,?,?)");
  int n = 1;
...
void Scheduler::loadEvents(Application *application) {
  ...
  while (resultSet->next()) {
    ...
    if (!user) {
      Log::debug("No user \"%s\" for schedule event \"%s.%s\"", userName,
                 appName, eventName);
      continue;
    }
    ...
    try {
      AppEvent *event =
          new AppEvent(database, application, eventName, user, schedule);
      database->scheduler->addEvent(event);
      event->nextEvent = events;
      events = event;
...
void Scheduler::addRef() { ++useCount; }
