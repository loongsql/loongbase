...
void MemFreeBlock::rotateLeft(void) {
  ...
  if ((larger = larger->smaller)) larger->parent = this;
...
void MemFreeBlock::rotateRight(void) {
  ...
  if ((smaller = smaller->larger)) smaller->parent = this;
...
void MemFreeBlock::remove() {
  ...
  if (nextTwin != this) {
    ...
    if ((twin->smaller = smaller)) smaller->parent = twin;

    if ((twin->larger = larger)) larger->parent = twin;
  ...
  // larger value) and swap it into our place

  bool shallower;
  MemFreeBlock *node = larger->getSuccessor(&larger, &shallower);

  if ((node->smaller = smaller)) smaller->parent = node;

  if ((node->larger = larger)) larger->parent = node;
...
void MemFreeBlock::rebalance(void) {
  if (balance > 1) {
    if (larger->balance < 0) larger->rotateRight();

    rotateLeft();
  } else if (balance < 1) {
    if (smaller->balance > 0) smaller->rotateLeft();
...
MemFreeBlock *MemFreeBlock::getSuccessor(MemFreeBlock **parentPointer,
                                         bool *shallower) {
  ...
  if (larger) larger->parent = parent;
...
MemFreeBlock *MemFreeBlock::getFirst(void) {
  ...
  while (node->smaller) node = node->smaller;
...
MemFreeBlock *MemFreeBlock::getNext(void) {
  ...
  for (node = this; node && node->parent; node = node->parent)
    if (node == parent->smaller) return parent;
...
int MemFreeBlock::validate(void) {
  ...
  if (smaller) {
    if (smaller->memHeader.length >= memHeader.length) corrupt("out of order");

    if (smaller->parent != this) corrupt("bad parent");

    leftDepth = smaller->validate();
  }

  if (larger) {
    if (larger->memHeader.length < memHeader.length) corrupt("out of order");

    if (larger->parent != this) corrupt("bad right parent");
...
void MemFreeBlock::corrupt(const char *text) { ASSERT(false); }
...
int MemFreeBlock::count(void) {
  ...
  if (smaller) count += smaller->count();

  if (larger) count += larger->count();
