...
static MemMgr memoryManager(MemMgrGeneral, defaultRounding, FREE_OBJECTS_SIZE,
                            HEAP_SIZE, &memoryManagerAlive, &memControlGeneral);

// Record data
static MemMgr recordDataManager(MemMgrRecordData, defaultRounding, 2, HEAP_SIZE,
                                NULL, &memControlRecord);

// Record metadata: Record object and RecordVersion object
static MemMgr recordManager(MemMgrRecord, defaultRounding,
                            sizeof(RecordVersion) + 100, HEAP_SIZE, NULL,
                            &memControlRecord);
static MemMgr recordVersionManager(MemMgrRecordVersion, defaultRounding,
                                   sizeof(RecordVersion) + 100, HEAP_SIZE, NULL,
                                   &memControlRecord);
...
#ifdef _DEBUG
void *MemMgrPoolAllocateDebug(MemMgr *pool, size_t s, const char *file,
                              int line) {
  void *object = pool->allocateDebug(s, file, line);

  if (object == stopAddress) printf("MemMgrAllocateDebug at %p\n", stopAddress);

  if (traceFile) fprintf(traceFile, "a " I64FORMAT " %p\n", (int64)s, object);

  return object;
}

void *MemMgrAllocateDebug(size_t s, const char *file, int line) {
  if (!memoryManagerAlive) return malloc(s);
...
void MemMgrRelease(void *object) {
  ...
  ***/
  ...
  if (traceFile) fprintf(traceFile, "r %p\n", object);
...
#else
void *MemMgrPoolAllocate(MemMgr *pool, size_t s) { return pool->allocate(s); }
...
void MemMgrRecordDelete(char *record) { recordDataManager.release(record); }
...
void MemMgrValidate(void *object) {
  if (object) MemMgr::validate(object);
}
...
MemMgr::MemMgr(int mgrId, int rounding, int cutoff, int minAlloc, bool *alive,
               MemControl *memCtrl)
    : id(mgrId),
  ...
  if (memControl) memControl->addPool(this);
...
MemMgr::MemMgr(void *arg1, void *arg2) : mutex("MemMgr::mutex") { MemMgr(); }
...
MemBlock *MemMgr::alloc(size_t s) {
  if (s > INT_MAX)
    throw SQLError(RUNTIME_ERROR,
                   "illegal memory allocate for " I64FORMAT " bytes", (int64)s);
  ...
  /*
   *  OK, we've got a "big block" on on hands.  To maximize confusing, the
   indicated
  ...
  if (length < (int)(OFFSET(MemBlock *, body) + sizeof(MemFreeBlock) -
                     sizeof(MemBigObject)))
    length = (int)(OFFSET(MemBlock *, body) + sizeof(MemFreeBlock) -
                   sizeof(MemBigObject));
  ...
  if (freeBlock) {
    ...
    // If there isn't room to split off a new free block, allocate the whole
    // thing
    ...
    if ((freeBlock->next = newBlock->next)) freeBlock->next->prior = freeBlock;
  ...
  if (hunkLength + (int)sizeof(MemBigObject) + threshold < minAllocation) {
    ...
    freeSpace =
        hunkLength - sizeof(MemBigHunk) - 2 * sizeof(MemBigHeader) - length;
  }
...
void *MemMgr::allocate(size_t size) {
  size_t length =
      ROUNDUP(size, roundingSize) + OFFSET(MemBlock *, body) + guardBytes;
  MemBlock *memory;
  ...
  memset(&memory->body + size, GUARD_BYTE,
         ABS(memory->length) - size - OFFSET(MemBlock *, body));
...
void *MemMgr::allocateDebug(size_t size, const char *fileName, int line) {
  ...
  ASSERT(l >= guardBytes &&
         l < length - size + guardBytes + (int)sizeof(MemFreeBlock));
...
void MemMgr::releaseBlock(MemBlock *block) {
  ...
  if (block->pool->signature != defaultSignature) corrupt("bad block released");

#ifdef MEM_DEBUG
  for (const UCHAR *end = (UCHAR *)block + ABS(block->length),
                   *p = end - guardBytes;
       p < end;)
    if (*p++ != GUARD_BYTE) corrupt("guard bytes overwritten");
#endif
  ...
  if (length < 0) {
    ...
    for (int slot = -length / roundingSize;;) {
      ...
      if (COMPARE_EXCHANGE_POINTER(freeObjects + slot, next, block)) return;
    }
  }
  ...
  MemFreeBlock *freeBlock =
      (MemFreeBlock *)((UCHAR *)block - sizeof(MemBigHeader));
  ...
  if (freeBlock->next && !freeBlock->next->memHeader.pool) {
    ...
    remove(next);
    freeBlock->memHeader.length +=
        next->memHeader.length + sizeof(MemBigHeader);

    if ((freeBlock->next = next->next)) freeBlock->next->prior = freeBlock;
  ...
  if (freeBlock->prior && !freeBlock->prior->memHeader.pool) {
    ...
    remove(prior);
    prior->memHeader.length +=
        freeBlock->memHeader.length + sizeof(MemBigHeader);

    if ((prior->next = freeBlock->next)) prior->next->prior = prior;
  ...
  if (freeBlock->prior == NULL && freeBlock->next == NULL) {
    for (MemBigHunk **ptr = &bigHunks, *hunk; (hunk = *ptr);
         ptr = &hunk->nextHunk)
...
void MemMgr::insert(MemFreeBlock *freeBlock) {
  ...
  ***/
  ...
  for (block = freeBlocks.nextLarger;
           block != &freeBlocks && freeBlock->memHeader.length >=
  block->memHeader.length; block = block->nextLarger) if
  (block->memHeader.length == freeBlock->memHeader.length)
                  {
...
void MemMgr::validateFreeList(void) {
  ...
  for (block = freeBlockTree.getFirst(); block; block = block->getNext()) {
    if (block->memHeader.length <= len) corrupt("bad free list\n");
    ...
    for (twin = block->nextTwin; twin != block;
         priorTwin = twin, twin = twin->nextTwin) {
      if (twin->priorTwin != priorTwin) corrupt("bad priorTwin pointer");
    ...
    if (twins) corrupt("bad twin list");
  }
...
void MemMgr::validateBigBlock(MemBigObject *block) {
  ...
  if ((neighbor = block->prior))
    if ((UCHAR *)&neighbor->memHeader + neighbor->memHeader.length !=
        (UCHAR *)block)
      corrupt("bad neighbors");

  if ((neighbor = block->next))
    if ((UCHAR *)&block->memHeader + block->memHeader.length !=
        (UCHAR *)neighbor)
      corrupt("bad neighbors");
}
...
{
  if (block) {
    if (*block) currentMemory -= (*block)->length;
...
void MemMgr::releaseDebug(void *object) { release(object); }

void MemMgr::validate() {}

void MemMgr::analyze(int mask, Stream *stream, InfoTable *summaryTable,
                     InfoTable *detailTable) {
#ifdef MEM_DEBUG
  Sync sync(&mutex, "MemMgr::analyze");

  if (summaryTable || detailTable) sync.lock(Exclusive);
  ...
  for (MemSmallHunk *smallHunk = smallHunks; smallHunk;
       smallHunk = smallHunk->nextHunk) {
    for (UCHAR *p = (UCHAR *)smallHunk + l; p < smallHunk->memory;) {
      ...
      for (client = hashTable[slot]; client; client = client->collision)
        if (client->fileName == file && client->line == line) break;
  ...
  for (MemBigHunk *bigHunk = bigHunks; bigHunk; bigHunk = bigHunk->nextHunk)
    for (MemBigObject *bigObject = (MemBigObject *)&bigHunk->blocks; bigObject;
         bigObject = bigObject->next) {
  ...
  if (stream) {
    ...
    stream->format("Total\t\t%d\t%d\t%d\t%d\n", totals.objectsInUse,
                   totals.spaceInUse, totals.objectsDeleted,
                   totals.spaceDeleted);
    stream->format("Number small hunks:\t%d\n", numberSmallHunks);
    stream->format("Number big hunks:\t%d\n", numberBigHunks);

    if (mask & validateMinutia) stream->format("Free blocks:\t");

    int batch = 0;
    int64 orderedSpace = 0;
    int sizes = 0;

    // for (MemFreeBlock *blk = freeBlocks.nextLarger; blk != &freeBlocks; blk =
    // blk->nextLarger)
    for (MemFreeBlock *blk = freeBlockTree.getFirst(); blk;
         blk = blk->getNext()) {
      ++sizes;
      int count = 1;

      for (MemFreeBlock *twin = blk->nextTwin; twin != blk;
           twin = twin->nextTwin)
        ++count;
