...
BERDecode::BERDecode(BERDecode *parent) {
  ...
  if (parent->tagNumber == BIT_STRING) *ptr++;
  level = 0;
}
...
BERDecode::~BERDecode() { delete[] buffer; }
...
UCHAR BERDecode::getOctet() {
  if (ptr >= end) {
    ...
    if (ptr >= end) throw BERException("key overrun");
  }
...
int BERDecode::getLength() {
  ...
  if (c == 0x80)
    ...
    if (c & 0x80)
      for (int count = c & 0x7f; count--;) length = (length << 8) + getOctet();
    else
      length = c;
  }
...
void BERDecode::next(int requiredTagNumber) {
  if (!next()) throw BERException("expected BER object");

  if (tagNumber != requiredTagNumber)
    throw BERException("expected BER object %d, got %d", requiredTagNumber,
                       tagNumber);
}

void BERDecode::pushDown() {
  if (level + 2 >= MAX_LEVELS) throw BERException("max decode levels exceeded");
  ...
  if (tagNumber == BIT_STRING) ++ptr;
}

void BERDecode::popUp() {
  if (level == 0) throw BERException("decode stack underflow");
...
void BERDecode::print(int level) {
  ...
  printf("%*s %s length %d", level * 3, "", (const char *)identifier,
         contentLength);
  ...
  switch (tagNumber) {
    ...
    case INTEGER:
      if (contentLength <= 4)
        ...
        for (int n = 0; n < 5; ++n) printf(" %.2x,", content[n]);
        printf(" ...\n");
      }
      break;
...
int BERDecode::getRawLength() { return end - ptr; }

int BERDecode::getInteger() {
  int value = 0;

  for (int n = 0; n < contentLength; ++n) value = (value << 8) | content[n];
