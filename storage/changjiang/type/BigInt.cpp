...
// Workaround for Visual Studio compiler bug where 32-bit shift right fails in
// optimized binaries.
...
BigInt::BigInt(BigInt *bigInt) { set(bigInt); }
...
void BigInt::set(BigInt *bigInt) {
  ...
  for (int n = 0; n < length; ++n) words[n] = bigInt->words[n];
}
...
void BigInt::set(int64 value, int valueScale) {
  ...
  if (value) {
    ...
    if (highWord) words[length++] = highWord;
  }
}
...
void BigInt::set2(unsigned long value, int valueScale) {
  set((uint64)value, valueScale);
}
...
int BigInt::getByteLength(void) {
  ...
  for (int n = 0; n < 4; ++n) {
    ...
    if (byte) return length * sizeof(BigWord) - n + ((byte & 0x80) ? 1 : 0);
  }
...
void BigInt::getBytes(char *bytes) {
  ...
  // The rest of the words are boring -- just squirt out the byte in the right
  // order

  while (position > 0) {
    BigWord word = words[--position];

    for (int m = 0; m < 4; ++m) *p++ = (char)(word >> byteShifts[m]);
  }
...
void BigInt::setBytes(int scl, int byteLength, const char *bytes) {
  ...
  if (partial == 0) partial = sizeof(BigWord);
...
BigInt BigInt::scaleTo(int toScale, BigInt *result) {
  ...
  if (!powerTableInit) buildPowerTable();
...
void BigInt::setString(int stringLength, const char *string) {
  ...
  for (const char *p = string, *end = string + stringLength;
       p < end && (c = *p++);) {
    if (c >= '0' && c <= '9') {
      multiply(10);
      add(0, c - '0');
...
void BigInt::getString(int bufferLength, char *buffer) {
  ...
  char *q = buffer;
  char *decimalPoint =
      (scale) ? buffer + (p - temp) + scale : buffer + bufferLength;
...
double BigInt::getDouble(void) {
  ...
  for (int n = length - 1; n >= 0; --n) d = d * 4294967296. + words[n];

  if (scale) d = Value::scaleDouble(d, -scale);
...
// If we're higher than the other guy, return 1; if lower, return -1; if equal,
// return 0
...
int BigInt::compare(BigInt *bigInt) {
  ...
  if (bigInt->length == 0) return (neg) ? -1 : 1;

  // If we have different signs, things are also easy

  if (neg != bigInt->neg) return (neg) ? -1 : 1;
...
int BigInt::divide(const BigInt *dividend, const BigInt *divisor,
