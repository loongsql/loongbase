...
void *Stack::mark() { return prior; }

void Stack::pop(void *mark) {
  while (prior && prior != mark) pop();
}
...
bool Stack::isEmpty() { return prior == NULL; }
...
int Stack::count() {
  ...
  for (Stack *node = prior; node; node = node->prior) ++n;

  return n;
}

bool Stack::isMark(void *mark) { return prior == mark; }
...
bool Stack::equal(Stack *stack) {
  ...
  for (; s1 && s2; s1 = s1->prior, s2 = s2->prior)
    if (s1->object != s2->object) return false;
