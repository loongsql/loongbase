...
static int sizes[] = {0, BITS_PER_SEGMENT, BITS_PER_SEGMENT *BITMAP_VECTOR_SIZE,
...
Bitmap::~Bitmap() {
  if (vector) deleteVector(level, vector);
}
...
void Bitmap::set(int32 bitNumber) {
  ...
  for (int seq = 0; seq < max; ++seq) {
    ...
    for (int lvl = level; lvl >= 2; --lvl) {
      ...
      if (!*ptr && lvl > 2) *ptr = allocVector(NULL);
    }
...
void Bitmap::clear(int32 number) {
  if (count == 1 && !vector) {
    if (number == unary) count = 0;

    return;
  }

  if (number >= sizes[level]) return;
...
int32 Bitmap::nextSet(int32 start) {
  if (!vector) {
    if (count == 1) return (start <= unary) ? unary : -1;

    if (level == 0) return -1;
  }

  if (start >= sizes[level]) return -1;
  ...
  for (int lvl = level; lvl <= level;)
    if (lvl == 1) {
      ...
      for (index = indexes[lvl]; index < BITMAP_VECTOR_SIZE && !vec[index];
           ++index)
        for (int n = 0; n < lvl; ++n) indexes[n] = 0;
...
void Bitmap::andBitmap(Bitmap *bitmap) {
  ...
  if (level > bitmap->level) swap(bitmap);

  // If we don't have a vector, we're either empty or unary

  if (!vector) {
    if (count == 1 && !bitmap->isSet(unary)) count = 0;
  ...
  if (level == 1) {
    if (!andSegments((Bms *)vector, (Bms *)vec)) clear();
...
void Bitmap::orBitmap(Bitmap *bitmap) {
  ...
  if (!bitmap->vector) {
    if (bitmap->count == 1) set(bitmap->unary);

    bitmap->release();
    return;
  }

  // Bring ourselves down to the other bitmap's level, if necessary creating
  // intermediates
  ...
  for (lvl = level; lvl > bitmap->level; --lvl) {
    ...
    if (lvl > 2 && !vec[0]) vec[0] = allocVector(NULL);
  ...
  while (lvl <= bitmap->level) {
    ...
    if (index >= BITMAP_VECTOR_SIZE) ++lvl;
  }
...
bool Bitmap::isSet(int32 number) {
  ...
  if (!vector && count == 1) return number == unary;

  if (number >= sizes[level]) return false;
...
void Bitmap::decompose(int32 number, uint *indexes) {
  switch (level) {
    ...
    case 5:
      indexes[5] = (number >> (CLUMP_BITS + INDEX_BITS + 3 * VECTOR_BITS)) &
                   (BITMAP_VECTOR_SIZE - 1);
    case 4:
      indexes[4] = (number >> (CLUMP_BITS + INDEX_BITS + 2 * VECTOR_BITS)) &
                   (BITMAP_VECTOR_SIZE - 1);
    case 3:
      indexes[3] = (number >> (CLUMP_BITS + INDEX_BITS + 1 * VECTOR_BITS)) &
                   (BITMAP_VECTOR_SIZE - 1);
    case 2:
      indexes[2] = (number >> (CLUMP_BITS + INDEX_BITS + 0 * VECTOR_BITS)) &
                   (BITMAP_VECTOR_SIZE - 1);
    case 1:
    case 0:
...
bool Bitmap::andSegments(Bms *segment1, Bms *segment2) {
  ...
  for (BitClump *c1 = segment1->clump, *c2 = segment2->clump,
                *end = c1 + CLUMPS;
       c1 < end;)
    if (*c1++ &= *c2++) hit = true;
  ...
  switch (lvl) {
    ...
    case 2: {
      for (int n = 0; n < BITMAP_VECTOR_SIZE; ++n)
        if ((item = vector[n])) delete (Bms *)item;
      delete[] vector;
    } break;
    ...
    default: {
      --lvl;
      for (int n = 0; n < BITMAP_VECTOR_SIZE; ++n)
        if ((item = vector[n])) deleteVector(lvl, (void **)item);
      delete[] vector;
    }
  }
}
  ...
  for (int seq = 0; seq < max; ++seq) {
    ...
    for (int lvl = level; lvl >= 2; --lvl) {
      ...
      if (!*ptr && lvl > 2) {
        ...
        if (!COMPARE_EXCHANGE_POINTER(ptr, NULL, vec)) delete vec;
      }
    }
...
void Bitmap::unitTest(void) {
  ...
  for (bitNumber = 0; bitNumber < 20000;
       bitNumber += pseudoRand(bitNumber, 200)) {
  ...
  for (bitNumber = 0; bitNumber < 2000000;
       bitNumber += pseudoRand(bitNumber, 20000)) {
    Log::log("Setting bit %d\n", bitNumber);
    tester->set(bitNumber);
  }
  ...
  for (bitNumber = 0;
       (bitNumber = tester->nextSet(pseudoRand(bitNumber, 1800000))) >= 0;) {
    Log::log("Clearing bit %d\n", bitNumber);
    tester->clear(bitNumber);
  }
