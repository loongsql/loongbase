...
static const Commands commands[] = {
    "quit", Quit, "dump", Dump, "next",  Left,  "prior", Right, "read", Read,
    "up",   Up,   "help", Help, "throw", Throw, "break", Break, NULL,   Break};
...
void Debug::dump(Page *page) {
  switch (page->pageType) {
    case PAGE_btree:
      IndexPage::printPage((IndexPage *)page, (bdb) ? bdb->pageNumber : 0,
                           false);
      break;
...
void Debug::execute(IndexPage *indexPage, DebugCommand command,
                    const char *string) {
  switch (command) {
    case Right:
...
bool Debug::execute(const char *buffer) {
  ...
  if (*buffer) printf("don't recognize command \"%s\"\n", buffer);
