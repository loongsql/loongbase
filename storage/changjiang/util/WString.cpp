...
WString::operator const WCHAR *() { return (string) ? string : wcharNull; }
...
WString::WString(WString &source) {
  if ((string = source.string)) ++(string[-1]);
}
...
WString::operator JString() {
  ...
  if (l) {
    ...
    char *q = buffer;
    for (const WCHAR *p = string; *p;) *q++ = (char)*p++;
    *q = 0;
    str.releaseBuffer();
  }
...
int WString::length() { return length(string); }
...
void WString::setString(const WCHAR *wString, int len) {
  ...
  *string++ = 1;
  ...
  for (int n = 0; n < len; ++n) string[n] = (WCHAR)wString[n];
...
int WString::length(const WCHAR *string) {
  ...
  for (const WCHAR *p = string; *p++;) ++count;
