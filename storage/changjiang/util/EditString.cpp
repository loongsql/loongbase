...
void EditString::parse() {
  ...
  while ((c = next()) != 0) {
    ...
    switch (c) {
      ...
      case 'Z':
        if (type != fmtDate) type = fmtNumber;
        ++digits;
        ++zones;
        if (fraction) ++fractions;
        break;
...
void EditString::reset() { repeat = pos = quote = 0; }
...
char EditString::next() {
  for (;;) {
    ...
    if (c == '(') {
      repeat = 0;
      while (pos < stringLength && (c = chars[pos++]) != 0 && c != ')')
        if (c >= '0' && c <= '9') repeat = repeat * 10 + c - '0';
      --repeat;
    } else {
...
char *EditString::formatNumber(Value *value, char *string) {
  ...
  if (!expansion) {
    ...
    reset();
    for (int n = 0; n < length; ++n) expansion[n] = next();
  }
  ...
  if (number != 0)
    for (int n = 0; n < length; ++n) string[n] = '#';
...
char *EditString::formatDate(DateTime date, char *to) {
  ...
  const char *monthString = Changjiang::months[month - 1];
  ...
  const char *dayString = Changjiang::weekDays[time->tm_wday];
  ...
  while ((c = next()) != 0) {
    if (quote == 0) switch (c) {
        case 'D':
          ...
          if (blank && dayPos == 1 && c == '0') c = ' ';
          break;

        case 'J':
          c = digit(julian, julianPos++, julians, true);
          if (julianPos == 1 && c == '0') c = ' ';
          break;
  ...
  while (tpos < length) to[tpos++] = ' ';

  return to;
}

char EditString::digit(int number, int pos, int length, bool blank) {
  for (int n = length - pos - 1; n > 0; --n) number /= 10;

  if (blank && (number == 0)) return ' ';
