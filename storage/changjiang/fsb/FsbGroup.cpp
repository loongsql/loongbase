...
FsbGroup::FsbGroup(NSelect *select, NNode *groupList, Fsb *source) {
  ...
  for (n = 0; n < numberColumns; ++n) valueSlots[n] = statement->getValueSlot();
  ...
  for (n = 0; n < groups->count; ++n) groupSlots[n] = statement->getValueSlot();
}
...
Row *FsbGroup::fetch(Statement *statement) {
  ...
  // Propogate non-statistical values into the result set while we still have
  // them
  ...
  for (bool brk = false; !brk;) {
    ...
    for (n = 0; n < numberColumns; ++n) children[n]->increment(statement);
...
void FsbGroup::close(Statement *statement) { stream->close(statement); }

Value *FsbGroup::getValue(Statement *statement, int index) {
  return &statement->values[valueSlots[index]];
}

int FsbGroup::getNumberValues() { return numberColumns; }
