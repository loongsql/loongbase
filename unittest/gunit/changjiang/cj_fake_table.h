...
#ifndef CJ_FAKE_TABLE_H
#define CJ_FAKE_TABLE_H
...
#include "cj_mock_field_long.h"  // todo: put this #include first
...
*/
class CJ_Fake_TABLE_LIST : public TABLE_LIST {
 public:
  CJ_Fake_TABLE_LIST() = default;
  ~CJ_Fake_TABLE_LIST() = default;
};
...
*/
class CJ_Fake_TABLE_SHARE : public TABLE_SHARE {
  uint32 all_set_buf;
  ...
  */
  CJ_Fake_TABLE_SHARE(uint number_of_columns) {
    ...
    //TODO : hardcoding
    static const char *fakedb = "ldy5";
    static const char *faketable_name = "t1";
    SetName(fakedb, faketable_name);

    EXPECT_EQ(0, bitmap_init(&all_set, &all_set_buf, fields));
    bitmap_set_above(&all_set, 0, true);
  }

  void SetName(const char* fakedb, const char* faketable_name) {
    db.str = const_cast<char *>(fakedb);
    db.length = strlen(db.str);
    table_name.str = const_cast<char *>(faketable_name);
    table_name.length = strlen(table_name.str);
  }
  ~CJ_Fake_TABLE_SHARE() = default;
};
...
*/
class CJ_Fake_TABLE : public TABLE {
  ...
  CJ_Fake_TABLE_SHARE table_share;
 ...
 private:
  void initialize() {
    ...
    pos_in_table_list = new (*THR_MALLOC) CJ_Fake_TABLE_LIST();
  ...
  */
  ...
  explicit CJ_Fake_TABLE(List<Field> fields) : table_share(fields.elements) {
    initialize();
  ...
  explicit CJ_Fake_TABLE(Field *column) : table_share(1) {
    initialize();
    add(column, 0);
  }

  CJ_Fake_TABLE(Field *column1, Field *column2) : table_share(2) {
    initialize();
    add(column1, 0);
    add(column2, 1);
  }

  CJ_Fake_TABLE(Field *column1, Field *column2, Field *column3) : table_share(3) {
    initialize();
    add(column1, 0);
    add(column2, 1);
    add(column3, 2);
  ...
  */
  CJ_Fake_TABLE(int column_count, bool cols_nullable)
  ...
  */
  CJ_Fake_TABLE(std::initializer_list<int> column_values, bool are_nullable = true)
  ...
  void SetName(const char* fakedb, const char* faketable_name) {
    table_share.SetName(fakedb, faketable_name);
  }
 private:
